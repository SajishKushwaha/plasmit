...
const seedSlots: Slot[] = [
  {
    ...
    title: "Open Slot",
    subtitle: "General Cardiology Consultation",
    status: "free",
  },
  {
    id: "s2",
    ...
    title: "Patient: Elena Rodriguez",
    subtitle: "Cancelled by patient",
    status: "cancelled",
  ...
  },
  {
    ...
    subtitle: "Priority: High - Room Reserved",
    status: "blocked",
  },
  {
    id: "s4",
  ...
  },
  {
    ...
    title: "Open Slot",
    subtitle: "Tele-Consultation Ready",
    status: "free",
  },
  {
    id: "s6",
    ...
    subtitle: "Documentation & Reports",
    status: "blocked",
  },
];
...
export default function DoctorAvailabilityPage() {
  ...
  function bulkUpdate(status: SlotStatus) {
    setSlots((current) =>
      current.map((slot) => (selected.includes(slot.id) ? { ...slot, status } : slot)),
    );
    setSelected([]);
  }
        ...
        <header className="flex min-h-16 items-center justify-between rounded-md border border-[#c4d0df] bg-[#f8fbff] px-5">
          <div>
            ...
            <p className="text-xs text-[#51657c]">
              Manage clinical schedule, generated slots, bookings, and cancellations.
            </p>
          </div>
            ...
            <section className="space-y-5">
              <div className="grid gap-3 md:grid-cols-4">
                <StatCard
                  ...
                  value={String(counts.free).padStart(2, "0")}
                  helper="4 added today"
                  tone="text-emerald-700"
                />
                <StatCard
                  label="Booked"
                  value={String(counts.booked).padStart(2, "0")}
                  helper="65% capacity"
                  tone="text-[#075baa]"
                />
                <StatCard
                  label="Blocked"
                  value={String(counts.blocked).padStart(2, "0")}
                  helper="Breaks & admin"
                  tone="text-slate-700"
                />
                <StatCard
                  label="Cancelled"
                  value={String(counts.cancelled).padStart(2, "0")}
                  helper="Today's total"
                  tone="text-red-700"
                />
              </div>
              ...
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
                <div className="rounded-md border border-[#c4d0df] bg-white">
                  <div className="flex items-center justify-between border-b border-[#dbe3ee] px-4 py-3">
                    ...
                    <button
                      className="rounded-md bg-[#075baa] px-4 py-2 text-sm font-bold text-white"
                  ...
                  <div className="divide-y divide-[#e1e8f0]">
                    {slots.map((slot) => (
                      <div
                        className={`grid gap-3 p-4 md:grid-cols-[90px_minmax(0,1fr)_auto] ${statusStyle[slot.status]}`}
                        key={slot.id}
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {slot.status === "free" ? (
                            <>
                              <input
                                className="h-9 w-40 rounded-md border border-[#c4d0df] bg-white px-2 text-xs"
                                onChange={(event) => setPatientName(event.target.value)}
                                placeholder="Patient name"
                                value={patientName}
                              />
                              <button
                                className="rounded-md bg-[#075baa] px-3 py-2 text-xs font-bold text-white"
                                onClick={() => updateSlot(slot.id, "booked")}
                                type="button"
                              >
                                Accept
                              </button>
                              <button
                                className="rounded-md border border-[#c4d0df] bg-white px-3 py-2 text-xs font-bold"
                          ...
                          {slot.status === "cancelled" || slot.status === "blocked" ? (
                            <button
                              className="rounded-md border border-[#c4d0df] bg-white px-3 py-2 text-xs font-bold"
                              onClick={() => updateSlot(slot.id, "free")}
                              type="button"
                            >
                              Re-open Slot
                            </button>
                ...
                <div className="space-y-4">
                  <button
                    className="w-full rounded-md bg-[#075baa] p-4 text-left text-white"
                    onClick={() => setScreen("generate")}
                    type="button"
                  >
                    <div className="text-xs uppercase">Next Available</div>
                    <div className="mt-2 text-2xl font-bold">
                      {slots.find((slot) => slot.status === "free")?.time ?? "--:--"}
                    </div>
                    <div className="mt-1 text-xs">Today, Room 4B</div>
                  </button>
                  <div className="rounded-md border border-[#c4d0df] bg-white p-4">
                    <div className="font-bold">Optimization Tip</div>
                    <p className="mt-2 text-xs text-[#51657c]">
                      Based on rush history, add two extra slots between 4 PM and 6 PM.
                    </p>
                    <button
                      className="mt-4 w-full rounded-md bg-[#075baa] px-4 py-2 text-sm font-bold text-white"
                      onClick={() => setScreen("generate")}
                      type="button"
                    >
                      Apply Suggestion
              ...
              <div className="space-y-4">
                <div className="rounded-md border border-[#c4d0df] bg-white p-5">
                  ...
                  <div className="mt-4 flex gap-2">
                    {["Today", "Tomorrow", "Next 7 Days"].map((item) => (
                      <button
                        className="rounded-md border border-[#c4d0df] bg-white px-4 py-2 text-sm font-semibold first:bg-[#dbeafe]"
                        key={item}
                        type="button"
                      >
                        {item}
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    <label className="text-xs font-bold uppercase">
                      Start Date
                      <input
                        className="mt-2 h-10 w-full rounded-md border border-[#c4d0df] bg-[#eef4fb] px-3"
                  ...
                  <div className="mt-5">
                    ...
                    <div className="mt-2 grid grid-cols-7 gap-2 rounded-md bg-[#e7eff8] p-2">
                      {["M", "T", "W", "T", "F", "S", "S"].map((day, index) => (
                        <button
                          className={`h-9 rounded-md border text-sm font-bold ${[0, 2, 4].includes(index) ? "bg-[#075baa] text-white" : "bg-white text-[#51657c]"}`}
                          key={`${day}-${index}`}
                          type="button"
                        >
                          {day}
                ...
                <div className="rounded-md border border-[#c4d0df] bg-white p-5">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    {["OPD", "Telemedicine", "Procedure"].map((item) => (
                      <button
                        className="rounded-md border border-[#c4d0df] bg-white p-3 text-left font-bold first:border-[#075baa] first:bg-[#edf5ff]"
                        key={item}
                        type="button"
                      >
                        {item}
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <div className="rounded-md border border-[#075baa] bg-[#edf5ff] p-4">
                      <MapPin className="mb-2 h-5 w-5" />
                      <b>Main Campus</b>
                      <div className="text-xs text-[#51657c]">Block B, 4th Floor</div>
                    </div>
                    <div className="rounded-md border border-[#c4d0df] bg-white p-4">
                      <CalendarDays className="mb-2 h-5 w-5" />
                      <b>Downtown Clinic</b>
                      <div className="text-xs text-[#51657c]">Suite 102</div>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="rounded-md border border-[#c4d0df] bg-[#e7eff8] p-5">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <b>Active Days</b>
                      <div>M, W, F</div>
                      <div className="text-xs text-[#51657c]">Oct 24 - Nov 24</div>
                    </div>
                  </div>
                  <div className="mt-5 flex gap-3">
                    ...
                    <div>
                      <b>Daily Shift</b>
                      <div>09:00 - 14:00</div>
                      <div className="text-xs text-[#51657c]">{interval} min slots</div>
                    </div>
                  </div>
                  <div className="mt-6 border-t border-[#c4d0df] pt-5">
                    <div className="flex items-center justify-between">
                      <span className="font-bold">Total New Slots</span>
                      <span className="text-4xl font-bold text-[#075baa]">
                        {generatedPreview.length}
                      </span>
                    </div>
                    <button
                      className="mt-5 flex w-full items-center justify-center gap-2 rounded-md bg-[#075baa] px-4 py-3 font-bold text-white"
                      onClick={publishGeneratedSlots}
                      type="button"
                    >
                      Generate Slots <ChevronRight className="h-4 w-4" />
                    </button>
                  </div>
                </div>
              </div>
            ...
            <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
              <div className="rounded-md border border-[#c4d0df] bg-white p-4">
                <div className="flex flex-wrap items-center gap-2 rounded-md border border-[#c4d0df] bg-[#eaf2fb] p-3">
                  <b>Bulk Actions:</b>
                  <button
                    className="rounded-md bg-green-700 px-3 py-2 text-xs font-bold text-white"
                    onClick={() => bulkUpdate("free")}
                    type="button"
                  >
                    Accept All
                  </button>
                  <button
                    className="rounded-md border border-[#c4d0df] bg-white px-3 py-2 text-xs font-bold"
                    onClick={() => bulkUpdate("blocked")}
                    type="button"
                  >
                    Block Selected
                  </button>
                  <button
                    className="rounded-md border border-orange-200 bg-orange-50 px-3 py-2 text-xs font-bold text-orange-700"
                    onClick={() => bulkUpdate("break")}
                    type="button"
                  >
                    Mark as Break
                  </button>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  {slots.map((slot) => (
                    <label
                      className={`min-h-20 rounded-md border p-3 ${statusStyle[slot.status]}`}
                      key={slot.id}
                    >
                      <div className="flex gap-2">
                        ...
                        <div>
                          ...
                          <div className="mt-1 text-xs">{slot.subtitle}</div>
                        </div>
                      </div>
                    </label>
                  ))}
              ...
              <div className="space-y-4">
                <div className="rounded-md border border-[#c4d0df] bg-white p-4">
                  ...
                  <div className="mt-4 space-y-2 text-sm">
                    ...
                    <div className="flex justify-between text-[#075baa]">
                      <span>Booked</span>
                      <b>{counts.booked}</b>
                    </div>
                ...
                <button
                  className="flex w-full items-center justify-center gap-2 rounded-md bg-[#075baa] px-4 py-3 font-bold text-white"
                  type="button"
                >
                ...
                <button
                  className="flex w-full items-center justify-center gap-2 rounded-md border border-[#c4d0df] bg-white px-4 py-3 font-bold"
                  onClick={() => setScreen("generate")}
                  type="button"
                >
                ...
                <button
                  className="flex w-full items-center justify-center gap-2 rounded-md bg-transparent px-4 py-3 font-bold"
