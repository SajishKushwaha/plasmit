...
function childIsActive(
  ...
  return (
    routeIsActive(child.route, pathname, currentHash, currentSearch) ||
    (child.children?.some((nestedChild) =>
      childIsActive(nestedChild, pathname, currentHash, currentSearch),
    ) ??
      false)
  );
}
      ...
      return (
        <div key={child.id}>
          ...
          {expanded ? (
            <div
              className={cn(
                "mt-1 space-y-1 border-l border-border pl-2",
                depth === 0 ? "ml-3" : "ml-2",
              )}
            >
            ...
            <div className="space-y-1">
              {visibleItems
                ...
                .map((item) => {
                  ...
                  const exactRouteActive = routeIsActive(
                    item.route,
                    pathname,
                    currentHash,
                    currentSearch,
                  ...
                  const childActive =
                    item.children?.some((child) =>
                      childIsActive(child, pathname, currentHash, currentSearch),
                    ) ?? false;
                  const moreSpecificRouteActive = visibleItems.some((candidate) => {
                    ...
                    const candidateMatches =
                      ...
                      (!candidateRouteHash &&
                        candidateRoutePath !== "/dashboard" &&
                        candidateRoutePath !== "/doctor-dashboard" &&
                        pathname.startsWith(`${candidateRoutePath}/`));
                      ...
                      <div key={item.id}>
                        <button
                          className={cn(
                            ...
                            active &&
                              "bg-gradient-to-r from-[#7367f0] to-[#5b8def] text-sidebar-active-foreground shadow-[0_8px_20px_rgba(115,103,240,0.24)] hover:text-white",
                            neuroIcu &&
                              !active &&
                              "hover:bg-[linear-gradient(90deg,rgba(79,110,247,0.10),rgba(124,107,255,0.10))] hover:text-[#4F6EF7]",
                            neuroIcu &&
                              active &&
                              "bg-[linear-gradient(90deg,#4F6EF7,#7C6BFF)] shadow-[0_10px_26px_rgba(79,110,247,0.34),0_0_0_1px_rgba(124,107,255,0.22)]",
                          )}
                          onClick={() =>
                            setOpenItems((curr) => ({ ...curr, [item.id]: !expanded }))
                          }
                          type="button"
                        >
