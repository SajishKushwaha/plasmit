...
const timepointOptions: MonitoringScheduleItem["label"][] = wholeBloodMonitoringSchedule.map(
  (item) => item.label,
);
...
function makeRow(
  label: MonitoringScheduleItem["label"],
  patch: Partial<VitalsRow> = {},
): VitalsRow {
  return {
...
function getScheduleFor(componentType: string) {
  return componentType === "Packed Red Cells" ||
    componentType === "Whole Blood / Leucoreduced Red Cells"
    ? wholeBloodMonitoringSchedule
    : otherComponentMonitoringSchedule;
}
...
function validateRow(row: VitalsRow, index: number): RowErrors {
  ...
  else if (!datePattern.test(row.date))
    errors.date = `Entry ${index + 1}: date must be in DD Mon YY format.`;
  if (!row.time) errors.time = `Entry ${index + 1}: time is required.`;
  else if (!timePattern.test(row.time))
    errors.time = `Entry ${index + 1}: time must be in HH:MM format.`;
  if (!row.timepointLabel)
    errors.timepointLabel = `Entry ${index + 1}: timepoint label is required.`;
  if (!row.pulse) errors.pulse = `Entry ${index + 1}: pulse is required.`;
  else if (!/^\d+$/.test(row.pulse) || Number(row.pulse) < 0 || Number(row.pulse) > 250)
    errors.pulse = `Entry ${index + 1}: pulse must be 0-250.`;
  if (!row.systolic) errors.systolic = `Entry ${index + 1}: systolic BP is required.`;
  else if (!/^\d+$/.test(row.systolic) || Number(row.systolic) < 0 || Number(row.systolic) > 300)
    errors.systolic = `Entry ${index + 1}: systolic BP must be 0-300.`;
  if (!row.diastolic) errors.diastolic = `Entry ${index + 1}: diastolic BP is required.`;
  else if (!/^\d+$/.test(row.diastolic) || Number(row.diastolic) < 0 || Number(row.diastolic) > 200)
    errors.diastolic = `Entry ${index + 1}: diastolic BP must be 0-200.`;
  if (!row.temperature) errors.temperature = `Entry ${index + 1}: temperature is required.`;
  else if (
    !/^\d+(\.\d)?$/.test(row.temperature) ||
    Number(row.temperature) < 30 ||
    Number(row.temperature) > 43
  )
    errors.temperature = `Entry ${index + 1}: temperature must be 30.0-43.0 C.`;
  if (!row.respiratoryRate)
    errors.respiratoryRate = `Entry ${index + 1}: respiratory rate is required.`;
  else if (
    !/^\d+$/.test(row.respiratoryRate) ||
    Number(row.respiratoryRate) < 0 ||
    Number(row.respiratoryRate) > 60
  )
    errors.respiratoryRate = `Entry ${index + 1}: respiratory rate must be 0-60.`;
...
export function NurseBloodAdministrationPage() {
  ...
  const [symptoms, setSymptoms] = React.useState<Record<ReactionSymptom, boolean>>(
    Object.fromEntries(reactionSymptoms.map((symptom) => [symptom, false])) as Record<
      ReactionSymptom,
      boolean
    >,
  );
  ...
  function addRow() {
    const nextTimepoint =
      schedule[Math.min(rows.length, schedule.length - 1)]?.label ?? "Ad-hoc / Reaction";
  ...
  const scheduleSatisfied = scheduleLabels.every((label) =>
    rows.some((row) => row.timepointLabel === label),
  );
  const requiresFinalPost =
    bloodAdministrationEpisode.componentType === "Packed Red Cells" ||
    bloodAdministrationEpisode.componentType === "Whole Blood / Leucoreduced Red Cells";
  const hasFinalCompletion = rows.some(
    (row) => row.timepointLabel === "At completion of each pack",
  );
  const hasPostCompletion =
    !requiresFinalPost || rows.some((row) => row.timepointLabel === "One hour after completion");
  const reactionFieldsComplete =
    !reactionTriggered ||
    (reactionDescription.trim().length > 0 && Object.values(actionsTaken).some(Boolean));
  const canComplete =
    !hasRowErrors &&
    scheduleSatisfied &&
    hasFinalCompletion &&
    hasPostCompletion &&
    reactionFieldsComplete;
    ...
    <div className="space-y-6">
                ...
                <thead className="bg-gradient-to-r from-stone-100 to-stone-50">
                  <tr>
                    {[
                      ...
                      <th
                        key={heading}
                        className="border border-stone-200 px-3 py-3 text-left font-semibold text-stone-700 md:px-4"
                      >
                        {heading}
                      </th>
                    ))}
                    ...
                    >
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <select
                          ...
                          onChange={(event) =>
                            updateRow(row.id, {
                              timepointLabel: event.target.value as MonitoringScheduleItem["label"],
                            })
                          }
                        >
                      ...
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput
                          value={row.pulse}
                          onChange={(value) => updateRow(row.id, { pulse: value })}
                        />
                      ...
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput
                          value={row.systolic}
                          onChange={(value) => updateRow(row.id, { systolic: value })}
                        />
                      ...
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput
                          value={row.diastolic}
                          onChange={(value) => updateRow(row.id, { diastolic: value })}
                        />
                      ...
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput
                          ...
                          onChange={(value) => updateRow(row.id, { temperature: value })}
                        />
                      ...
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput
                          ...
                          onChange={(value) => updateRow(row.id, { respiratoryRate: value })}
                        />
                        <InlineError message={rowErrors[index]?.respiratoryRate ?? ""} />
                      </td>
                      <td className="border border-stone-200 px-3 py-3 md:px-4">
                        <CellInput value={`${row.nurseName} / ${row.clockNo}`} readOnly />
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <CardContent className="space-y-4">
          ...
          {reactionTriggered ? (
            <div className="mt-6 space-y-4">
              ...
              >
                Stop transfusion immediately, inform doctor and Blood Bank, retain the blood bag,
                and keep the IV line patent.
              </AlertBanner>

              <div>
                <p className="mb-2 text-sm font-semibold uppercase tracking-wide text-stone-700">
                ...
                <InlineError
                  message={
                    reactionDescription.trim()
                      ? ""
                      : "Required when any reaction symptom is selected."
                  }
                />
              </div>

              <div>
                <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-stone-700">
                  Actions Taken
                </p>
                <div className="grid gap-3 md:grid-cols-2">
                  {[
                    ...
                    <label
                      key={action}
                      className="flex cursor-pointer items-center gap-2 rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm text-stone-800 transition-colors hover:bg-stone-100"
                    >
                      <input
                        type="checkbox"
                        checked={Boolean(actionsTaken[action])}
                ...
                <InlineError
                  message={
                    Object.values(actionsTaken).some(Boolean)
                      ? ""
                      : "At least one action is required when reaction symptoms are selected."
                  }
                />
              </div>
      ...
      {showReactionReportPrompt ? (
        <AlertBanner icon={AlertTriangle} tone="warning" title="Transfusion Reaction Report">
          Open this workflow to document the reaction details, retain the bag and samples, record
          compatibility form handling, notify doctor and Blood Bank, and close the episode only
          after the reaction fields are complete.
        </AlertBanner>
      ) : null}
      {episodeCompleted ? (
        <AlertBanner icon={ShieldAlert} tone="success" title="Flowsheet completed">
          Monitoring episode closed for {bloodAdministrationEpisode.patientName}.
        </AlertBanner>
      ) : null}
    </div>
  );
...
function SignCard({ title, name, time }: { title: string; name: string; time: string }) {
  return (
    <div className="rounded-lg border border-stone-300 bg-white p-4 shadow-sm">
      <div className="mb-3 text-xs font-semibold uppercase tracking-wide text-stone-600">
        [ ] {title}
      </div>
      <div className="mt-2">
