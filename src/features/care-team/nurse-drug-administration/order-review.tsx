  ...
  if (action === "Status") {
    return (
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <ReadOnlyInput
          label="Administered"
          value={`${order.administeredQty}/${order.receivedQty}`}
        />
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-md border border-border bg-surface-muted p-3 text-sm">
          Pharmacy dispensed {order.dispensedQty} of {order.orderedQty}. Receive is{" "}
          {canReceive ? "enabled" : "disabled"} for this order.
        </div>
        <Input
          type="number"
          min={0}
          max={Math.max(order.dispensedQty - order.receivedQty, 0)}
          defaultValue={Math.max(order.dispensedQty - order.receivedQty, 0)}
      ...
      <div className="grid gap-2 sm:grid-cols-4">
        ...
        <label className="space-y-1">
          ...
          <Input
            type="number"
            min={0}
            max={remainingQty(order)}
            defaultValue={remainingQty(order)}
          />
        </label>
      </div>
    );
...
function CategoryList({
  ...
  onCategoryChange: (_category: NurseDrugOrder["category"]) => void;
}) {
  return (
...
function OrderCard({
  ...
  order: NurseDrugOrder;
  onAction: (_order: NurseDrugOrder, _action: NurseOrderAction | "Status") => void;
}) {
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_150px_320px] lg:items-start">
        <div className="min-w-0 space-y-1">
          ...
          <div className="text-xs text-muted-foreground">
            {order.form} / {order.dosage || "-"} / {order.frequency || "-"} / {order.days || "-"}{" "}
            days
          </div>
          <div className="text-xs text-muted-foreground">
            Instructions: {order.instructions || "-"}{" "}
            {order.taperedDose ? `(Tapered dose: ${order.taperedDose})` : ""}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
          <Button
            ...
            title={
              canReceive
                ? `Dispensed pending: ${order.dispensedQty - order.receivedQty}`
                : "Receive disabled after receiving dispensed drugs"
            }
          ...
          <Button
            ...
            size="sm"
            variant="outline"
            onClick={() => onAction(order, "Discontinue")}
          >
            Discontinue
          </Button>
          <Button
            ...
            size="sm"
            variant="outline"
            title={`Remaining quantity: ${remainingQty(order)}`}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Status opens detail popup. Action buttons open accept popups.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {categoryOrders.length ? (
          ...
          <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">
            No {activeCategory.toLowerCase()} drug orders found.
          </div>
        )}
      </CardContent>
    </Card>
...
export function DrugOrderReviewTab({ orders }: { orders: NurseDrugOrder[] }) {
  ...
  return (
    <>
      ...
      <OrderActionPopup
        open={Boolean(popupOrder && popupAction)}
        action={popupAction}
        order={popupOrder}
        onOpenChange={(open) => !open && setPopupOrder(null)}
      />
    </>
  );
}
