...
type FollowUpStatus =
  "Pending" | "In Progress" | "Completed" | "Overdue" | "Escalated" | "Not Required";
...
const initialLogs: CommunicationLog[] = [
  {
    ...
    uhid: "UHID-ICU-1024",
    bed: "ICU-Bed-05",
    type: "Critical Condition Update",
    attendees: "Wife, Son, Intensivist",
    summary:
      "Current condition, ventilator support, treatment response, and next review explained.",
    questions: 3,
  ...
  },
  {
    ...
    attendees: "Husband, Family Coordinator",
    summary:
      "Updated family about overnight stability, current medicines, and planned investigations.",
    questions: 1,
  ...
  },
  {
    ...
    uhid: "UHID-ICU-1026",
    bed: "ICU-Bed-08",
    type: "Consent Discussion",
    attendees: "Daughter, Brother, Doctor",
    summary: "Shock status, central-line need, benefits, risks, and alternatives discussed.",
    questions: 2,
  ...
  },
  {
    ...
    uhid: "UHID-ICU-1027",
    bed: "ICU-Bed-04",
    type: "Complaint / Concern Discussion",
    attendees: "Husband, ICU Head",
    summary:
      "Concern about delayed update acknowledged; communication schedule agreed with family.",
    questions: 4,
    consentStatus: "Pending",
  ...
  },
  {
    ...
    uhid: "UHID-ICU-1024",
    bed: "ICU-Bed-05",
    type: "Procedure Explanation",
    attendees: "Wife, Senior Resident",
    summary: "Ventilator weaning steps and possible tracheostomy pathway explained.",
    questions: 2,
    consentStatus: "Pending",
    followUpStatus: "Pending",
    recordedBy: "Dr. Mehta",
  ...
  },
  {
    ...
    bed: "ICU-Bed-02",
    type: "General Query",
    attendees: "Husband, Bedside Nurse",
    summary: "Diet, visitation timing, and expected transfer milestones clarified.",
    questions: 3,
    consentStatus: "Not Required",
    followUpStatus: "Completed",
    recordedBy: "Anjali Verma",
  ...
  },
  {
    ...
    bed: "ICU-Bed-08",
    type: "Procedure Explanation",
    attendees: "Daughter, Intensivist",
    summary: "Central-line procedure outcome and monitoring plan discussed.",
    questions: 1,
    consentStatus: "Obtained",
    followUpStatus: "Completed",
    recordedBy: "Dr. Rao",
  ...
  },
  {
    ...
    type: "General Query",
    attendees: "Husband, ICU Head",
    summary: "Current treatment response and timing of the next consultant update discussed.",
    questions: 2,
    consentStatus: "Pending",
...
const initialFollowUps: FollowUp[] = [
  {
    id: "FU-2001",
    communicationId: "FAM-1001",
    patientName: "Ramesh Kumar",
    bed: "ICU-Bed-05",
    issue: "Family requested update about ventilator removal plan",
    assignedTo: "Dr. Mehta",
    due: "Today, 05:00 PM",
    priority: "High",
    status: "Pending",
  ...
  {
    id: "FU-2002",
    communicationId: "FAM-1004",
    patientName: "Kavita Singh",
    bed: "ICU-Bed-04",
    issue: "Family concern regarding delay in update",
    assignedTo: "ICU Head",
    due: "Today, 02:00 PM",
    priority: "Sensitive",
    status: "Overdue",
  ...
  {
    id: "FU-2003",
    communicationId: "FAM-1003",
    patientName: "Mohan Verma",
    bed: "ICU-Bed-08",
    issue: "Post central-line procedure update",
    assignedTo: "Dr. Iyer",
    due: "Today, 12:00 PM",
    priority: "High",
    status: "Completed",
  ...
  {
    id: "FU-2004",
    communicationId: "FAM-1002",
    patientName: "Anjali Sharma",
    bed: "ICU-Bed-02",
    issue: "Clarify discharge readiness after rounds",
  ...
  {
    id: "FU-2005",
    communicationId: "FAM-1005",
    patientName: "Ramesh Kumar",
    bed: "ICU-Bed-05",
    issue: "Consent counselling review by senior doctor",
    assignedTo: "Dr. Mehta",
    due: "Today, 06:30 PM",
    priority: "Critical",
    status: "Pending",
...
export function FamilyCommunicationWorkspace() {
  ...
  const visibleLogs = logs.filter((row) => {
    ...
    const searchable =
      `${row.id} ${row.patientName} ${row.uhid} ${row.bed} ${row.type} ${row.attendees} ${row.summary} ${row.recordedBy}`.toLowerCase();
    return (
  ...
  const metrics = [
    ...
    {
      label: "Updates Today",
      value: logs.filter((row) => row.dateLabel === "Today").length,
      icon: RefreshCcw,
      tone: "info" as StatusTone,
    },
    {
      label: "Consent Pending",
      value: logs.filter((row) => row.consentStatus === "Pending").length,
      icon: FileQuestion,
      tone: "warning" as StatusTone,
    },
    {
      label: "Follow-up Required",
      value: logs.filter((row) => !["Completed", "Not Required"].includes(row.followUpStatus))
        .length,
      icon: ClipboardCheck,
      tone: "warning" as StatusTone,
    },
    {
      label: "High Priority Discussions",
      value: logs.filter((row) => ["Critical", "Sensitive"].includes(row.priority)).length,
      icon: ShieldAlert,
      tone: "danger" as StatusTone,
    },
    {
      label: "Completed Updates",
      value: logs.filter((row) => row.reviewStatus === "Reviewed").length,
      icon: CheckCircle2,
  ...
  function saveCommunication() {
    ...
    if (editingLog) {
      setLogs((current) =>
        current.map((row) => (row.id === editingLog.id ? { ...row, ...draft } : row)),
      );
  ...
  function setReviewStatus(id: string, status: ReviewStatus) {
    setLogs((current) =>
      current.map((row) => (row.id === id ? { ...row, reviewStatus: status } : row)),
    );
  ...
  function updateFollowUp(id: string, patch: Partial<FollowUp>) {
    ...
    if (item && patch.status) {
      setLogs((current) =>
        current.map((row) =>
          row.id === item.communicationId
            ? { ...row, followUpStatus: patch.status as FollowUpStatus }
            : row,
        ),
      );
    }
  ...
  return (
    <div className="w-full min-w-0 max-w-full space-y-4 overflow-x-hidden [&>*]:min-w-0 [&>*]:max-w-full">
        ...
        >
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block truncate text-xs text-muted-foreground">
              {visibleLogs.length} communication record(s)
            </span>
          </span>
          ...
          <CardContent className="space-y-3 border-t border-border p-3">
            ...
            <div className="grid min-w-0 gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(220px,1.6fr)_repeat(7,minmax(112px,1fr))]">
              <label className="relative min-w-0">
                ...
                <input
                  className="h-9 w-full min-w-0 rounded-md border border-input bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-ring/20"
              ...
              <FilterSelect
                ...
                value={bed}
                onChange={setBed}
                options={["All ICU beds", ...beds]}
              />
              <FilterSelect
                ariaLabel="Doctor or recorder"
                value={doctor}
                onChange={setDoctor}
              ...
              <FilterSelect
                ...
                value={type}
                onChange={setType}
                options={["All types", ...communicationTypes]}
              />
              <FilterSelect
                ariaLabel="Consent"
                value={consent}
                onChange={setConsent}
                options={["All consent", "Pending", "Obtained", "Not Required"]}
              />
              <FilterSelect
                ariaLabel="Follow-up"
                value={followUp}
              ...
              <FilterSelect
                ...
                options={["All priority", "Normal", "High", "Sensitive", "Critical"]}
              />
            </div>
            {advancedOpen ? (
              <div className="grid min-w-0 gap-2 border-t border-border pt-3 sm:grid-cols-2 lg:grid-cols-[minmax(180px,240px)_minmax(180px,240px)]">
                <input
                  ...
                  className="h-9 min-w-0 rounded-md border border-input bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring/20"
                  type="date"
                  value={date}
            ...
            <div className="grid gap-2 sm:flex sm:flex-wrap sm:justify-end">
              <Button className="w-full sm:w-auto" variant="outline" onClick={resetFilters}>
                <X className="h-4 w-4" />
                Reset
              </Button>
              <Button
                className="w-full sm:w-auto"
                onClick={() => toast.success(`${visibleLogs.length} communication logs matched`)}
              >
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-border bg-border shadow-sm lg:grid-cols-3 xl:grid-cols-6">
        {metrics.map((item) => (
          <CommunicationMetric key={item.label} {...item} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-1 rounded-md border border-border bg-white p-1 shadow-sm">
        ...
        />
        <SectionTab
          active={activeSection === "follow-ups"}
        ...
        <SectionTab
          ...
          count={logs.filter((row) => row.reviewStatus !== "Reviewed").length}
          label="Review"
      ...
      {activeSection === "follow-ups" ? (
        <FollowUpTracker
          ...
          onView={(row) =>
            setSelectedLog(logs.find((item) => item.id === row.communicationId) ?? null)
          }
        />
      ) : null}

      {activeSection === "review" ? (
        <CommunicationReviewOverview
          page={safeReviewPage}
          pages={reviewPages}
      ...
      <CenterModal
        ...
        title={selectedLog?.id ?? "Communication"}
        description={selectedLog ? `${selectedLog.patientName} | ${selectedLog.bed}` : undefined}
      >
        {selectedLog ? <CommunicationDetails row={selectedLog} /> : null}
      </CenterModal>

      <CenterModal
        open={Boolean(followUpEditor)}
        onOpenChange={(open) => !open && setFollowUpEditor(null)}
        title="Update follow-up"
        description={followUpEditor?.id}
      >
        {followUpEditor ? (
          <div className="space-y-4">
            <FormField label="Issue / question">
              <textarea
                className="min-h-24 w-full rounded-md border border-input bg-background p-3 text-sm"
                value={followUpEditor.issue}
                onChange={(event) =>
                  setFollowUpEditor({ ...followUpEditor, issue: event.target.value })
                }
              />
            </FormField>
            <div className="grid gap-3 sm:grid-cols-2">
              <FormField label="Assigned to">
                <input
                  ...
                  value={followUpEditor.assignedTo}
                  onChange={(event) =>
                    setFollowUpEditor({ ...followUpEditor, assignedTo: event.target.value })
                  }
                />
              </FormField>
              <FormField label="Status">
                <FilterSelect
                  ariaLabel="Follow-up status"
                  value={followUpEditor.status}
                  onChange={(value) =>
                    setFollowUpEditor({ ...followUpEditor, status: value as FollowUp["status"] })
                  }
                  options={["Pending", "In Progress", "Completed", "Overdue", "Escalated"]}
                />
              </FormField>
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setFollowUpEditor(null)}>
                Cancel
              </Button>
              <Button
                onClick={() => {
                  updateFollowUp(followUpEditor.id, followUpEditor);
                  toast.success(`${followUpEditor.id} updated`);
...
function CommunicationLogTable({
  ...
  onView: (_row: CommunicationLog) => void;
  onEdit: (_row: CommunicationLog) => void;
}) {
  return (
            ...
            rows.map((row) => (
              <article className="space-y-2 p-3" key={`mobile-${row.id}`}>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  <Badge tone={consentTone(row.consentStatus)}>Consent: {row.consentStatus}</Badge>
                  <Badge tone={followUpTone(row.followUpStatus)}>
                    Follow-up: {row.followUpStatus}
                  </Badge>
                </div>
                <div className="flex items-center justify-between gap-2 border-t border-border pt-2">
                  <span className="text-xs text-muted-foreground">
                    {row.id} | {row.dateLabel}, {row.timeLabel}
                  </span>
            ...
            <thead className="bg-surface-muted text-xs uppercase text-muted-foreground">
              <tr>
                {[
                  ...
                  <th className="border-b border-border px-3 py-2 text-left" key={label}>
                    {label}
                  </th>
                ))}
              </tr>
              ...
              {rows.length ? (
                rows.map((row) => (
                  <tr className="border-b border-border last:border-0" key={row.id}>
                    ...
                    <Cell strong>
                      {row.type}
                      <span className="mt-0.5 block max-w-40 truncate text-xs font-normal text-muted-foreground">
                        {row.attendees}
                      </span>
                    </Cell>
                    <Cell>
                      <span className="line-clamp-2 max-w-[220px]">{row.summary}</span>
                    </Cell>
                    <Cell strong>{row.questions}</Cell>
                    <Cell>
                      <div className="flex max-w-36 flex-wrap gap-1">
                        <Badge tone={consentTone(row.consentStatus)}>{row.consentStatus}</Badge>
                        <Badge tone={followUpTone(row.followUpStatus)}>{row.followUpStatus}</Badge>
                      </div>
                    </Cell>
                    <Cell strong>{row.recordedBy}</Cell>
                    <Cell>
                      <Badge tone={priorityTone(row.priority)}>{row.priority}</Badge>
                    </Cell>
...
function FollowUpTracker({
  ...
  rows: FollowUp[];
  onEdit: (_row: FollowUp) => void;
  onComplete: (_row: FollowUp) => void;
  onEscalate: (_row: FollowUp) => void;
  onView: (_row: FollowUp) => void;
}) {
  return (
    <Card className="min-w-0 overflow-hidden">
          ...
          {rows.map((row) => (
            <article className="space-y-2 p-3" key={`mobile-${row.id}`}>
              <div className="flex items-start justify-between gap-2">
                ...
                <Badge tone={followUpTone(row.status)}>{row.status}</Badge>
              </div>
            ...
            <thead className="bg-surface-muted text-xs uppercase text-muted-foreground">
              <tr>
                {[
                  ...
                  <th className="border-b border-border px-3 py-2.5 text-left" key={label}>
                    {label}
                  </th>
                ))}
              </tr>
...
function CommunicationReviewOverview({
  rows,
  total,
  page,
  pages,
  ...
  onPage: (_page: number) => void;
  onView: (_row: CommunicationLog) => void;
  onStatus: (_id: string, _status: ReviewStatus) => void;
}) {
  return (
    <Card className="min-w-0 overflow-hidden">
      <CardContent className="min-w-0 p-0">
        <div className="divide-y divide-border md:hidden">
          {rows.map((row) => (
            <article className="space-y-2 p-3" key={`mobile-review-${row.id}`}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="font-semibold">{row.patientName}</div>
                  <div className="text-xs text-muted-foreground">
                    {row.bed} | {row.dateLabel}, {row.timeLabel}
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between gap-2 border-t border-border pt-2">
                <Badge tone={priorityTone(row.priority)}>{row.priority}</Badge>
                <ReviewActions row={row} onView={onView} onStatus={onStatus} />
              </div>
            </article>
          ))}
        </div>
        <div className="hidden max-w-full overflow-x-auto overscroll-x-contain md:block">
          <table className="w-full min-w-[820px] border-collapse text-sm">
            ...
            <tbody>
              {rows.map((row) => (
                <tr className="border-b border-border last:border-0" key={`review-${row.id}`}>
                  <Cell strong>
                    {row.patientName}
                    <span className="block text-xs font-normal text-muted-foreground">
                      {row.bed}
...
function ReviewActions({
  ...
  return (
    <div className="flex gap-1">
      ...
      <Button
        aria-label={`Clarify ${row.id}`}
        size="icon"
        title="Ask clarification"
        variant="outline"
        onClick={() => onStatus(row.id, "Needs Clarification")}
      >
        <FileQuestion className="h-4 w-4" />
      </Button>
      <Button
        aria-label={`Mark ${row.id} reviewed`}
        size="icon"
        title="Mark reviewed"
        onClick={() => onStatus(row.id, "Reviewed")}
      >
        <CheckCircle2 className="h-4 w-4" />
      </Button>
      <Button
        ...
        title="Escalate"
        variant="outline"
        onClick={() => onStatus(row.id, "Escalated")}
      >
        <ShieldAlert className="h-4 w-4" />
      </Button>
    </div>
...
function CommunicationFormModal({
  ...
  open: boolean;
  editing: CommunicationLog | null;
  draft: CommunicationDraft;
  onDraft: (_draft: CommunicationDraft) => void;
  onClose: () => void;
  onSave: () => void;
}) {
  return (
    <CenterModal
      ...
      title={editing ? `Edit ${editing.id}` : "Add Family Update"}
      description="Document the discussion, consent, questions, priority, and next action."
    >
      <div className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <FormField label="Patient">
            <input
              ...
              onChange={(event) => onDraft({ ...draft, patientName: event.target.value })}
            />
          </FormField>
          <FormField label="UHID">
            <input
              className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
              value={draft.uhid}
              onChange={(event) => onDraft({ ...draft, uhid: event.target.value })}
            />
          </FormField>
          <FormField label="ICU bed">
            <input
              className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm"
              value={draft.bed}
              onChange={(event) => onDraft({ ...draft, bed: event.target.value })}
            />
          </FormField>
          <FormField label="Date">
            <input
              ...
              onChange={(event) => onDraft({ ...draft, dateValue: event.target.value })}
            />
          </FormField>
          ...
          <FormField label="Recorded by">
            <input
              ...
              onChange={(event) => onDraft({ ...draft, recordedBy: event.target.value })}
            />
          </FormField>
        </div>
        <FormField label="Attendees">
          <input
            ...
            onChange={(event) => onDraft({ ...draft, attendees: event.target.value })}
          />
        </FormField>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <FormField label="Questions">
            <input
              ...
              onChange={(event) => onDraft({ ...draft, questions: Number(event.target.value) })}
            />
          </FormField>
          <FormField label="Consent">
            <FilterSelect
              ariaLabel="Consent status"
              value={draft.consentStatus}
              onChange={(value) => onDraft({ ...draft, consentStatus: value as ConsentStatus })}
              options={["Pending", "Obtained", "Not Required"]}
            />
          </FormField>
          <FormField label="Follow-up">
            <FilterSelect
              ...
              onChange={(value) => onDraft({ ...draft, followUpStatus: value as FollowUpStatus })}
              options={["Pending", "In Progress", "Completed", "Overdue", "Not Required"]}
            />
          </FormField>
          <FormField label="Priority">
            <FilterSelect
              ...
              onChange={(value) => onDraft({ ...draft, priority: value as CommunicationPriority })}
              options={["Normal", "High", "Sensitive", "Critical"]}
            />
          </FormField>
        </div>
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={onSave}>{editing ? "Save Changes" : "Add Update"}</Button>
        </div>
      </div>
    </CenterModal>
  );
...
function CommunicationDetails({ row }: { row: CommunicationLog }) {
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <Detail label="Patient" value={`${row.patientName} (${row.uhid})`} />
        <Detail label="Location" value={row.bed} />
        <Detail label="Date & time" value={`${row.dateLabel}, ${row.timeLabel}`} />
        <Detail label="Recorded by" value={row.recordedBy} />
        <Detail label="Communication type" value={row.type} />
        <Detail label="Attendees" value={row.attendees} />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Badge tone={consentTone(row.consentStatus)}>Consent: {row.consentStatus}</Badge>
        <Badge tone={followUpTone(row.followUpStatus)}>Follow-up: {row.followUpStatus}</Badge>
        <Badge tone={priorityTone(row.priority)}>{row.priority}</Badge>
        <Badge tone={reviewTone(row.reviewStatus)}>{row.reviewStatus}</Badge>
      </div>
    </div>
...
function FilterSelect({
  ...
  return (
    <select
      ...
      className="h-9 w-full min-w-0 rounded-md border border-input bg-background px-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring/20"
      value={value}
...
function Cell({ children, strong = false }: { children: React.ReactNode; strong?: boolean }) {
  return (
    <td className={cn("px-3 py-2 align-middle text-foreground", strong && "font-semibold")}>
      {children}
    </td>
  );
}
...
function priorityTone(value: CommunicationPriority): StatusTone {
...
function consentTone(value: ConsentStatus): StatusTone {
  return value === "Obtained" ? "success" : value === "Pending" ? "warning" : "muted";
}
function followUpTone(value: FollowUpStatus): StatusTone {
  return value === "Completed" || value === "Not Required"
    ? "success"
    : value === "Overdue" || value === "Escalated"
      ? "danger"
      : value === "In Progress"
        ? "info"
        : "warning";
}
function reviewTone(value: ReviewStatus): StatusTone {
