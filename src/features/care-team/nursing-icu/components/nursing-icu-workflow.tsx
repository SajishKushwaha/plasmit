...
function WardNursePatientSelect({
  ...
  disabled?: boolean;
  label?: string;
  onChange: (_patientId: string) => void;
  value: string;
}) {
...
function _admissionCandidateLabel(patientId: string) {
...
const initialDoctorMedicationOrders: DoctorMedicationOrder[] = [
  {
    ...
    instructions: "Administer after identity check. Escalate if dose is delayed beyond 30 minutes.",
    priority: "High",
    highRisk: false,
    doubleVerificationRequired: false,
  ...
  },
  {
    ...
    doctor: "Dr. Sameer Mehta",
    indication: "Fever or pain",
    instructions: "Administer if temp > 38 C or pain score > 5. Capture reason.",
    priority: "Routine",
    highRisk: false,
    doubleVerificationRequired: false,
    pharmacyStatus: "Available",
...
function readReadyMedicationIds() {
...
function persistReadyMedication(dose: MedicationDoseRow) {
  const ids = readReadyMedicationIds();
  ids.add(dose.id);
  ids.add(dose.orderId);
  saveReadyMedicationIds(ids);
}

function applyReadyMedicationState(doses: MedicationDoseRow[]) {
  const readyIds = readReadyMedicationIds();
  if (!readyIds.size) return doses;
  return doses.map((dose) => {
    if (!readyIds.has(dose.id) && !readyIds.has(dose.orderId)) return dose;
    return {
      ...dose,
...
function _calculateClinicalHandoffCompletion(draft: NursingClinicalHandoffDraft) {
...
function _patientPendingMedicationCount(patientId?: string) {
...
function TaskDashboardTable({
  ...
  tasks: IcuTask[];
  onOpenAction: (_task: IcuTask, _nextStatus: IcuTask["status"]) => void;
}) {
...
function _taskMatrixPillClass(tone: StatusTone) {
...
function _taskMatrixDotClass(tone: StatusTone) {
...
function TaskActionDialog({
  ...
  onConfirm: (_task: IcuTask, _nextStatus: IcuTask["status"], _note: string) => void;
  onOpenChange: (_open: boolean) => void;
}) {
  return (
...
function NursingAssessmentWorkspace({
  ...
  onCreateFollowUpTask: (_task: IcuTask) => void;
...
function CreateNurseTaskPanel({
  onCreateTask,
}: {
  onCreateTask: (_draft: NurseTaskDraft) => void;
}) {
...
export function MedicationTimelineWorkspace() {
  ...
  const _hasBlockingDoctorScenario = doctorOrderScenarios.some((scenario) => scenario.blocking);
  ...
  const _requestDoctorOrderAction = (orderId: string, action: DoctorOrderStatusAction) => {
    setPendingOrderAction({ orderId, action });
  };

  const _addDoctorOrder = (targetStatus: "Draft" | "Active") => {
  ...
  const _signDraftOrder = (orderId: string) => {
    const targetOrder = orders.find((order) => order.id === orderId);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="overflow-hidden border-slate-200 bg-white shadow-sm">
        <CardHeader className="border-b border-slate-200 bg-white">
          <div>
            <CardTitle>Medicine Receive & Verify</CardTitle>
          </div>
          <div className="flex flex-wrap gap-2">
      ...
      <Card className="overflow-hidden border-slate-200 bg-white shadow-sm">
        <CardHeader className="border-b border-slate-200 bg-white">
          ...
          <Badge tone="info">{queueRows.length} medicine(s)</Badge>
        </CardHeader>
        <CardContent className="p-0">
                  ...
                  return (
                    <tr className="hover:bg-slate-50/70" key={dose.id}>
                      ...
                      <td className="px-4 py-4">
                        ...
                        <p className="text-xs text-muted-foreground">{dose.indication}</p>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 text-right">
                        {ready ? (
                          ...
                          <Button size="sm" onClick={() => receiveAndVerify(dose.id)}>
                            <Check className="h-4 w-4" />
                            Receive & verify
                          </Button>
                        )}
...
function MedicationOrderComposer({
  ...
  onDraftChange: (_draft: Partial<MedicationOrderDraft>) => void;
  onSaveDraft: () => void;
  onSignOrder: () => void;
  onCopyOrder: (_orderId: string) => void;
  onHoldOrder: (_orderId: string) => void;
  onResumeOrder: (_orderId: string) => void;
  onSignDraft: (_orderId: string) => void;
  onStopOrder: (_orderId: string) => void;
...
function MedicationPatientOrdersDialog({
  ...
  orders: DoctorMedicationOrder[];
  orderSearch: string;
  onOrderSearch: (_value: string) => void;
  orderStatusFilter: "All" | MedicationOrderStatus;
  onOrderStatusFilter: (_value: "All" | MedicationOrderStatus) => void;
  patient?: IcuPatient;
  totalOrders: number;
  onCopyOrder: (_orderId: string) => void;
  onHoldOrder: (_orderId: string) => void;
...
function DoctorOrderAmendDialog({
  ...
  order?: DoctorMedicationOrder;
  onOpenChange: (_open: boolean) => void;
  onConfirm: (_payload: DoctorOrderAmendPayload) => void;
}) {
...
function MedicationChartTable({
  ...
  onSelectDose: (_doseId: string) => void;
  onRequestAction: (_doseId: string, _action: MedicationNurseAction) => void;
  onMarkPharmacyAvailable: (_orderId: string) => void;
}) {
...
function MedicationActionCell({
  ...
  open,
  onRequestAction,
  onMarkPharmacyAvailable: _onMarkPharmacyAvailable,
  onOpenChange,
  onSelectDose,
}: {
  ...
  if (!actions.length) {
    return <span className="text-xs font-semibold text-muted-foreground">Completed</span>;
  }
...
function medicationTableActions(dose: MedicationDoseRow): MedicationTableAction[] {
  ...
  if (!canAct) return [];
  return [
    { action: "Administered", label: "Administer" },
    { action: "Held", label: "Hold", variant: "outline" },
    { action: "Skipped", label: "Skip", variant: "outline" },
  ];
}
...
function MedicationDoseCard({
  ...
  compact?: boolean;
  onSelect: () => void;
  onRequestAction: (_action: MedicationNurseAction) => void;
...
function medicationActionLabel(action: MedicationNurseAction) {
  if (action === "Administered") return "Administer medicine";
...
function AdmissionPatientSearchRows({
  ...
  onCreate: () => void;
  onSelect: (_patientId: string) => void;
}) {
  if (!rows.length) {
    return (
...
function EmergencyAdmissionCreatePanel({
  ...
  onCancel: () => void;
  onChange: (_key: keyof EmergencyAdmissionQuickDraft, _value: string) => void;
  onCreate: () => void;
}) {
  return (
...
function SelectField({
  ...
  onChange: (_value: string) => void;
  options: string[];
  renderOption?: (_value: string) => string;
  wide?: boolean;
}) {
  return (
...
function _isPriorityMedicationDose(dose: MedicationDoseRow) {
  return (
    isMedicationDueStatus(dose.status) ||
    dose.orderType === "STAT" ||
    dose.status === "Running" ||
