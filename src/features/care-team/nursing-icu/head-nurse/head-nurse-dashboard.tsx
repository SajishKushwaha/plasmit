...
import { Activity, AlertTriangle, ArrowLeftRight, BedDouble, CheckCircle2, ChevronLeft, ChevronRight, Eye, Search, ShieldAlert, Stethoscope, Users, X } from "lucide-react";
...
export function HeadNurseDashboard() {
  ...
  const baseRoute = pathname.startsWith("/nursing-icu/head-nurse") ? "/nursing-icu/head-nurse" : "/head-nurse";
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-1">
            <ViewButton active={view === "patients"} label="Patient Dashboard" onClick={() => { setPage(1); router.push(`${baseRoute}?view=patients`); }} />
            <ViewButton active={view === "icu"} label="ICU Dashboard" onClick={() => { setPage(1); router.push(`${baseRoute}?view=icu`); }} />
        ...
        {view === "icu" ? <IcuDashboardView rows={filteredIcuRows} /> : <PatientDashboardView onOpenAlert={setActiveAlert} onOpenAssignment={setActiveAssignment} onOpenWorkflow={setActiveWorkflow} page={page} patientRows={filteredPatientRows} setPage={setPage} />}
      </section>

      {activeAssignment ? <AssignmentDialog onClose={() => setActiveAssignment(null)} row={activeAssignment.row} /> : null}
      {activeAlert ? <AlertDialog onClose={() => setActiveAlert(null)} row={activeAlert.row} /> : null}
...
function PatientDashboardView({ onOpenAlert, onOpenAssignment, onOpenWorkflow, page, patientRows, setPage }: { onOpenAlert: (state: AlertDialogState) => void; onOpenAssignment: (state: AssignmentDialogState) => void; onOpenWorkflow: (state: WorkflowDialogState) => void; page: number; patientRows: HeadNursePatientRow[]; setPage: React.Dispatch<React.SetStateAction<number>> }) {
  const router = useRouter();
    ...
    <div>
        ...
        <table className="w-full min-w-[1080px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-black uppercase tracking-wide text-slate-950">
              <th className="sticky left-0 z-30 w-[230px] bg-slate-50 px-5 py-4 text-left shadow-[1px_0_0_0_#e2e8f0]">Patient / ICU</th>
          ...
          <tbody>
            {visibleRows.map((row) => {
              ...
              const assignedUnitNurse = selectedUnitNurseForPatient(row.patient) || row.assignmentStatus.replace(/^ICU Nurse\s+/i, "").trim();
              ...
              return (
                <tr className="border-b border-slate-200 last:border-b-0" key={row.patient.id}>
                  <td className="sticky left-0 z-20 border-r border-slate-200 bg-white p-0 align-middle shadow-[1px_0_0_0_#e2e8f0]">
                    <div className={cn("relative min-h-[116px] px-7 py-6 before:absolute before:inset-y-0 before:left-0 before:w-1", rowAccentClass(row.tone))}>
                  ...
                  <td className="px-4 py-6 text-center align-middle">
                    {assignmentUnlocked ? (
                      <button suppressHydrationWarning className={pillClass(hasAssignment ? "success" : "warning", true)} type="button">
                  ...
                  <td className="px-4 py-6 text-center align-middle">
                    {row.alertStatus === "Clear" ? (
                      <StatusPill label={row.alertStatus} tone={statusTone(row.alertStatus, row.tone)} />
                    ) : (
                      <button className={pillClass(statusTone(row.alertStatus, row.tone), true)} onClick={() => onOpenAlert({ row })} type="button">
                        {row.alertStatus}
                      </button>
                    )}
                  </td>
                  <td className="px-4 py-6 text-center align-middle">
                    <div className="flex items-center justify-center gap-2">
                        ...
                        activeQuickAssign?.rowId === row.patient.id ? (
                          <div className="inline-flex items-center gap-2 bg-white p-2 shadow-sm">
                            <select
                              ...
                              onChange={(event) => setActiveQuickAssign({ rowId: row.patient.id, nurse: event.target.value })}
                              value={activeQuickAssign.nurse}
                            >
                              {getAvailableIcuNursesForPatient(row.patient).map((nurse) => (
                                <option key={`${nurse.unit}-${nurse.nurse}`} value={nurse.nurse}>{nurse.nurse}</option>
                              ))}
                            </select>
                            <button
                      ...
                      {activeQuickAssign?.rowId !== row.patient.id ? (
                        <IconActionButton ariaLabel="View workflow status" tone="info" onClick={() => router.push("/icu-command-center/patients/icu-001?tab=overview")}>
      ...
      <div className="flex flex-col gap-3 border-t border-slate-200 bg-slate-50/70 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex h-9 min-w-24 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-sm font-black text-slate-700 shadow-sm">Page {page} / {totalPages}</span>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[1180px] border-collapse text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-black uppercase tracking-wide text-slate-950">
            <th className="sticky left-0 z-30 w-[230px] bg-slate-50 px-5 py-4 text-left shadow-[1px_0_0_0_#e2e8f0]">ICU Unit</th>
          ...
          {rows.map((row) => (
            <tr className="border-b border-slate-200 last:border-b-0" key={row.id}>
              ...
              <td className="px-4 py-4 text-center align-middle">
                <DashboardMatrixCell icon={BedDouble} title={`${row.availableBeds}/${row.totalBeds}`} detail={`${row.occupiedBeds} occupied`} tone={availabilityTone(row.availableBeds) ?? row.tone} />
              </td>
              <td className="px-4 py-4 text-center align-middle">
                <DashboardMatrixCell icon={ArrowLeftRight} title={`${row.availableVentilatorBeds}/${row.ventilatorBeds}`} tone={availabilityTone(row.availableVentilatorBeds) ?? row.tone} />
              </td>
              <td className="px-4 py-4 text-center align-middle">
                <DashboardMatrixCell icon={Users} title={`${row.availableIcuNurses}/${row.totalIcuNurses}`} tone={availabilityTone(row.availableIcuNurses) ?? row.tone} />
              </td>
              {/* <td className="px-4 py-4 text-center align-middle">
              ...
              <tbody>
                {availableNurses.map((staff) => (
                  <tr className="border-t border-slate-100" key={`${staff.unit}-${staff.nurse}`}>
                    ...
                    <td className="px-4 py-3 text-center">{staff.assignedPatients}/{staff.maxCapacity}</td>
                ...
                {!availableNurses.length ? (
                  <tr>
                    <td className="px-4 py-8 text-center text-sm font-bold text-slate-500" colSpan={4}>No available ICU nurse found for {row.patient.unit}.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
...
function AlertDialog({ onClose, row }: { onClose: () => void; row: HeadNursePatientRow }) {
  const openAlerts = icuAlerts.filter((alert) => alert.patientId === row.patient.id && alert.status !== "Resolved");
  const [actionMessage, setActionMessage] = React.useState<string | null>(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 py-6" role="dialog" aria-modal="true">
      <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
              ...
              <tbody>
                {openAlerts.length ? openAlerts.map((alert) => (
                  <tr className="border-t border-slate-100" key={alert.id}>
                    ...
                    <td className="px-4 py-3 text-center"><StatusPill label={alert.severity} tone={alertSeverityTone(alert.severity)} /></td>
                    <td className="px-4 py-3 text-slate-600">{alert.message}</td>
                    <td className="px-4 py-3 text-center font-semibold text-slate-600">{alert.source}</td>
                    <td className="px-4 py-3 text-center font-semibold text-slate-600">{alert.owner}</td>
                  </tr>
                )) : (
                  <tr>
                    <td className="px-4 py-8 text-center text-sm font-bold text-slate-500" colSpan={5}>Clear status. No popup action required.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <DialogFooter>
          ...
          <DialogButton variant="outline" onClick={() => handleAction("Acknowledge")}>Acknowledge</DialogButton>
          <button
...
function DialogHeader({ onClose, patientRow, title }: { onClose: () => void; patientRow: HeadNursePatientRow; title: string }) {
  return (
    <div className="border-b border-slate-200 px-5 py-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          {/* <p className="text-xs font-black uppercase tracking-wide text-slate-500">{title}</p> */}
          <h3 className="mt-1 text-xl font-black text-slate-950">{patientRow.patient.patientName}</h3>
          <p className="mt-1 text-sm font-semibold text-slate-500">{patientRow.patient.bedNo} | {patientRow.patient.unit} | {patientRow.patient.mrn}</p>
        </div>
        <button aria-label="Close dialog" className="inline-flex h-12 w-12 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 shadow-sm transition hover:bg-slate-100" onClick={onClose} type="button">
          <X className="h-5 w-5" />
        </button>
      </div>
    </div>
...
function alertSummaryText(alerts: typeof icuAlerts) {
...
function DashboardUnitCell({ row }: { row: HeadNurseIcuDashboardRow }) {
  return (
    <div className={cn("relative min-h-[87px] px-5 py-4 before:absolute before:inset-y-0 before:left-0 before:w-1", rowAccentClass(row.tone))}>
...
function DashboardMatrixCell({ icon: Icon, title, detail, tone }: { icon: typeof Activity; title: string; detail?: string; tone: HeadNurseTone }) {
  return (
    <div className="flex min-h-[84px] flex-col items-center justify-center rounded-md px-2  transition hover:bg-slate-50/60">
      <span className={cn("inline-flex h-9 min-w-24 items-center justify-center gap-1 rounded-full px-3 text-xs font-black text-white shadow-[0_3px_8px_rgba(0,0,0,0.28)]", statusPillClass(tone))}>
