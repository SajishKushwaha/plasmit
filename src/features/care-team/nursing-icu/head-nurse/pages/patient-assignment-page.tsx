...
export function PatientAssignmentPage({ initialPatientId }: HeadNursePageProps) {
  ...
  const icuNurses = headNurseStaffRows
    .filter((row) => row.role === "ICU Nurse")
    .map((row) => row.nurse);
  ...
  return (
    <HeadNursePatientContext initialPatientId={initialPatientId} moduleId="patient-assignment">
      {({ patient }) => {
        ...
        const selectedIcuNurseRow = headNurseStaffRows.find(
          (item) => item.role === "ICU Nurse" && item.nurse === selectedIcuNurse,
        );
        ...
        return (
          <Card className="border-slate-200 bg-white shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Head Nurse assignment keeps unit readiness and nurse workload visible before
                confirming.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-3 md:grid-cols-4">
                ...
                <InfoTile
                  ...
                  value={
                    <HeadNurseTonePill tone={row?.tone ?? "info"}>
              ...
              <div className="grid gap-3 md:grid-cols-[minmax(220px,1fr)_minmax(220px,1fr)]">
                <NativeSelect
                  label="ICU Nurse"
                  value={selectedIcuNurse || "Select ICU Nurse"}
                  onChange={() => undefined}
                  options={["Select ICU Nurse", ...icuNurses]}
                />
                <NativeSelect
                  label="Assignment priority"
                  value={patient.criticalityScore >= 8 ? "Critical first" : "Routine assignment"}
                  onChange={() => undefined}
                  options={["Critical first", "Routine assignment"]}
                />
              </div>
