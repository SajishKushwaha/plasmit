...
const nonNegativeAssessmentRows = new Set([
  "Urine assessment:Urine Volume (in ml)",
  "Urine assessment:Diaper weight (in ml)",
  "Stool assessment:Stool (ml)",
  "Emesis Assessment:Emesis (in ml)",
  "NG Aspiration assessment:Volume (ml)",
]);
...
function DateTimeFieldWithSave({
  value,
  onChange,
}: {
  value: string;
...
function AssessmentField({
  ...
  const showDateTimeSave =
    row.group === "Oxygen therapy Assessment" &&
    (row.label === "Start" || row.label === "End") &&
    row.fieldType === "Date and time";

  if (row.fieldType === "Dropdown") {
    return (
      <select
        className="h-9 w-full min-w-[150px] rounded-md border border-input bg-background px-2 text-sm outline-none focus:ring-2 focus:ring-ring/20"
        value={value}
  ...
  if (row.fieldType === "Calculated") {
    return (
      <div className="rounded-md border border-info/30 bg-info/10 px-2 py-2 text-sm text-info">
        {value || row.formula || "Auto calculated"}
      </div>
    );
  }
  if (showDateTimeSave) {
...
function AssessmentTable({
  ...
  onAddColumn: () => void;
  onDeleteColumn: (_time: string) => void;
  onDeleteNow: () => void;
  onRenameColumn: (_oldTime: string, _newTime: string) => void;
  onValueChange: (_rowId: string, _time: string, _value: string) => void;
  onComment: (_row: AssessmentRow) => void;
}) {
  const [deleteTime, setDeleteTime] = React.useState<string | null>(null);
  const [draftTime, setDraftTime] = React.useState("");
  const deletingHasData = deleteTime
    ? group.rows.some((row) => Boolean(values[`${row.id}-${deleteTime}`]?.trim()))
    : false;
  ...
  const duplicateDraftTime = Boolean(
    deleteTime &&
    draftTimeTrimmed &&
    draftTimeTrimmed !== deleteTime &&
    times.includes(draftTimeTrimmed),
  );

  function getMorseFallScore(time: string) {
    return ["ri4", "ri5", "ri6", "ri7", "ri8", "ri9"].reduce(
      (total, rowId) => total + scoreFromSelectedOption(values[`${rowId}-${time}`] ?? ""),
      0,
    );
  }
    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>
            Time-based documentation grid configured from assessment master rows.
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent>
              ...
              <tr>
                      ...
                      <div className="absolute left-2 top-9 z-40 w-80 rounded-lg border border-border bg-surface p-3 normal-case tracking-normal text-foreground shadow-soft">
                        ...
                        <label className="mt-3 block text-xs font-medium text-muted-foreground">
                          ...
                          <Input
                            ...
                            placeholder="Enter time or label"
                          />
                        </label>
                        {duplicateDraftTime ? (
                          <div className="mt-1 text-xs text-danger">
                            This time column already exists.
                          </div>
                        ) : null}
                        <div className="mt-1 text-xs text-muted-foreground">
                          {!canDelete
                            ? "At least one TIME column is required."
                            : deletingHasData
                              ? "This column has documented values. Deleting will remove those values."
                              : "This column has no documented values."}
                        </div>
                ...
                {showNow ? (
                  <th className="relative border-b border-border px-3 py-2 text-left">
                    <button
                      className="rounded px-1 py-0.5 text-left hover:bg-background"
                      onClick={() => setDeleteTime(deleteTime === "NOW" ? null : "NOW")}
                    >
                      NOW
                    </button>
                    {deleteTime === "NOW" ? (
                      <div className="absolute left-2 top-9 z-40 w-72 rounded-lg border border-border bg-surface p-3 normal-case tracking-normal text-foreground shadow-soft">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {nowHasData
                            ? "NOW column has documented values. Deleting will remove those values."
                            : "NOW column has no documented values."}
                        </div>
                        <div className="mt-3 flex justify-end gap-2">
                          <Button size="sm" variant="outline" onClick={() => setDeleteTime(null)}>
                            Cancel
                          </Button>
                          <Button
                            size="sm"
                ...
                >
                  ...
                  {times.map((time) => (
                    <td className="px-3 py-2 align-top" key={time}>
                      <AssessmentField
                        row={row}
                        value={getAssessmentValue(row, time)}
                        onChange={(value) => onValueChange(row.id, time, value)}
                      />
                    </td>
                  ))}
                  {showNow ? (
                    <td className="px-3 py-2 align-top">
                      <AssessmentField
                        row={row}
                        value={getAssessmentValue(row, "NOW")}
                        onChange={(value) => onValueChange(row.id, "NOW", value)}
                      />
                    </td>
                  ) : null}
                  ...
                  <td className="px-3 py-2 align-top">
                    <div className="min-w-[160px] space-y-2">
                      <Button
                        ...
                        title={comments[row.id] || "Add comment"}
                        onClick={() => onComment(row)}
                      >
                        <MessageSquareText className="h-4 w-4" />
                        {comments[row.id] ? "Edit" : "Add"}
                      </Button>
                      {comments[row.id] ? (
                        <div className="max-w-[220px] rounded-md border border-border bg-surface-muted p-2 text-xs text-muted-foreground">
                          {comments[row.id]}
                        </div>
                      ) : null}
                    </div>
...
export function NursingAssessmentsPage() {
  ...
  const filteredAssessmentGroups = sortedGroups.filter(
    (group) =>
      group.displayName.toLowerCase().includes(assessmentSearch.toLowerCase()) ||
      group.name.toLowerCase().includes(assessmentSearch.toLowerCase()),
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="relative">
              ...
              {assessmentSearchOpen ? (
                <div className="absolute z-40 mt-1 max-h-72 w-full overflow-auto rounded-md border border-border bg-surface shadow-soft">
                  ...
                  {!filteredAssessmentGroups.length ? (
            ...
            <Button
              className="w-full"
              variant={preferenceIds.includes(selected.id) ? "default" : "outline"}
              onClick={() => togglePreference(selected.id)}
            >
              <Star className="h-4 w-4" />
              {preferenceIds.includes(selected.id) ? "Preferred" : "Add to preference"}
            </Button>
            <div className="space-y-2">
              ...
              {preferences.map((group) => (
                <div className="flex items-center gap-2" key={group.id}>
                  <button
                    className="min-w-0 flex-1 rounded-md border border-border p-2 text-left text-sm hover:bg-surface-muted"
                    onClick={() => {
                      setSelectedGroupId(group.id);
                      setAssessmentSearch(group.displayName);
                    }}
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() => togglePreference(group.id)}
        ...
        <AssessmentTable
          ...
          onValueChange={(rowId, time, value) =>
            setValues((current) => ({ ...current, [`${rowId}-${time}`]: value }))
          }
      ...
      <Drawer
        ...
        description={commentTarget?.row.label}
      >
          ...
          <textarea
            ...
            onChange={(event) =>
              setCommentTarget((target) =>
                target ? { ...target, comment: event.target.value } : target,
              )
            }
          />
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              onClick={() => {
                ...
                setComments((current) => ({
                  ...current,
                  [commentTarget.row.id]: commentTarget.comment,
                }));
...
function ExampleList() {
  const rows = sortAssessmentGroups(assessmentGroups).flatMap((group) =>
    group.rows.map((row, index) => ({ group: index === 0 ? group.name : "", row })),
  );
  return (
    <Card>
      <CardHeader>
            ...
            <tbody>
              {rows.map(({ group, row }) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 align-top">
                    {row.selectable === "None" ? "" : row.selectable}
                  </td>
...
export function NursingAssessmentConfigurationPage() {
  ...
  const filtered = rows.filter((row) =>
    `${row.id} ${row.row} ${row.display}`.toLowerCase().includes(search.toLowerCase()),
  );

  function openNew(type: MasterRow["type"]) {
    setDraft({
      id: `${type === "Grouper" ? "grp" : "row"}-${Date.now().toString().slice(-4)}`,
      row: "",
      display: "",
      type,
      active: true,
  ...
  function saveDraft() {
    if (!draft) return;
    setRows((current) =>
      current.some((row) => row.id === draft.id)
  ...
  return (
    <NursingShell
      ...
      description="Configure grouper rows, content rows, field types, dropdown options, comments, and intake/output flags."
    >
          ...
          <Card>
            <CardHeader>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button size="sm" onClick={() => openNew("Content")}>
                  <Plus className="h-4 w-4" />
                  Add row
                </Button>
                <div className="relative min-w-0 flex-1 max-w-xs">
                  ...
                  <Input
                    className="pl-8"
                    placeholder="Search row ID or name"
                    value={search}
                    onChange={(event) => setSearch(event.target.value)}
                  />
                </div>
                <Button variant="outline" onClick={() => openNew("Grouper")}>
                  New grouper
                </Button>
              </div>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto rounded-lg border border-border">
                <table className="w-full min-w-[900px] border-collapse text-sm">
                  <thead className="bg-surface-muted text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                    <tr>
                      {["Row ID", "Row name", "Display name", "Type", "Active", "Edit"].map(
                        (head) => (
                          <th className="border-b border-border px-3 py-2 text-left" key={head}>
                            {head}
                          </th>
            ...
            <div className="flex items-start justify-between gap-4 border-b border-border bg-surface px-4 py-3">
              <div>
                <Dialog.Title className="text-sm font-semibold text-foreground">
                  {draft?.type === "Grouper" ? "Edit grouper row" : "Edit content row"}
                </Dialog.Title>
                <Dialog.Description className="mt-1 text-xs text-muted-foreground">
                  Configure the assessment master row and save your changes.
                </Dialog.Description>
              </div>
            ...
            <div className="max-h-[calc(88dvh-62px)] overflow-auto p-4">
              {draft ? (
                <div className="space-y-3">
                  ...
                  <Input
                    value={draft.row}
                    onChange={(event) => setDraft({ ...draft, row: event.target.value })}
                  ...
                  <FieldLabel
                    ...
                    value={
                      <select
                        ...
                        value={draft.type}
                        onChange={(event) =>
                          setDraft({ ...draft, type: event.target.value as MasterRow["type"] })
                        }
                      >
                    ...
                    <>
                        ...
                        value={
                          <select
                            ...
                            onChange={(event) =>
                              setDraft({
                                ...draft,
                                fieldType: event.target.value as NursingFieldType,
                              })
                            }
                          >
                      ...
                      <FieldLabel
                        ...
                        value={
                          <select
                            className="h-9 w-full rounded-md border border-input bg-background px-2"
                            value={draft.selectable}
                            onChange={(event) =>
                              setDraft({
                                ...draft,
                                selectable: event.target.value as NursingSelectable,
                              })
                            }
                          >
                  ...
                  {rows.some((row) => row.id === draft.id) ? (
                    <div className="rounded-md border border-danger/30 bg-danger/10 p-3 text-sm text-danger">
                      ...
                      <p className="mt-1 text-xs text-danger-foreground">
                        This will permanently remove the selected assessment row or grouper.
                      </p>
                      <Button
                        size="sm"
                        variant="danger"
