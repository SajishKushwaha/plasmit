...
const systemRows = [
  {
    system: "Airway / Breathing",
    status: "Stable",
    finding: "SpO2 stable on current support",
    action: "Continue monitoring",
    tone: "success" as AssessmentTone,
  },
  {
    system: "Circulation",
    status: "Watch",
    finding: "HR trend elevated during last round",
    action: "Repeat vitals in 30 min",
    tone: "warning" as AssessmentTone,
  },
  {
    system: "Neurology",
    status: "Stable",
    finding: "Alert, oriented, no new deficit",
    action: "Routine neuro checks",
    tone: "success" as AssessmentTone,
  },
  {
    system: "Infection",
    status: "Isolation",
    finding: "Droplet isolation active",
    action: "PPE compliance and visitor control",
    tone: "info" as AssessmentTone,
  },
  {
    system: "Lines / Devices",
    status: "Review",
    finding: "IV line site clean, dressing due today",
    action: "Nurse dressing check",
    tone: "warning" as AssessmentTone,
  },
];
...
export function AssessmentPage({
  isolationType: _isolationType = "Droplet",
  patient,
}: AssessmentPageProps) {
  const [assessmentType, setAssessmentType] = React.useState(assessmentTypes[1]);
  const [checks, _setChecks] = React.useState<Record<string, boolean>>(() =>
    Object.fromEntries(quickChecks.map((check, index) => [check, index < 4])),
  );
  const [doctorAssessment, setDoctorAssessment] = React.useState(
    `${patient.diagnosis}. Patient requires continued IPD monitoring with repeat vitals and escalation if any deterioration is noted.`,
  );
  const [plan, setPlan] = React.useState(
    "Continue current treatment plan. Repeat assessment next round. Maintain isolation precautions and document response to interventions.",
  );

  const _completedChecks = Object.values(checks).filter(Boolean).length;
          ...
          <Card>
            <CardContent className="grid gap-3 p-4 md:grid-cols-[220px_minmax(0,1fr)]">
              ...
              <label className="space-y-1.5">
                ...
                <Input
                  readOnly
                  className="h-10 bg-surface-muted font-semibold"
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <CardDescription>
                  Review findings, immediate action, and escalation state.
                </CardDescription>
              </div>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <div>
                ...
                <CardDescription>
                  Document impression, action plan, and next review target.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="grid gap-3 lg:grid-cols-2">
              <label className="space-y-1.5">
                ...
                <textarea
                  className="min-h-36 w-full rounded-lg border border-input bg-white p-3 text-sm font-semibold outline-none focus:ring-2 focus:ring-ring/20"
