...
import { StickyActionBar } from "@/features/operations/admin/admin-shared";
import {
  backendApis,
  builderFields,
  clinicalScores,
...
function clinicalPatientFromRecord(patient: (typeof mockPatients)[number]): ClinicalPatientContext {
  ...
  const encounter = visits.length
    ? visits.map((visit) => visit.referenceNumber).join(" / ")
    : "No active encounter";
  ...
  const nonAllergyFlags = patient.alertFlags.filter(
    (flag) => !flag.toLowerCase().includes("allergy"),
  );
  ...
  return {
    ...
    encounter,
    wardBed:
      activeVisit?.visitType === "IPD"
        ? `${patient.department} Ward / Bed pending`
        : `${patient.department} ${activeVisit?.visitType ?? "OPD"}`,
    consultant: activeVisit?.provider ?? consultantForDepartment(patient.department),
    allergies: allergyFlags.length
      ? allergyFlags.map((flag) => flag.replace(/^Allergy:\s*/i, "")).join(", ")
      : "No known allergy",
...
function PatientHeaderStrip({
  ...
  const [patientSearch, _setPatientSearch] = React.useState("");
  const _patientResults = React.useMemo(() => {
    const query = patientSearch.trim().toLowerCase();
    if (!query) return [];
    return mockPatients
      .filter((item) =>
        `${patientName(item)} ${item.uhid} ${item.mobile} ${item.department}`
          .toLowerCase()
          .includes(query),
      )
      .slice(0, 8);
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      {cards.map(([label, value, severity, note]) => (
        <Card
          className="transition hover:-translate-y-0.5 hover:shadow-[0_12px_26px_rgba(39,37,54,0.08)]"
          key={label}
        >
      ...
      <CardContent className="grid gap-2 p-3 sm:grid-cols-2 xl:grid-cols-4">
        {quickClinicalActions.map((action) => {
          ...
          return (
            <Button
              key={action}
              variant={active === action ? "default" : "outline"}
...
function SpecialtyPicker({
  ...
  onSelect: (_id: SpecialtyId) => void;
}) {
  const [search, setSearch] = React.useState("");
  const [department, setDepartment] = React.useState("All departments");
  const departments = [
    "All departments",
    ...Array.from(new Set(clinicalSpecialties.map((item) => item.department))),
  ];
  const specialties = clinicalSpecialties.filter((item) => {
    const matchesSearch = `${item.label} ${item.department}`
      .toLowerCase()
      ...
      <div className="flex flex-col gap-2 rounded-xl border border-border bg-white p-3 shadow-soft md:flex-row md:items-center">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <Input
            ...
            placeholder="Search specialty or department..."
          />
        </div>
        <select
          className="h-10 rounded-lg border border-input bg-white px-3 text-sm font-medium outline-none focus:ring-2 focus:ring-ring/15"
          value={department}
        ...
        <Button variant="outline" onClick={() => toast.success("Custom template builder opened")}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {specialties.map((item) => {
          ...
          return (
            <button
              className={cn(
                "rounded-xl border bg-white p-4 text-left shadow-soft transition hover:-translate-y-0.5 hover:shadow-[0_12px_26px_rgba(39,37,54,0.08)]",
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  {item.favorite ? <Star className="h-4 w-4 fill-warning text-warning" /> : null}
                  {item.recentlyUsed ? <Badge tone="info">Recent</Badge> : null}
                </div>
              </div>
...
function ToggleMatrix({
  ...
  findings: (typeof examinationFindings)[SpecialtyId];
  ...
  const [genericDetails, setGenericDetails] = React.useState<
    Record<string, { severity: ClinicalSeverity; note: string }>
  >(() =>
    Object.fromEntries(
      findings
        .slice(0, 6)
        .map((finding) => [finding.label, { severity: finding.severity, note: finding.value }]),
    ),
  );
  ...
  function selectOption(label: string, option: string) {
    ...
    if (cvsStructuredLabels.includes(label))
      setCvsEditing((current) => ({ ...current, [label]: false }));
  }
  ...
  function getCvsFields(label: string) {
    if (label === "Heart sounds")
      return [
        {
          label: "S1",
          key: "s1",
          value: heartSoundDetails.s1,
          options: ["Normal", "Loud", "Soft"],
        },
        {
          label: "S2",
          key: "s2",
          value: heartSoundDetails.s2,
          options: ["Normal", "Loud", "Soft"],
        },
        {
          label: "Added",
          key: "addedSounds",
          value: heartSoundDetails.addedSounds,
          options: ["None", "S3", "S4", "Click", "Rub"],
        },
        {
          label: "Rhythm",
          key: "rhythm",
          value: heartSoundDetails.rhythm,
      ...
      return [
        {
          label: "Timing",
          key: "timing",
          value: murmurDetails.timing,
          options: ["Systolic", "Diastolic", "Continuous"],
        },
        {
          label: "Grade",
          key: "grade",
          value: murmurDetails.grade,
          options: ["Grade 1", "Grade 2", "Grade 3", "Grade 4", "Grade 5", "Grade 6"],
        },
        {
          label: "Site",
          key: "site",
          value: murmurDetails.site,
          options: ["Apex", "Aortic", "Pulmonary", "Tricuspid"],
        },
        {
          label: "Radiation",
          key: "radiation",
          value: murmurDetails.radiation,
          options: ["None", "Axilla", "Carotid", "Back"],
        },
      ];
      ...
      Pulse: [
        {
          label: "Rate",
          key: "rate",
          options: ["60 bpm", "72 bpm", "86 bpm", "100 bpm", "120 bpm"],
        },
      ...
      BP: [
        {
          label: "Reading",
          key: "reading",
          options: ["120/80 mmHg", "140/90 mmHg", "148/92 mmHg", "160/100 mmHg"],
        },
    ...
    return (configs[label] ?? []).map((field) => ({
      ...field,
      value: detail[field.key] ?? field.options[0],
    }));
  }
  ...
  function updateCvsField(label: string, key: string, value: string) {
    if (label === "Heart sounds")
      return updateHeartSoundDetail(key as keyof typeof heartSoundDetails, value);
    if (label === "Murmur") return updateMurmurDetail(key as keyof typeof murmurDetails, value);
    return updateCvsDetail(label, key, value);
  }
  function updateGenericDetail(
    label: string,
    patch: Partial<{ severity: ClinicalSeverity; note: string }>,
  ) {
    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>
            Single-click selection with auto score generation and keyboard-friendly focus states.
          </CardDescription>
        </div>
                  ...
                  <div className="flex flex-col gap-3 border-b border-border bg-surface-muted/70 p-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge tone={selected[finding.label] === "Present" ? "success" : "danger"}>
                          {selected[finding.label]}
                        </Badge>
                      </div>
                      {selected[finding.label] === "Present" ? (
                        <div className="mt-1 text-xs font-medium text-muted-foreground">
                          {getCvsHelper(finding.label)}
                        </div>
                      ) : null}
                    </div>
                    ...
                    <div className="p-3">
                          ...
                          >
                            ...
                            {isCvsEditing(finding.label) ? (
                              <select
                                ...
                                value={field.value}
                                onChange={(event) =>
                                  updateCvsField(finding.label, field.key, event.target.value)
                                }
                              >
                      ...
                      <div className="mt-3 flex justify-end">
                        <Button
                          ...
                          type="button"
                          variant="outline"
                          onClick={() => toggleCvsEditing(finding.label)}
                        >
                ...
                <div className="overflow-hidden rounded-lg border border-border bg-white shadow-sm">
                  <div className="flex flex-col gap-3 border-b border-border bg-surface-muted/70 p-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="text-sm font-bold text-foreground">{finding.label}</div>
                        <Badge tone={selected[finding.label] === "Absent" ? "danger" : "success"}>
                          {selected[finding.label] === "Absent" ? "Absent" : "Present"}
                        </Badge>
                      </div>
                      {selected[finding.label] !== "Absent" ? (
                        <div className="mt-1 text-xs font-medium text-muted-foreground">
                          Severity and note documented.
                        </div>
                      ) : null}
                    </div>
                    <div className="grid grid-cols-2 rounded-md border border-border bg-white p-1 sm:w-[240px]">
                      {["Present", "Absent"].map((option) => (
                        <button
                          className={cn(
                            "h-8 rounded px-3 text-xs font-bold transition focus:outline-none focus:ring-2 focus:ring-ring/20",
                            (selected[finding.label] ?? "Present") !== "Absent" &&
                              option === "Present" &&
                              "bg-success text-white shadow-sm",
                            selected[finding.label] === "Absent" &&
                              option === "Absent" &&
                            ...
                            !(
                              ((selected[finding.label] ?? "Present") !== "Absent" &&
                                option === "Present") ||
                              (selected[finding.label] === "Absent" && option === "Absent")
                            ) &&
                              "text-muted-foreground hover:bg-surface-muted hover:text-foreground",
                          )}
                          key={option}
                    ...
                    <div className="p-3">
                      <div className="grid gap-2 md:grid-cols-[180px_minmax(0,1fr)]">
                        <div className="rounded-lg border border-border bg-surface-muted p-2">
                          ...
                          {genericEditing[finding.label] ? (
                            <select
                              className="mt-2 h-9 w-full rounded-md border border-input bg-white px-3 text-sm font-semibold text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15"
                              value={genericDetails[finding.label]?.severity ?? finding.severity}
                              onChange={(event) =>
                                updateGenericDetail(finding.label, {
                                  severity: event.target.value as ClinicalSeverity,
                                })
                              }
                            >
                              {(
                            ...
                            <div className="mt-1 text-sm font-bold text-foreground">
                              {genericDetails[finding.label]?.severity ?? finding.severity}
                            </div>
                          )}
                        </div>
                        <div className="rounded-lg border border-border bg-surface-muted p-2">
                          ...
                          {genericEditing[finding.label] ? (
                            <Input
                              className="mt-2 h-9 bg-white font-semibold"
                              value={genericDetails[finding.label]?.note ?? finding.value}
                              onChange={(event) =>
                                updateGenericDetail(finding.label, { note: event.target.value })
                              }
                            />
                          ) : (
                            <div className="mt-1 truncate text-sm font-bold text-foreground">
                              {genericDetails[finding.label]?.note ?? finding.value}
                            </div>
                          )}
                        </div>
                      </div>
                      <div className="mt-3 flex justify-end">
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            setGenericEditing((current) => ({
                              ...current,
                              [finding.label]: !current[finding.label],
                            }))
                          }
                        >
...
function ExaminationWorkspace({ specialty }: { specialty: SpecialtyId }) {
  const activeSpecialty =
    clinicalSpecialties.find((item) => item.id === specialty) ?? clinicalSpecialties[0];
  ...
  const visibleFindings =
    specialty === "cvs" ? findings.filter((item) => item.label !== "Heart sounds") : findings;
  return (
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 space-y-4">
          <ToggleMatrix
            key={specialty}
            findings={findings}
            specialtyLabel={activeSpecialty.label}
          />
          <Card>
            <CardHeader>
              <div>
                ...
                <CardDescription>
                  Structured {activeSpecialty.department.toLowerCase()} entries, severity, numeric
                  score fields, and doctor impression in one compact working screen.
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <textarea
                key={specialty}
                className="min-h-28 w-full rounded-lg border border-input bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-ring/15"
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline">Insert shortcut</Button>
        ...
        <div className="min-w-0 space-y-4">
          <ClinicalScorePanel />
          <TrendCard compact />
          <AlertBanner icon={AlertTriangle} tone="warning" title="Risk indicator">
            NEWS score moderate. Consultant verification recommended before final submit.
          </AlertBanner>
        </div>
      </div>
    </div>
...
function ClinicalScorePanel() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Clinical scoring</CardTitle>
        <CardDescription>Auto-calculated risk and red flag state.</CardDescription>
      </CardHeader>
        ...
        {clinicalScores.map((score) => (
          <div className="rounded-lg border border-border bg-surface-muted p-3" key={score.label}>
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-bold">{score.label}</div>
              <Badge tone={severityTone[score.severity]}>{score.value}</Badge>
            </div>
...
function TrendCard({ compact }: { compact?: boolean }) {
  return (
    <Card>
      <CardHeader>
        <div>
          <CardTitle>Graph & trend analysis</CardTitle>
          <CardDescription>Vitals, score, recovery, and risk progression.</CardDescription>
        </div>
        {!compact ? (
          <Button size="sm" variant="outline" onClick={() => toast.info("PDF export queued")}>
...
function TimelinePanel() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Clinical history timeline</CardTitle>
        <CardDescription>
          Date/time-wise entries, previous comparison, and doctor ownership.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {clinicalTimeline.map((item) => (
          <div className="rounded-xl border border-border bg-white p-3 shadow-sm" key={item.id}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="text-sm font-bold text-foreground">{item.specialty} examination</div>
              <Badge tone={item.status.includes("verified") ? "success" : "info"}>
                {item.status}
              </Badge>
            </div>
...
function BuilderAndReports() {
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>Dynamic assessment builder</CardTitle>
          <CardDescription>
            Configurable sections, scoring, mandatory rules, and conditional logic.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid gap-2 sm:grid-cols-2">
            {builderFields.map((field) => (
              <button
                className="rounded-lg border border-border bg-surface-muted px-3 py-2 text-left text-sm font-semibold transition hover:bg-white hover:shadow-sm"
                key={field}
              >
                <Plus className="mr-2 inline h-4 w-4 text-primary" />
                {field}
              </button>
            ))}
          </div>
          <AlertBanner icon={ShieldCheck} tone="info" title="Template governance">
            Reusable templates support specialty, department, doctor, mandatory field, score, and
            audit-ready edit tracking.
          </AlertBanner>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle>Investigation & report linking</CardTitle>
          <CardDescription>
            Inline preview-ready links for labs, ECG, imaging, and uploaded documents.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          {reportLinks.map((report) => (
            ...
            >
              ...
              <Badge tone={report.status.includes("Critical") ? "danger" : "info"}>
                {report.status}
              </Badge>
            </div>
          ))}
...
function NotesApprovalArchitecture() {
  return (
    <div className="grid gap-4 xl:grid-cols-3">
      <Card>
        <CardHeader>
          <CardTitle>Notes & impression</CardTitle>
          <CardDescription>Smart-template ready doctor note surface.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <textarea
            className="min-h-40 w-full rounded-lg border border-input bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-ring/15"
            defaultValue="Clinical impression: BP elevation with stable perfusion. Differential diagnosis and follow-up plan to be reviewed with consultant."
          />
          <div className="flex flex-wrap gap-2">
            <Button variant="outline">
              <Mic className="h-4 w-4" />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Junior doctor entry, consultant verification, audit logs, and digital signature state.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          {[
            "Junior doctor draft captured",
            "Consultant verification pending",
            "Digital signature ready",
            "Edit history retained",
          ].map((item, index) => (
            <div
              className="flex items-center gap-2 rounded-lg border border-border bg-surface-muted p-3 text-sm font-semibold"
              key={item}
            >
              <CheckCircle2
                className={cn("h-4 w-4", index === 1 ? "text-warning" : "text-success")}
              />
              {item}
            </div>
          ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Backend/API ready</CardTitle>
          <CardDescription>Phase-wise contracts and database structure.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-1">
            {backendApis.slice(0, 5).map((api) => (
              <div
                className="rounded-md bg-surface-muted px-2 py-1 text-xs font-semibold text-muted-foreground"
                key={api}
              >
                {api}
              </div>
...
function MobileTabletOptimization() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Doctor workflow and mobile/tablet optimization</CardTitle>
        <CardDescription>
          Low-scroll OPD/IPD rounds flow with touch-friendly actions.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div
            className="rounded-lg border border-border bg-surface-muted p-3 text-sm font-semibold"
            key={step}
          >
        ...
        <div className="rounded-lg border border-primary/20 bg-primary-soft p-3 text-sm font-semibold text-primary xl:col-span-4">
          Tablet: collapsible left menu, large touch buttons, sticky patient header, floating save.
          Mobile: step tabs, swipe navigation, bottom action bar, voice input support.
        </div>
      </CardContent>
    </Card>
  );
...
function ClinicalActionWorkspace({
  action,
  specialty,
  patient,
  rows,
  ...
  patient: ClinicalPatientContext;
  rows: ClinicalActionRow[];
  onAddRow: (_row: ClinicalActionRow) => void;
...
function ClinicalActionForm({
  ...
  const [fieldValues, setFieldValues] = React.useState<Record<string, string>>(() =>
    Object.fromEntries(
      config.fields.map((field) => [field.label, field.value ?? field.options?.[0] ?? ""]),
    ),
  );
  ...
  const addDynamicEntry = (buttonLabel: string) => {
    ...
    const type =
      fieldValues[primaryField.label] ||
      primaryField.value ||
      primaryField.options?.[0] ||
      config.title;
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 md:grid-cols-2">
              {config.fields.map((field) => (
                <ClinicalField
                  key={field.label}
                  field={field}
                  value={fieldValues[field.label] ?? field.value ?? field.options?.[0] ?? ""}
                  onChange={(value) =>
                    setFieldValues((current) => ({ ...current, [field.label]: value }))
                  }
                />
              ))}
            </div>
            {config.textarea ? (
              <label className="block space-y-1.5">
                ...
                <textarea
                  className="min-h-32 w-full rounded-xl border border-input bg-white p-3 text-sm font-semibold outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15"
                  value={note}
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Clinical handoff</CardTitle>
            <CardDescription>Connected status for current encounter.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            {config.handoff.map((item) => (
              <div
                key={item.label}
        ...
        <AlertBanner icon={ShieldCheck} tone="info" title="Production workflow">
          Autosave, audit log, consultant verification, print, and EMR handoff states are modeled
          for this workspace.
        </AlertBanner>
        <Button className="w-full" variant="outline" onClick={onOpenExamination}>
          Back to examination
        </Button>
      </div>
    </div>
...
function dynamicStatusForAction(
  action: string,
  buttonLabel: string,
  values: Record<string, string>,
): Pick<ClinicalActionRow, "status" | "tone"> {
  if (buttonLabel.toLowerCase().includes("print") || buttonLabel.toLowerCase().includes("export"))
    return { status: "Printed", tone: "success" };
  if (
    buttonLabel.toLowerCase().includes("verification") ||
    buttonLabel.toLowerCase().includes("sign-off")
  )
    return { status: "Pending review", tone: "warning" };
  if (action === "Prescription" && values["Pharmacy status"] === "Send to pharmacy")
    return { status: "Pharmacy queue", tone: "info" };
  if (
    action === "Vitals" &&
    values["Pain score"] &&
    !["0", "1", "2"].includes(values["Pain score"])
  )
    return { status: "Watch", tone: "warning" };
  if (action === "Reports" && values.Status === "Critical")
...
const clinicalActionConfigs: Record<
  string,
  {
    title: string;
    description: string;
...
> = {
  "Progress Notes": {
    ...
    fields: [
      {
        label: "Note type",
        options: ["SOAP note", "Round note", "Procedure note", "Follow-up note"],
      },
      { label: "Priority", options: ["Routine", "Monitor", "Urgent"] },
      { label: "Author", value: "Dr. Kavita Rao" },
      { label: "Next review", value: "Today 18:00" },
    ],
    textarea: {
      label: "Clinical note",
      value:
        "Subjective: Chest tightness improved. Objective: BP elevated but perfusion stable. Assessment: Continue BP monitoring. Plan: Review labs and adjust medication if trend persists.",
    },
    actions: ["Save progress note", "Send for verification", "Print note"],
    handoff: [
      { label: "Autosave", status: "Ready", tone: "success" },
      { label: "Consultant review", status: "Pending", tone: "warning" },
      { label: "EMR sync", status: "Queued", tone: "info" },
    ],
  },
  Orders: {
    title: "Clinical Orders",
    description:
      "Medication, lab, radiology, diet, nursing, and procedure orders for the active encounter.",
    badge: "6 order sets",
    badgeTone: "info",
    fields: [
      {
        label: "Order type",
        options: ["Lab", "Radiology", "Medication", "Nursing", "Diet", "Procedure"],
      },
      { label: "Priority", options: ["Routine", "Urgent", "STAT"] },
      { label: "Department", options: ["Laboratory", "Radiology", "Pharmacy", "Nursing"] },
      { label: "Start time", value: "Now" },
    ],
    textarea: {
      label: "Order instructions",
      value: "Repeat BP charting 4 hourly. CBC, LFT, KFT and ECG before consultant review.",
    },
    actions: ["Place order", "Save order set", "Print requisition"],
    handoff: [
      { label: "Lab handoff", status: "Ready", tone: "success" },
      { label: "Pharmacy", status: "Pending", tone: "warning" },
      { label: "Billing", status: "Connected", tone: "info" },
    ],
  },
  Prescription: {
    title: "Prescription",
    description:
      "Medication prescription with allergy warnings, dose checks, frequency, duration, and pharmacy handoff.",
    badge: "Allergy alert",
    badgeTone: "warning",
    fields: [
      { label: "Drug", value: "Amlodipine" },
      { label: "Dose", options: ["2.5 mg", "5 mg", "10 mg"] },
      { label: "Frequency", options: ["OD", "BD", "TDS", "SOS"] },
      { label: "Duration", value: "7 days" },
      { label: "Route", options: ["Oral", "IV", "IM", "Topical"] },
      { label: "Pharmacy status", options: ["Draft", "Send to pharmacy", "Hold"] },
    ],
    textarea: {
      label: "Prescription notes",
      value: "Avoid Penicillin. Continue antihypertensive therapy and review BP trend.",
    },
    actions: ["Add medicine", "Send to pharmacy", "Print prescription"],
    handoff: [
      { label: "Allergy check", status: "Active", tone: "warning" },
      { label: "Drug interaction", status: "Clear", tone: "success" },
      { label: "Pharmacy queue", status: "Ready", tone: "info" },
    ],
  },
  Vitals: {
    title: "Vitals",
    ...
    badgeTone: "warning",
    fields: [
      { label: "BP", value: "148/92" },
      { label: "Pulse", value: "86 bpm" },
      { label: "SpO2", value: "97%" },
      { label: "Temperature", value: "37.2 C" },
      { label: "Respiratory rate", value: "18/min" },
      { label: "Pain score", options: ["0", "1", "2", "3", "4", "5", "6+"] },
    ],
    textarea: {
      label: "Vitals note",
      value: "BP remains above baseline. No respiratory distress. Continue monitoring.",
    },
    actions: ["Save vitals", "Add to trend", "Notify nurse"],
    handoff: [
      { label: "NEWS score", status: "Moderate", tone: "warning" },
      { label: "Nursing chart", status: "Updated", tone: "success" },
      { label: "Doctor alert", status: "Notified", tone: "info" },
    ],
  },
  Reports: {
    title: "Reports",
    description:
      "Lab, radiology, ECG, imaging, and document linking with critical value visibility.",
    badge: "3 linked",
    badgeTone: "info",
    fields: [
      { label: "Report type", options: ["Lab", "Radiology", "ECG", "Echo", "Uploaded document"] },
      { label: "Status", options: ["Available", "Pending", "Critical", "Reviewed"] },
      { label: "Date range", value: "Last 7 days" },
      { label: "Compare with", options: ["Previous visit", "Previous admission", "None"] },
    ],
    textarea: {
      label: "Report interpretation",
      value: "ECG reviewed. Lab values pending final consultant interpretation.",
    },
    actions: ["Link report", "Mark reviewed", "Export PDF"],
    handoff: [
      { label: "Critical values", status: "None", tone: "success" },
      { label: "Radiology", status: "Pending", tone: "warning" },
      { label: "EMR filing", status: "Ready", tone: "info" },
    ],
  },
  "Nursing Notes": {
    title: "Nursing Notes",
    description:
      "Nursing observation, care tasks, intake-output note, safety checks, and escalation workflow.",
    badge: "Bedside",
    badgeTone: "info",
    fields: [
      { label: "Shift", options: ["Morning", "Evening", "Night"] },
      { label: "Care level", options: ["Routine", "Close watch", "Critical"] },
      { label: "Fall risk", options: ["Low", "Moderate", "High"] },
      { label: "Intake/output", value: "Balanced" },
    ],
    textarea: {
      label: "Nursing observation",
      value:
        "Patient comfortable, oriented, BP monitored. No acute distress. Medication administered as per chart.",
    },
    actions: ["Save nursing note", "Escalate", "Print shift note"],
    handoff: [
      { label: "Care plan", status: "Active", tone: "success" },
      { label: "Escalation", status: "None", tone: "success" },
      { label: "Shift handover", status: "Pending", tone: "warning" },
    ],
  },
  "Discharge Summary": {
    title: "Discharge Summary",
    description:
      "Discharge-ready diagnosis, course, medication, follow-up, advice, and consultant sign-off.",
    badge: "Not finalized",
    badgeTone: "warning",
    fields: [
      { label: "Discharge type", options: ["Routine", "DAMA", "Transfer", "Death summary"] },
      { label: "Primary diagnosis", value: "Hypertension follow-up" },
      { label: "Follow-up", value: "7 days" },
      { label: "Consultant sign-off", options: ["Pending", "Approved"] },
    ],
    textarea: {
      label: "Summary",
      value:
        "Patient clinically stable. BP monitoring advised. Follow up with cardiology OPD with reports.",
    },
    actions: ["Save summary", "Request sign-off", "Print discharge"],
    handoff: [
      { label: "Pharmacy clearance", status: "Pending", tone: "warning" },
      { label: "Billing clearance", status: "Pending", tone: "warning" },
      { label: "Consultant", status: "Review", tone: "info" },
    ],
  },
};
...
function ClinicalField({
  ...
  return (
    <label className="space-y-1.5 rounded-xl border border-border bg-white p-3 shadow-soft">
      ...
      {field.options ? (
        <select
          className="h-10 w-full rounded-lg border border-input bg-white px-3 text-sm font-semibold outline-none focus:border-primary focus:ring-2 focus:ring-primary/15"
          value={value}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Audit-ready entries for this clinical workflow.</CardDescription>
      </CardHeader>
      <CardContent className="overflow-x-auto p-0">
        <table className="w-full min-w-[680px] text-sm">
          <thead className="bg-surface-muted text-xs text-muted-foreground">
            <tr>
              {["Time", "Type", "Owner", "Status", "Action"].map((head) => (
                <th className="px-3 py-3 text-left font-semibold" key={head}>
                  {head}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row) => (
              <tr className="border-t border-border" key={`${row.time}-${row.type}`}>
                <td className="px-3 py-3 font-semibold">{row.time}</td>
...
const actionRows: Record<string, ClinicalActionRow[]> = {
  ...
  Reports: [
    { time: "Today 09:50", type: "ECG", owner: "Diagnostics", status: "Reviewed", tone: "success" },
    { time: "Today 10:05", type: "CBC", owner: "Laboratory", status: "Pending", tone: "warning" },
  ],
  "Nursing Notes": [
    {
      time: "Today 11:00",
    ...
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        {[
          ...
          <div
            className="flex justify-between gap-3 border-b border-border py-1.5 last:border-0"
            key={label}
          >
...
export function ClinicalExaminationPage({
  ...
  const requestedPatientId = embedded
    ? (embeddedPatientId ?? mockPatients[0]?.id)
    : (searchParams.get("patientId") ?? initialPatientId ?? mockPatients[0]?.id);
  const selectedPatientRecord =
    mockPatients.find((patient) => patient.id === requestedPatientId) ?? mockPatients[0];
  ...
  const [actionActivity, setActionActivity] =
    React.useState<Record<string, ClinicalActionRow[]>>(actionRows);
  ...
  const openSpecialty = (id: SpecialtyId) => {
    ...
    window.setTimeout(
      () => inlineExamRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }),
      80,
    );
    toast.success(
      `${clinicalSpecialties.find((item) => item.id === id)?.label ?? "Specialty"} examination opened`,
    );
  };
  ...
  return (
    <PageMotion>
      <PatientHeaderStrip
        ...
        selectedPatientId={selectedPatientRecord.id}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent
          ...
          className={cn(
            "gap-4",
            inlineSpecialty ? "space-y-4" : "grid xl:grid-cols-[minmax(0,1fr)_360px]",
          )}
        >
          <div className="space-y-4">
            <SpecialtyPicker selected={specialty} onSelect={openSpecialty} />
            {inlineSpecialty ? (
              <div ref={inlineExamRef} className="scroll-mt-24">
                <ExaminationWorkspace specialty={inlineSpecialty} />
              </div>
            ) : null}
          </div>
        ...
        <TabsContent value="action">
          <ClinicalActionWorkspace
            ...
            specialty={specialty}
            patient={selectedPatient}
            rows={actionActivity[activeAction] ?? []}
            onAddRow={(row) => addActionRow(activeAction, row)}
            onOpenExamination={() => openAction("Clinical Examination")}
          />
        </TabsContent>
      </Tabs>
      <div className="fixed inset-x-3 bottom-3 z-30 flex gap-2 rounded-xl border border-border bg-white/95 p-2 shadow-[0_16px_40px_rgba(39,37,54,0.18)] backdrop-blur md:hidden">
        {["Exam", "Scores", "Notes", "Save"].map((item, index) => (
          <Button
            className="flex-1"
            key={item}
            size="sm"
      ...
      {embedded ? (
        ...
        <StickyActionBar
          onSave={() => toast.success("Clinical examination saved as draft")}
          saveLabel="Final submit"
        />
      )}
    </PageMotion>
  );
