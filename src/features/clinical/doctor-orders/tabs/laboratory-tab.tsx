...
function buildLaboratorySnapshotRows(
  testIds: string[],
  groupIds: string[],
...
function buildLaboratorySnapshotBlocks(
  testIds: string[],
  groupIds: string[],
...
export function LaboratoryTab() {
  ...
  const [savedSummaryRows, setSavedSummaryRows] = React.useState<PathologySummaryRow[]>(() =>
    buildSavedLaboratoryRows(selectedByDefault, selectedGroupDefault),
  );
  const [savedResultBlocks, setSavedResultBlocks] = React.useState<PathologyResultBlock[]>(() =>
    buildSavedLaboratoryBlocks(selectedByDefault, selectedGroupDefault),
  );
  ...
  const filteredTests = React.useMemo(() => {
    ...
    return testList.filter((test) => {
      const matchesSearch = `${test.name} ${test.description} ${test.code ?? ""}`
        .toLowerCase()
        .includes(query);
  ...
  const _availableDepartments = React.useMemo(
    () => ["All", ...Array.from(new Set(testList.map((test) => test.department)))],
    [],
  );
  const _selectedTests = React.useMemo(
    () => testList.filter((test) => selectedTestIds.includes(test.id)),
    [selectedTestIds],
  );
  ...
  const selectedResultBlocks = React.useMemo<LaboratoryResultBlock[]>(
    () =>
      savedResultBlocks.map((block) => ({
        ...
        name: block.name,
        specimen:
          savedSummaryRows.find(
            (row) => normalizeSelectionLabel(row.name) === normalizeSelectionLabel(block.name),
          )?.specimen ?? "Blood",
          ...
          <CardContent className="space-y-4">
            ...
            <TabsContent value="order-summary" className="mt-0">
              <LaboratoryOrderSummaryTab
                ...
                onViewAll={() =>
                  setSavedSummaryRows(buildSavedLaboratoryRows(savedTestIds, savedGroupIds))
                }
