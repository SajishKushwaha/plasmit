...
export function LaboratoryCriticalFindingsTab({
  resultBlocks,
}: {
  ...
  const pagedFindings = React.useMemo(
    () => sortedFindings.slice(currentPage * pageSize, currentPage * pageSize + pageSize),
    [currentPage, pageSize, sortedFindings],
  );
              ...
              {pagedFindings.map((finding) => (
                <tr key={finding.id} className="border-t border-danger/10">
                  ...
                  <td className="px-3 py-2">
                    <span className="inline-flex rounded-full bg-danger/10 px-2 py-0.5 text-[10px] font-medium text-danger">
                      {finding.flag}
                    </span>
                  </td>
                </tr>
