...
const initialItems: ProcedureItem[] = [
  {
    ...
    priority: "Routine",
    reason: "Post-op wound care",
    requiredDateTime: "2026-06-08T11:00",
    consentRequired: false,
    specialInstructions: "Aseptic technique",
    assignedDepartment: "Nursing",
    status: "Ordered",
  },
  {
    id: "proc-2",
    procedureName: "Central line insertion",
    procedureType: "ICU",
    priority: "STAT",
    reason: "Vasopressor support",
    requiredDateTime: "2026-06-08T12:30",
    consentRequired: true,
    specialInstructions: "Use ultrasound",
...
export function ProceduresTab() {
  ...
  const filtered = items.filter((item) =>
    `${item.procedureName} ${item.procedureType} ${item.status}`
      .toLowerCase()
            ...
            <div className="grid gap-4 ">
              <div className="grid gap-4 md:grid-cols-3">
                <label className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">Procedure Name</div>
                  <Input
                ...
                <label className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">Procedure Type</div>
                  <select
                ...
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, priority: e.target.value as ProcedurePriority }))
                    }
                  >
                ...
                <label className="space-y-2">
                  ...
                  <Input
                    type="datetime-local"
                    value={draft.requiredDateTime}
                    onChange={(e) => setDraft((d) => ({ ...d, requiredDateTime: e.target.value }))}
                  />
                </label>
                <label className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">Consent Required</div>
                  <select
                    className="h-10 w-full rounded-md border border-input px-3 text-sm"
                    value={String(draft.consentRequired)}
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, consentRequired: e.target.value === "true" }))
                    }
                  >
                    <option value="true">Written Consent</option>
                ...
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, assignedDepartment: e.target.value }))
                    }
                  >
                    {departments.map((d) => (
                ...
                <label className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, specialInstructions: e.target.value }))
                    }
                  />
                </label>
              </div>
          ...
          {activeTab === "order-summary" ? (
            <div className="space-y-3">
              {filtered.map((item) => (
                <div
                  key={item.id}
                  className="grid gap-3 rounded-xl border border-border bg-surface p-4 md:grid-cols-[minmax(0,1fr)_120px_100px_auto] md:items-center"
                >
                  <div>
