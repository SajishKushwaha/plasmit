...
export function RadiologyTestOrderTab({
  ...
  priority: RadiologyPriority;
  onPriorityChange: (_value: RadiologyPriority) => void;
  notes: string;
  onNotesChange: (_value: string) => void;
  onOpenSummary: () => void;
  onSave: () => void;
}) {
  const [activeCategory, setActiveCategory] = React.useState("CT Scan");
  const [_selectedSpecs, setSelectedSpecs] = React.useState<string[]>(["Left"]);
  ...
  React.useEffect(() => {
    const currentSpecs = activeTest?.specifications ?? [
      "Left",
      "Right",
      "Upper",
      "Lower",
  ...
  const _toggleSpec = (label: string) => {
