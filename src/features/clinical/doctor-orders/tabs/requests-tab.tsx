...
const initialItems: RequestItem[] = [
  {
    ...
    bloodProduct: "",
    units: "",
    description: "Patient needs repeat vitals",
    priority: "Routine",
    requiredByDateTime: "2026-06-08T10:30",
    assignedToDepartment: "Nursing",
    remarks: "Before rounds",
    status: "Requested",
  },
  {
    id: "req-2",
    requestType: "Physiotherapy",
    requestTitle: "Mobility review",
    dietType: "",
    ...
    bloodProduct: "",
    units: "",
    description: "Post-op ambulation support",
    priority: "Urgent",
    requiredByDateTime: "2026-06-08T12:00",
...
function _isType(value: string, type: RequestType) {
  return value === type;
}
...
export function RequestsTab() {
    ...
    setDraft((current) => {
      ...
      if (type === "Dietary")
        return {
          ...base,
          requestTitle: base.requestTitle || "",
          dietType: base.dietType || "Normal Diet",
          assignedToDepartment: "Dietary",
          sessionFrequency: "",
          equipmentType: "",
          quantity: "",
          fromLocation: "",
      ...
      if (type === "Physiotherapy")
        return {
          ...base,
          requestTitle: base.requestTitle || "",
          sessionFrequency: base.sessionFrequency || "Once",
          assignedToDepartment: "Physiotherapy",
          dietType: "",
          equipmentType: "",
          quantity: "",
      ...
      if (type === "Equipment")
        return {
          ...base,
          requestTitle: base.requestTitle || "",
          equipmentType: base.equipmentType || "Wheelchair",
          quantity: base.quantity || "1",
          assignedToDepartment: "Maintenance",
          dietType: "",
          sessionFrequency: "",
          fromLocation: "",
          toLocation: "",
      ...
      if (type === "Transport")
        return {
          ...base,
          requestTitle: base.requestTitle || "",
          fromLocation: base.fromLocation || "Ward",
          toLocation: base.toLocation || "ICU",
          assignedToDepartment: "Transport",
          dietType: "",
          sessionFrequency: "",
          equipmentType: "",
          quantity: "",
      ...
      if (type === "Discharge")
        return {
          ...base,
          requestTitle: base.requestTitle || "",
          expectedDischargeDate: base.expectedDischargeDate || todayIso(),
          assignedToDepartment: "Ward",
          dietType: "",
          sessionFrequency: "",
          equipmentType: "",
          quantity: "",
      ...
      return {
        ...base,
        requestTitle: base.requestTitle || "",
        bloodProduct: base.bloodProduct || "Packed RBC",
        units: base.units || "1",
        assignedToDepartment: "Blood Bank",
        dietType: "",
        sessionFrequency: "",
        equipmentType: "",
        quantity: "",
  ...
  const filtered = items.filter((item) =>
    `${item.requestTitle} ${item.requestType} ${item.status} ${item.dietType} ${item.equipmentType} ${item.bloodProduct}`
      .toLowerCase()
  ...
  const save = () => {
    const needsTitle =
      ["Nursing", "Housekeeping", "Other"].includes(draft.requestType) ||
      !["Dietary", "Physiotherapy", "Equipment", "Transport", "Discharge", "Blood Bank"].includes(
        draft.requestType,
      );
    if (needsTitle && !draft.requestTitle.trim()) return toast.error("Request Title is required");
    if (draft.requestType === "Dietary" && !draft.dietType)
      return toast.error("Diet Type is required");
    if (draft.requestType === "Physiotherapy" && !draft.sessionFrequency)
      return toast.error("Session Frequency is required");
    if (draft.requestType === "Equipment" && (!draft.equipmentType || !draft.quantity.trim()))
      return toast.error("Equipment Type and Quantity are required");
    if (draft.requestType === "Transport" && (!draft.fromLocation || !draft.toLocation))
      return toast.error("From and To Location are required");
    if (draft.requestType === "Discharge" && !draft.expectedDischargeDate)
      return toast.error("Expected Discharge Date is required");
    if (draft.requestType === "Blood Bank" && (!draft.bloodProduct || !draft.units.trim()))
      return toast.error("Blood Product and Units are required");
    if (editingId) {
      setItems((current) =>
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Doctor general requests for nursing, dietary, physiotherapy, equipment, transport,
            housekeeping, discharge, and blood bank.
          </CardDescription>
        </CardHeader>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    value={requestType}
                    onChange={(e) => syncTypeDefaults(e.target.value as RequestType)}
                  >
                    {requestTypes.map((t) => (
                ...
                {[
                  ...
                  <label className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setDraft((d) => ({ ...d, requestTitle: e.target.value }))}
                ...
                <label className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, priority: e.target.value as RequestPriority }))
                    }
                  >
                ...
                <label className="space-y-2">
                  ...
                  <Input
                    type="datetime-local"
                    value={draft.requiredByDateTime}
                    onChange={(e) =>
                      setDraft((d) => ({ ...d, requiredByDateTime: e.target.value }))
                    }
                  />
                </label>
                ...
                {requestType === "Dietary" ? (
                  <label className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) => setDraft((d) => ({ ...d, dietType: e.target.value }))}
                    >
                ...
                {requestType === "Physiotherapy" ? (
                  <label className="space-y-2">
                    ...
                    <select
                      className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                      value={draft.sessionFrequency}
                      onChange={(e) =>
                        setDraft((d) => ({ ...d, sessionFrequency: e.target.value }))
                      }
                    >
                      <option value="">Select</option>
                      {sessionFrequencies.map((item) => (
                        <option key={item}>{item}</option>
                      ))}
                    </select>
                  </label>
                  ...
                  <>
                    <label className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={(e) => setDraft((d) => ({ ...d, equipmentType: e.target.value }))}
                      >
                        <option value="">Select</option>
                ...
                {requestType === "Discharge" ? (
                  <label className="space-y-2">
                    ...
                    <Input
                      type="date"
                      value={draft.expectedDischargeDate}
                      onChange={(e) =>
                        setDraft((d) => ({ ...d, expectedDischargeDate: e.target.value }))
                      }
                    />
                  </label>
                ) : null}
                ...
                {requestType === "Blood Bank" ? (
                  <>
                    <label className="space-y-2">
                      ...
                      <select
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                        value={draft.bloodProduct}
                        onChange={(e) => setDraft((d) => ({ ...d, bloodProduct: e.target.value }))}
                      >
              ...
              <div className="space-y-3 rounded-xl border border-border bg-surface-muted p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="rounded-md border border-border bg-white p-3">
                    Title: <span className="font-semibold">{draft.requestTitle || "-"}</span>
                  </div>
                  <div className="rounded-md border border-border bg-white p-3">
                    Type: <span className="font-semibold">{draft.requestType}</span>
                  </div>
                  <div className="rounded-md border border-border bg-white p-3">
                    Priority: <span className="font-semibold">{draft.priority}</span>
                  </div>
                  <div className="rounded-md border border-border bg-white p-3">
                ...
                >
                  <div>
                    <div className="font-semibold text-foreground">
                      {item.requestTitle || item.requestType}
                    </div>
