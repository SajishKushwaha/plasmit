...
import {
  ...
  SensitivityBadge,
  SignatureBadge,
} from "@/features/clinical/emr/emr-shared";
import {
  getEmrEncounterById,
...
export function EmrDashboardPage() {
  ...
  const rows = mockEmrEncounters.filter((encounter) => {
    ...
    return (
      ...
      (type === "All types" || encounter.encounterType === type) &&
      (status === "All status" || encounter.status === status)
    );
  });
  ...
  const columns = React.useMemo<ColumnDef<EmrEncounter>[]>(
    () => [
      ...
      {
        header: "Signature",
        cell: ({ row }) => <SignatureBadge status={row.original.signatureStatus} />,
      },
      {
        header: "Actions",
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            <Button size="sm" asChild>
              <Link href={`/emr/patients/${row.original.patientId}`}>Open</Link>
            </Button>
      ...
      {() => (
        <>
          <PageHeader
            ...
            actions={
              <>
                <Button
                  variant="outline"
                  onClick={() => toast.success("Recent static records refreshed")}
                >
          ...
          <SummaryGrid>
            <StatCard
              label="Viewed today"
              value={mockRecordAccessAudit.length}
              change="Static audit"
            ...
            <StatCard
              label="Recent OPD"
              value={mockEmrEncounters.filter((item) => item.encounterType === "OPD").length}
              change="Clinical"
              context="OPD records"
              tone="success"
              icon={Stethoscope}
            ...
            <StatCard
              label="Recent IPD"
              value={mockEmrEncounters.filter((item) => item.encounterType === "IPD").length}
              change="Inpatient"
              context="IPD records"
              tone="warning"
              icon={FileText}
            ...
            <StatCard
              label="Emergency"
              value={mockEmrEncounters.filter((item) => item.encounterType === "Emergency").length}
              change="Restricted"
              context="Break-glass"
              tone="critical"
              icon={ShieldAlert}
            ...
            <StatCard
              ...
              value={mockDigitalSignatures.filter((item) => item.status === "Pending").length}
              change="Worklist"
              context="Doctor action"
              tone="warning"
              icon={FileSignature}
            />
            <StatCard
              label="Attach verify"
              value={
                mockClinicalAttachments.filter(
                  (item) => item.verificationStatus === "Pending verification",
                ).length
              }
              change="Documents"
          ...
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search patient, UHID, encounter, department, provider, status..."
          >
            <NativeSelect
              label="Type"
              value={type}
      ...
      <ProtectedEmr>
        {() => (
          <EmptyState
            ...
            description="The selected patient is not part of the static Phase 7 record set."
          />
        )}
      </ProtectedEmr>
    );
    ...
    <ProtectedEmr>
      {({ readOnly }) => (
        <>
            ...
            actions={
              <>
                ...
                <Button variant="outline" asChild>
                  <Link href={`/emr/patients/${patientId}/audit`}>Access audit</Link>
                </Button>
          ...
          {encounters.some((item) => item.sensitivity === "Break-glass placeholder") ? (
            <BreakGlassWarning />
          ) : (
            <PrivacyWarning />
          )}
          <Tabs defaultValue="encounters" className="space-y-4">
            ...
            <TabsContent value="opd" className="grid gap-3 lg:grid-cols-2">
              {encounters
                .filter(
                  (item) =>
                    item.encounterType === "OPD" ||
                    item.encounterType === "Teleconsultation placeholder",
                )
                .map((item) => (
                  <EncounterSummaryCard key={item.id} encounter={item} onOpen={setSelected} />
                ))}
            </TabsContent>
            <TabsContent value="ipd">
              <SimpleRecords
                title="IPD continuity"
                records={mockAdmissions.filter((item) => item.patientId === patientId)}
              />
            </TabsContent>
            <TabsContent value="emergency">
              <SimpleRecords
                title="Emergency continuity"
                records={mockEmergencyCases.filter((item) => item.patientId === patientId)}
              />
            </TabsContent>
            <TabsContent value="rx">
              <SimpleRecords
                title="Medication continuity"
                records={mockPrescriptionMedicines.filter((item) =>
                  encounters.some((encounter) =>
                    encounter.prescriptionSummary.includes(item.medicineName),
                  ),
                )}
              />
            </TabsContent>
            <TabsContent value="vitals">
              <SimpleRecords
                title="Vitals trend records"
                records={mockVitals.filter((item) => item.patientId === patientId)}
              />
            </TabsContent>
            <TabsContent value="dx">
              <SimpleRecords
                title="Diagnosis records"
                records={mockDiagnoses.filter((item) => item.patientId === patientId)}
              />
            </TabsContent>
            <TabsContent value="documents">
              <SimpleRecords
                title="Clinical and patient documents"
                records={[
                  ...mockClinicalAttachments.filter((item) => item.patientId === patientId),
                  ...mockPatientDocuments.filter((item) => item.patientId === patientId),
                ]}
              />
            </TabsContent>
            <TabsContent value="audit">
              <AuditPanel patientId={patientId} compact />
            </TabsContent>
          </Tabs>
      ...
      <ProtectedEmr>
        {() => (
          <EmptyState
            icon={FileText}
            title="Patient not found"
            description="The selected patient is not available."
          />
        )}
      </ProtectedEmr>
    );
          ...
          <PageHeader
            ...
            actions={
              <>
                ...
                <Button asChild>
                  <Link href={`/emr/patients/${patientId}/timeline`}>Clinical timeline</Link>
                </Button>
              </>
            }
          />
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                ...
                <LongitudinalPanel
                  ...
                  records={mockProcedures.filter((item) => item.patientId === patientId)}
                />
                <LongitudinalPanel
                  title="Vaccinations"
                  records={mockVaccinations.filter((item) => item.patientId === patientId)}
                />
                <LongitudinalPanel
                  title="Admission / emergency history"
                  records={[
                    ...mockAdmissions.filter((item) => item.patientId === patientId),
                    ...mockEmergencyCases.filter((item) => item.patientId === patientId),
                  ]}
                />
                <LongitudinalPanel
                  title="Lab / radiology placeholders"
                  records={encounters.filter((item) => item.encounterType.includes("placeholder"))}
                />
              </div>
            </div>
            <Card>
              <CardContent className="space-y-3 p-4">
                ...
                <Button className="w-full" variant="outline" asChild>
                  <Link href={`/patients/${patientId}`}>Open patient profile</Link>
                </Button>
                <Button className="w-full" variant="outline" asChild>
                  <Link href={`/emr/patients/${patientId}/disclosures`}>
...
export function MedicalHistoryPage({ patientId }: { patientId: string }) {
  ...
  const columns = React.useMemo<ColumnDef<MedicalHistoryItem>[]>(
    () => [
      ...
      {
        header: "Sensitivity",
        cell: ({ row }) => <SensitivityBadge sensitivity={row.original.sensitivity} />,
      },
      {
        header: "Actions",
        cell: ({ row }) => (
  ...
  return (
    <PatientRecordTablePage
      patientId={patientId}
      title="Medical History"
      description="Structured past medical, surgical, family, social, medication, allergy, immunization, and obstetric placeholders."
      rows={rows}
      columns={columns}
      icon={History}
      primaryLabel="Add history item"
      drawer={
        <HistoryDrawer
          item={selected}
          open={Boolean(selected)}
...
export function ProgressNotesPage({ patientId }: { patientId: string }) {
  ...
  return (
    <PatientRecordTablePage
      ...
      description="Chronological OPD, IPD, emergency, nursing, round, addendum, and referral note placeholders."
      rows={rows}
      columns={columns}
      icon={NotebookText}
      primaryLabel="Add progress note"
      drawer={
        <ProgressNoteDrawer
          note={selected}
          open={Boolean(selected)}
...
export function AttachmentsPage({ patientId }: { patientId: string }) {
  ...
  const columns = React.useMemo<ColumnDef<ClinicalAttachment>[]>(
    () => [
      ...
      {
        header: "Verification",
        cell: ({ row }) => <EmrStatus status={row.original.verificationStatus} />,
      },
      {
        header: "Signature",
        cell: ({ row }) => <SignatureBadge status={row.original.signatureStatus} />,
  ...
  return (
    <PatientRecordTablePage
      ...
      description="Unified documents, reports, scanned notes, discharge summaries, consent forms, preview, verification, retention, and legal hold placeholders."
      rows={rows}
      columns={columns}
      icon={FileSearch}
      primaryLabel="Upload placeholder"
      drawer={
        <AttachmentDrawer
          attachment={selected}
          open={Boolean(selected)}
...
export function SignaturesPage({ patientId }: { patientId: string }) {
  ...
  return (
    <PatientRecordTablePage
      ...
      description="Signature worklists for pending, signed, rejected, expired certificate, and not-required states without real provider integration."
      rows={rows}
      columns={columns}
      icon={FileSignature}
      primaryLabel="Request signature"
      drawer={
        <SignatureDrawer
          signature={selected}
          open={Boolean(selected)}
  ...
  return (
    <ProtectedEmr>
      {() => (
        <>
          <PageHeader
            eyebrow="Phase 7 • Timeline"
            title="Clinical Timeline"
            description="Chronological cross-module events with filters, linked encounters, sensitive markers, version context, and governance signals."
            actions={
              <>
                <Button variant="outline" onClick={() => window.print()}>
                  <Printer className="h-4 w-4" />
                ...
                <Button asChild>
                  <Link href={`/emr/patients/${patientId}`}>Open EMR</Link>
                </Button>
              </>
            }
          />
          ...
          <FilterBar
            search=""
            onSearch={() => undefined}
            placeholder="Timeline filters are static in this phase"
          >
            <NativeSelect
              label="Category"
              value={category}
              onChange={setCategory}
              options={["All categories", "Clinical", "Operational", "Governance"]}
            />
          </FilterBar>
          <div className="grid gap-4 xl:grid-cols-[280px_1fr_360px]">
            ...
            <div className="space-y-3">
              {rows.map((event) => (
                <button
                  key={event.id}
                  className="w-full rounded-lg border border-border bg-surface p-3 text-left transition hover:bg-surface-muted focus:outline-none focus:ring-2 focus:ring-ring/20"
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        {event.occurredAt} • {event.eventType} • {event.department} •{" "}
                        {event.provider}
                      </div>
                    </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    <Badge tone="info">{event.versionContext}</Badge>
                  </div>
                </button>
              ))}
            </div>
              ...
              <CardContent className="space-y-3 p-4">
                ...
                {selected ? (
                  ...
                  <div className="text-sm text-muted-foreground">
                    Choose a timeline event to review linked encounter, audit note, and governance
                    context.
                  </div>
                )}
              </CardContent>
...
function AuditPanel({ patientId, compact }: { patientId: string; compact?: boolean }) {
  ...
  const columns = React.useMemo<ColumnDef<RecordAccessAudit>[]>(
    () => [
      ...
      {
        header: "IP/device",
        cell: ({ row }) => `${row.original.ipAddress} • ${row.original.device}`,
      },
      ...
      {!compact ? (
        <SummaryGrid>
          <StatCard
            ...
            value={rows.length}
            change="Today"
            context="Static audit"
            tone="info"
            icon={LockKeyhole}
          />
          <StatCard
            label="Restricted"
            value={rows.filter((item) => item.sensitivity === "Restricted").length}
            change="Review"
          ...
          <StatCard
            label="Print/export"
            value={rows.filter((item) => item.action.includes("Export")).length}
            change="Governed"
            context="Privacy warning"
            tone="danger"
            icon={Download}
          />
          <StatCard
            label="Signature events"
            value={rows.filter((item) => item.recordType === "Signature").length}
            change="Worklist"
            context="Audit linked"
            tone="success"
            icon={FileSignature}
          />
        </SummaryGrid>
      ...
      {() => (
        <>
          <PageHeader
            ...
            title="Record Access Audit"
            description="Read-only clinical record access history with restricted, break-glass, print/export, addendum, signature, disclosure, and legal hold events."
            actions={
              <Button variant="outline">
                <Download className="h-4 w-4" />
          ...
          />
          <PatientRecordHeader patientId={patientId} title="Access audit" />
          {content}
        </>
      )}
    </ProtectedEmr>
...
export function VersionsPage({ patientId }: { patientId: string }) {
      ...
      { header: "Updated", accessorKey: "updatedAt" },
      {
        ...
        cell: ({ row }) =>
          row.original.legalHold ? (
            ...
            <Badge tone="success">No hold</Badge>
          ),
      },
      {
        header: "Actions",
  ...
  return (
    <PatientRecordTablePage
      ...
      description="Record lineage with current, previous, addendum, superseded, archived, compare, and legal hold placeholders."
      rows={rows}
      columns={columns}
      icon={FileClock}
      primaryLabel="Add addendum"
      drawer={
        <VersionDrawer
          version={selected}
          open={Boolean(selected)}
...
export function DisclosuresPage({ patientId }: { patientId: string }) {
  ...
  return (
    <PatientRecordTablePage
      ...
      description="Future print/export/share governance with consent, approval, recipient, purpose, record scope, and package generation placeholders."
      rows={rows}
      columns={columns}
      icon={Share2}
  ...
  if (!records.length)
    return (
      <EmptyState
        ...
        description="No static continuity records are available for this patient and filter."
      />
    );
  return (
    <Card>
          ...
          {records.map((record) => (
            ...
            >
              {Object.entries(record)
                .filter(([key]) => key !== "id" && key !== "patientId")
                .slice(0, 6)
                .map(([key, value]) => (
                  <div
                    key={key}
                    className="grid gap-2 border-b border-border/70 py-1 last:border-0 sm:grid-cols-[150px_1fr]"
                  >
    ...
    <Card>
      <CardContent className="space-y-3 p-4">
        <div className="flex items-center justify-between gap-2">
          <div className="text-sm font-semibold">{title}</div>
          <Badge tone={alert && records.length ? "danger" : records.length ? "info" : "muted"}>
        ...
        {records.length ? (
          <div className="space-y-2">
            {records.slice(0, 4).map((record) => (
              ...
              >
                {Object.entries(record)
                  .filter(([key]) => !["id", "patientId", "visitId"].includes(key))
                  .slice(0, 4)
                  .map(([key, value]) => (
          ...
          <div className="rounded-md border border-dashed border-border p-3 text-xs text-muted-foreground">
            No static longitudinal records available.
          </div>
        )}
      </CardContent>
    </Card>
...
function EncounterDrawer({
  ...
  return (
    <Drawer
      ...
      description={
        encounter ? `${encounter.encounterNo} • ${patientName(encounter.patientId)}` : undefined
      }
      className="md:w-[640px]"
    >
      {encounter ? (
        <div className="space-y-4">
          {encounter.sensitivity === "Break-glass placeholder" ? (
            <BreakGlassWarning />
          ) : (
            <PrivacyWarning label="Opening this encounter will be audit tracked in production." />
          )}
          <Card>
            <CardContent className="space-y-1 p-4">
              <DetailRow label="Encounter" value={encounter.encounterNo} />
              <DetailRow label="Type" value={encounter.encounterType} />
              <DetailRow label="Department" value={encounter.department} />
              <DetailRow label="Provider" value={encounter.provider} />
              <DetailRow label="Status" value={<RecordStatusBadge status={encounter.status} />} />
              <DetailRow
                label="Signature"
                value={<SignatureBadge status={encounter.signatureStatus} />}
              />
              <DetailRow
                label="Sensitivity"
                value={<SensitivityBadge sensitivity={encounter.sensitivity} />}
              />
              <DetailRow label="Version" value={<EmrStatus status={encounter.versionState} />} />
            </CardContent>
          </Card>
          <SimpleRecords
            ...
            records={[
              {
                ...
                prescription: encounter.prescriptionSummary,
                documents: `${encounter.documentsCount} linked documents`,
                related: encounter.relatedRoute,
              },
            ]}
          />
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" asChild>
              <Link href={`/emr/patients/${encounter.patientId}/versions`}>Version history</Link>
            </Button>
            <Button asChild>
              <Link href={encounter.relatedRoute}>Cross-module link</Link>
            </Button>
          </div>
        </div>
      ) : null}
      ...
      {item ? (
        <DrawerRows
          rows={[
            ...
            ["Onset", item.onset],
            ["Status", <EmrStatus key="s" status={item.status} />],
            ["Severity", <EmrStatus key="sev" status={item.severity} />],
            ["Source", item.sourceEncounter],
            ["Verified", item.verified ? "Verified" : "Unverified"],
            ["Sensitivity", <SensitivityBadge key="sens" sensitivity={item.sensitivity} />],
            ["Notes", item.notes],
          ]}
          warning="Archive and verification changes require reason capture in production."
        />
      ) : null}
    </Drawer>
  );
  ...
  return (
    ...
    >
      {note ? (
        <div className="space-y-4">
          {note.internalOnly ? (
            <AlertBanner icon={LockKeyhole} tone="warning" title="Internal-only note">
              This note is hidden from patient-facing print/export by default.
            </AlertBanner>
          ) : null}
          <DrawerRows
            rows={[
              ...
              ["Encounter", encounterLabel(note.encounterId)],
              ["Created", note.createdAt],
              ["Status", <RecordStatusBadge key="s" status={note.status} />],
              ["Signature", <SignatureBadge key="sig" status={note.signatureStatus} />],
              ["Version", note.version],
              ["Summary", note.summary],
              ["Full note", note.fullNote],
              ["Addendum", note.addendumHistory],
            ]}
            warning="Completed notes are read-only; addendums never overwrite the original note."
          />
        </div>
      ) : null}
    </Drawer>
      ...
      {attachment ? (
        <div className="space-y-4">
          {attachment.legalHold ? (
            <AlertBanner icon={Archive} tone="critical" title="Legal hold active">
              Archive, destructive, and casual download placeholders are blocked.
            </AlertBanner>
          ) : (
            <PrivacyWarning label="Attachment download and print require privacy acknowledgement." />
          )}
          <div className="rounded-lg border border-border bg-surface-muted p-6 text-center text-sm text-muted-foreground">
            {attachment.previewAvailable
              ? "Document preview placeholder uses print-safe light styling in production."
              : "Unsupported preview. Metadata remains visible and accessible."}
          </div>
          <DrawerRows
            rows={[
              ["Category", attachment.category],
              ["Encounter", encounterLabel(attachment.encounterId)],
              ["Uploaded by", attachment.uploadedBy],
              ["Uploaded", attachment.uploadedAt],
              ["Verification", <EmrStatus key="v" status={attachment.verificationStatus} />],
              ["Signature", <SignatureBadge key="s" status={attachment.signatureStatus} />],
              ["Sensitivity", <SensitivityBadge key="sens" sensitivity={attachment.sensitivity} />],
              ["Retention", attachment.retentionStatus],
            ]}
            warning="Reject/archive actions require reason capture and are placeholders only."
          />
        </div>
      ) : null}
    </Drawer>
...
function SignatureDrawer({
  ...
  return (
    <Drawer
      ...
      description={signature?.documentName}
    >
      {signature ? (
        <DrawerRows
          rows={[
            ["Document", signature.documentName],
            ["Encounter", encounterLabel(signature.encounterId)],
            ["Requested by", signature.requestedBy],
            ["Signer", signature.signer],
            ["Requested", signature.requestedAt],
            ["Status", <SignatureBadge key="s" status={signature.status} />],
            ["Certificate", signature.certificateStatus],
            ["Reason", signature.reason],
          ]}
          warning="No real digital signature provider is connected in Phase 7."
        />
      ) : null}
    </Drawer>
  );
...
function EventDrawer({
  ...
  event: ClinicalTimelineEvent | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}) {
  return (
  ...
  return (
    <DrawerRows
      rows={[
        ...
        ["Status", <EmrStatus key="s" status={event.status} />],
        ["Encounter", encounterLabel(event.encounterId)],
        ["Sensitivity", <SensitivityBadge key="sens" sensitivity={event.sensitivity} />],
        ["Version", event.versionContext],
        ["Summary", event.summary],
      ]}
      warning="Timeline detail preserves linked encounter, audit, version, and disclosure context."
    />
  );
}
      ...
      {audit ? (
        <DrawerRows
          rows={[
            ...
            ["Role", audit.role],
            ["Action", audit.action],
            ["Record", `${audit.recordType} • ${audit.recordId}`],
            ["Reason", audit.reason],
            ["Sensitivity", <SensitivityBadge key="sens" sensitivity={audit.sensitivity} />],
            ["IP", audit.ipAddress],
            ["Device", audit.device],
            ["Outcome", <EmrStatus key="out" status={audit.outcome} />],
          ]}
          warning="Audit data is immutable, read-only, and masks sensitive values where required."
        />
      ) : null}
    </Drawer>
  );
...
function VersionDrawer({
  ...
  return (
    <Drawer
      ...
      description={version?.recordName}
    >
      {version ? (
        <DrawerRows
          rows={[
            ["Record", version.recordName],
            ["Type", version.recordType],
            ["Encounter", encounterLabel(version.encounterId)],
            ["Version", version.version],
            ["State", <EmrStatus key="state" status={version.state} />],
            ["Author", version.author],
            ["Updated", version.updatedAt],
            ["Reason", version.reason],
            ["Current ref", version.currentVersionId],
            ["Legal hold", version.legalHold ? "Active - archive blocked" : "No hold"],
          ]}
          warning="Previous and superseded versions are read-only; compare is a placeholder only."
        />
      ) : null}
    </Drawer>
  );
...
function DisclosureDrawer({
  ...
  request: DisclosureRequest | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}) {
  return (
      ...
      {request ? (
        <DrawerRows
          rows={[
            ...
            ["Scope", request.recordScope],
            ["Consent", <EmrStatus key="consent" status={request.consentStatus} />],
            ["Approval", <EmrStatus key="approval" status={request.approvalStatus} />],
            ["Requested by", request.requestedBy],
            ["Requested", request.requestedAt],
            ["Reason", request.reason],
          ]}
          warning="Package generation is disabled; consent and restricted-record checks are UI placeholders."
        />
      ) : null}
    </Drawer>
  );
...
function DrawerRows({ rows, warning }: { rows: [string, React.ReactNode][]; warning: string }) {
  return (
    <div className="space-y-4">
      <AlertBanner icon={ShieldAlert} tone="warning" title="Governance placeholder">
        {warning}
      </AlertBanner>
      <Card>
        <CardContent className="space-y-1 p-4">
        ...
        <CardContent className="space-y-2 p-4">
          ...
          {mockRetentionPolicies.map((policy) => (
            <div
              ...
              className="rounded-md border border-border bg-surface-muted p-2 text-xs"
            >
              {policy.recordType}: {policy.retention} • {policy.archiveAllowed}
            </div>
          ))}
        </CardContent>
      </Card>
