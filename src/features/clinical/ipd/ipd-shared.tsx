...
export const ipdAccessRoles: Role[] = [
  "Super Admin",
  "Hospital Admin",
  "Doctor",
  "Doctor IPD",
...
export function useIpdAccess() {
  ...
  return {
    role,
    allowed: ipdAccessRoles.includes(role),
    readOnly: ipdReadOnlyRoles.includes(role),
  };
}
...
export function ProtectedIpd({
  ...
  if (!access.allowed) {
    return (
      <EmptyState
        icon={LockKeyhole}
        title="IPD/Emergency permission required"
        description="Your current static role cannot access Phase 6 inpatient or emergency workflows."
      />
    );
  }
  return (
...
function IpdPatientWorkspaceHeader({
  ...
  const patient =
    mockPatients.find((record) => record.id === requestedPatientId) ?? mockPatients[0];
  ...
  const fields = [
    ...
    [
      "Ward/Bed",
      admission ? `${admission.ward} / ${admission.bedId}` : `${patient.department} OPD`,
    ],
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="truncate text-lg font-bold text-foreground">
                {patientContext?.name ?? patientName(patient)}
              </h1>
          ...
          <div className="flex min-w-0 flex-1 flex-wrap justify-start gap-2 xl:justify-end">
                      ...
                      filteredPatients.map((record) => (
                        ...
                        >
                          ...
                          <span className="truncate text-xs text-muted-foreground">
                            {record.uhid} | {record.age}/{record.gender} | {record.department}
                          </span>
                        </button>
                      ))
                    ) : (
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => toast.info("Last IPD draft restored")}
            >
              <RefreshCcw className="h-4 w-4" />
              Restore
            </Button>
            <Button size="sm" onClick={() => toast.success("IPD draft autosaved")}>
...
export function InpatientHeader({ admission }: { admission: AdmissionRecord }) {
  ...
  const isolation = mockInfectionIsolationFlags.find(
    (flag) => flag.patientId === admission.patientId,
  );
  ...
  return (
    <Card>
      <CardContent className="space-y-3 p-3">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              {bed.ward} • {bed.roomNo} • {bed.bedType}
            </div>
          </div>
          <IpdStatus status={bed.status} />
        </div>
        {patient ? (
          <div className="rounded-md bg-surface-muted p-2 text-xs">
        ...
        <Button size="sm" variant="outline" onClick={() => onAction?.(bed)}>
    ...
    <div className="space-y-2">
      {patient?.alertFlags.length ? (
        <AlertBanner icon={ShieldAlert} tone="warning" title="Patient safety context">
          {patient.alertFlags.join(", ")}. These alerts remain visible across nursing, MAR, bed, and
          emergency workflows.
        </AlertBanner>
      ) : null}
      {isolation ? (
