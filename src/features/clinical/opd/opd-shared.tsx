...
import { DetailRow } from "@/features/operations/admin/admin-shared";
import { OperationalStatus } from "@/features/operations/appointments/appointment-shared";
...
export const opdAccessRoles: Role[] = [
  "Super Admin",
  "Hospital Admin",
  "Doctor",
  "Doctor OPD",
...
export const opdReadOnlyRoles: Role[] = [
  "Hospital Admin",
  "Receptionist",
  "Pharmacist",
  "Lab Technician",
...
export function ProtectedOpd({
  children,
}: {
  children: (_state: { role: Role; readOnly: boolean; vitalsOnly: boolean }) => React.ReactNode;
}) {
  ...
  if (!access.allowed) {
    return (
      <EmptyState
        icon={LockKeyhole}
        title="OPD permission required"
        description="Your current static role cannot access OPD clinical workflows."
      />
    );
  }
  return (
...
export function clinicalTone(status: string): StatusTone {
  if (["Completed", "Printed", "Normal", "Active", "Administered", "Controlled"].includes(status))
    return "success";
  if (
    [
      "In progress",
    ...
    return "warning";
  if (
    ["Critical", "Severe", "Missing", "Not started", "Contraindicated placeholder"].includes(status)
  )
    return "danger";
...
export function ClinicalPatientHeader({
  ...
  const allergies = mockAllergies.filter(
    (allergy) => allergy.patientId === patientId && allergy.status === "Active",
  );
      ...
      <CardContent className="flex flex-col gap-3 p-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-2 xl:min-w-[420px]">
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Appt: {appointmentTime ?? "Not linked"}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Vitals:{" "}
            {latestVitals ? `${latestVitals.bloodPressure}, SpO2 ${latestVitals.spo2}` : "Missing"}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Last visit: {patient.lastVisitAt}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
            State: {consultation ? consultation.status : "Not started"}
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="space-y-2">
      {allergies.length ? (
        <AlertBanner
          icon={AlertTriangle}
          tone="danger"
          title="Allergy warning visible during prescribing"
        >
      ...
      {vitals && vitals.status !== "Normal" ? (
        <AlertBanner
          icon={HeartPulse}
          tone={vitals.status === "Critical" ? "critical" : "warning"}
          title={`${vitals.status} vitals`}
        >
          BP {vitals.bloodPressure}, pulse {vitals.pulse}, SpO2 {vitals.spo2}. Unit and reference
          range placeholders are shown in vitals.
        </AlertBanner>
      ) : null}
    </div>
  );
