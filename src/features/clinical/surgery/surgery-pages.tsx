...
} from "@/features/clinical/surgery/surgery-data";
import { DetailRow, SurgeryShell, SurgeryStatus } from "@/features/clinical/surgery/surgery-shared";
...
function searchMatches(value: string, query: string) {
  ...
  return (
    ...
    queryParts.every((part) => searchableValue.includes(part))
  );
}
    ...
    (nextValue: React.SetStateAction<T>) => {
      ...
      const resolvedValue =
        typeof nextValue === "function"
          ? (nextValue as (_current: T) => T)(currentValue)
          : nextValue;
...
function useSurgeryState() {
  const [requests, setRequests] = usePersistentSurgeryState<SurgeryRequest[]>(
    "surgery.requests.v6",
    surgeryRequests,
  );
  const [storedBookings, setStoredBookings] = usePersistentSurgeryState<OtBooking[]>(
    "surgery.bookings.v6",
    otBookings,
  );
  ...
  const bookings = React.useMemo(
    () => ensureUniqueBookingIds(uniqueBookingsByRequest(storedBookings)),
    [storedBookings],
  );
  const setBookings = React.useCallback(
    (nextValue: React.SetStateAction<OtBooking[]>) => {
      setStoredBookings((currentBookings) => {
        const currentUniqueBookings = ensureUniqueBookingIds(
          uniqueBookingsByRequest(currentBookings),
        );
        const resolvedBookings =
          typeof nextValue === "function"
            ? (nextValue as (_current: OtBooking[]) => OtBooking[])(currentUniqueBookings)
            : nextValue;
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={`fixed left-1/2 top-1/2 z-50 max-h-[88dvh] w-[min(calc(100vw-2rem),720px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-border bg-surface shadow-soft outline-none ${className ?? ""}`}
        >
...
function SurgeryRequestModal({
  ...
  open: boolean;
  onOpenChange: (_open: boolean) => void;
  onChange: (_draft: RequestDraft) => void;
  onSave: () => void;
  onDelete?: (_draft: RequestDraft) => void;
  ...
  return (
    <CenterModal
      ...
      title="Edit surgery request"
      description={draft?.mrn}
      className="aspect-[5/4] w-[min(calc(100vw-2rem),860px)]"
    >
      {draft ? (
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <LabeledInput label="Age/Gender">
              <Input
                ...
                onChange={(event) => onChange({ ...draft, ageGender: event.target.value })}
            ...
            <LabeledInput label="MRN">
              <Input
                value={draft.mrn}
                onChange={(event) => onChange({ ...draft, mrn: event.target.value })}
                placeholder="MRN"
              />
            </LabeledInput>
            ...
            <LabeledInput label="Chief surgeon">
              <Input
                value={draft.chiefSurgeon}
                onChange={(event) => onChange({ ...draft, chiefSurgeon: event.target.value })}
                placeholder="Chief surgeon"
              />
            </LabeledInput>
            <LabeledInput label="Anesthetist">
              <Input
                value={draft.anesthetist}
                onChange={(event) => onChange({ ...draft, anesthetist: event.target.value })}
            ...
            <LabeledInput label="Surgery name">
              <Input
                ...
                onChange={(event) => onChange({ ...draft, surgeryName: event.target.value })}
            ...
            <LabeledInput label="Surgery date">
              <Input
                ...
                onChange={(event) => onChange({ ...draft, surgeryDate: event.target.value })}
            ...
            <LabeledInput label="Status">
              <select
                ...
                value={draft.status}
                onChange={(event) =>
                  onChange({ ...draft, status: event.target.value as SurgeryRequestStatus })
                }
              >
          ...
          {onDelete ? (
            <Button className="w-full" variant="danger" onClick={() => onDelete(draft)}>
              Delete request
            </Button>
          ) : null}
        </div>
...
export function SurgeryDashboardPage() {
  ...
  const scheduledRequestIds = React.useMemo(
    () => new Set(bookings.map((booking) => booking.requestId)),
    [bookings],
  );
  const waitingListRequests = requests.filter(
    (request) => request.status !== "Scheduled" && !scheduledRequestIds.has(request.id),
  );
  ...
  const scheduledPatients = requests.filter(
    (request) => request.status === "Scheduled" || scheduledRequestIds.has(request.id),
  ).length;
  ...
  return (
    <SurgeryShell
      ...
      description="OT command center for surgery waiting list, acceptance, scheduling, slot status, and theatre utilization."
    >
...
export function SurgeryWaitingListPage() {
  ...
  const filtered = requests.filter(
    (request) =>
      ...
      filters.every((filterItem) =>
        normalizeFilterValue(String(request[filterItem.criteria])).includes(
          normalizeFilterValue(filterItem.value),
        ),
      ),
  );
  ...
  return (
    <SurgeryShell
      title="Surgery Waiting List"
      description="Requests placed by doctors, ready for filtering, editing, acceptance, and scheduling."
    >
      <Card>
        <CardHeader>
          <div>
            ...
            <CardDescription>
              Add multiple criteria filters for patient, MRN, surgeon, anesthetist, procedure, date,
              or status.
            </CardDescription>
          </div>
          <Button size="sm" onClick={addFilter} disabled={filters.length >= maxWaitingListFilters}>
        ...
        <CardContent className="space-y-2">
          {filters.length === 0 ? (
            <AlertBanner icon={Filter} title="No filters active">
              Add a filter to narrow the waiting list.
            </AlertBanner>
          ) : null}
          {filters.length >= maxWaitingListFilters ? (
            <AlertBanner icon={Filter} title="Filter limit reached">
              Maximum 8 filters can be active at once.
            </AlertBanner>
          ) : null}
          {filters.map((filterItem) => (
            <div className="grid gap-2 md:grid-cols-[220px_minmax(0,1fr)_auto]" key={filterItem.id}>
              <select
                ...
                value={filterItem.criteria}
                onChange={(event) => {
                  setPage(1);
                  setFilters((current) =>
                    current.map((item) =>
                      item.id === filterItem.id
                        ? { ...item, criteria: event.target.value as SurgeryFilter["criteria"] }
                        : item,
                    ),
                  );
                }}
              ...
              <Input
                ...
                onChange={(event) => {
                  ...
                  setFilters((current) =>
                    current.map((item) =>
                      item.id === filterItem.id ? { ...item, value: event.target.value } : item,
                    ),
                  );
                }}
              ...
              <Button
                ...
                onClick={() => {
                  setPage(1);
                  setFilters((current) => current.filter((item) => item.id !== filterItem.id));
                }}
              >
                Remove
              </Button>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{filtered.length} surgery requests</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <thead className="bg-surface-muted text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                <tr>
                  {[
                    ...
                    <th className="border-b border-border px-3 py-2 text-left" key={head}>
                      {head}
                    </th>
                  ))}
                </tr>
                ...
                {paginatedRequests.map((request) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <SurgeryStatus status={request.status} />
                    </td>
                    <td className="px-3 py-2">
                      <DropdownMenu.Root>
                        <DropdownMenu.Trigger asChild>
                          <Button
                            size="icon"
                            variant="ghost"
                            aria-label={`Actions for ${request.patientName}`}
                          >
                        ...
                        <DropdownMenu.Portal>
                          <DropdownMenu.Content
                            align="end"
                            sideOffset={6}
                            className="z-50 min-w-40 rounded-md border border-border bg-surface p-1 shadow-soft"
                          >
                            <DropdownMenu.Item
      ...
      <SurgeryRequestModal
        ...
        onChange={setDraft}
        onSave={saveDraft}
        statusOptions={waitingRequestStatusOptions}
      />
      <CenterModal
        open={Boolean(scheduleRequest)}
        onOpenChange={(open) => !open && setScheduleRequest(null)}
        title="Schedule surgery"
        description={scheduleRequest?.patientName}
        ...
        {scheduleRequest ? (
          <div className="space-y-4">
            ...
            <Button className="w-full" asChild>
              <Link href={`/surgery/schedule?requestId=${scheduleRequest.id}`}>
                Open OT schedule
              </Link>
            </Button>
          </div>
...
export function SurgeryGlobalSearchPage() {
  ...
  return (
    <SurgeryShell
      title="Global Search"
      description="Search surgery requests by patient name, patient ID, chief surgeon, requested by, anesthetist, or surgery name."
    >
      <Card>
        <CardHeader>
          <div>
        ...
        <CardContent className="space-y-4">
          <div className="relative max-w-xl">
            ...
            <Input
              ...
              placeholder="Patient name, Patient ID, chief surgeon, requested by, anesthetist, or surgery name"
            />
          </div>
          <div className="overflow-x-auto rounded-lg border border-border">
            <table className="w-full min-w-[1120px] border-collapse text-sm">
              ...
              <tbody>
                {rows.map((request) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <SurgeryStatus
                        status={scheduledRequestIds.has(request.id) ? "Scheduled" : request.status}
                      />
                    </td>
                  </tr>
                ))}
                {rows.length === 0 ? (
                  <tr>
                    ...
                    >
                      {query.trim()
                        ? "No matching surgery requests found."
                        : "Enter a search term to view matching surgery rows."}
                    </td>
                  </tr>
                ) : null}
              </tbody>
...
export function SurgerySchedulePage() {
  const {
    ...
    setBookings,
    rooms: scheduleRooms,
    setRooms: setScheduleRooms,
    times: scheduleTimes,
    setTimes: setScheduleTimes,
  } = useSurgeryState();
  const searchParams = useSearchParams();
  const requestIdFromUrl = searchParams.get("requestId");
  const selectedRequestFromUrl = requestIdFromUrl
    ? requests.find((request) => request.id === requestIdFromUrl)
    : undefined;
  const defaultRequestId =
    requests.find((request) => request.status !== "Completed")?.id ?? requests[0].id;
  ...
  const [requestSearch, setRequestSearch] = React.useState(() =>
    selectedRequestFromUrl
      ? `${selectedRequestFromUrl.patientName} - ${selectedRequestFromUrl.surgeryName}`
      : "",
  );
  const [requestSearchOpen, setRequestSearchOpen] = React.useState(false);
  const [selectedRequestVisible, setSelectedRequestVisible] = React.useState(
    Boolean(selectedRequestFromUrl),
  );
  ...
  const [contextOtId, setContextOtId] = React.useState(
    selectedBooking?.otId ?? scheduleRooms[0]?.id ?? "",
  );
  const [contextTime, setContextTime] = React.useState(
    selectedBooking?.time || selectedRequest.surgeryTime || scheduleTimes[0],
  );
  const searchResults = requestSearch.trim()
    ? requests.flatMap((request) =>
        matchingSearchFields(request, requestSearch).map((field) => ({ request, field })),
      )
    : [];
  ...
  function scheduleSlot(otId: string, time: string) {
    ...
    setBookings((current) => {
      const existingForRequest = current.find(
        (booking) => booking.requestId === selectedRequest.id,
      );
      if (existingForRequest) {
        return current.map((booking) =>
          booking.id === existingForRequest.id ? { ...booking, otId, time, date } : booking,
        );
      }
      const next: OtBooking = {
        id: createBookingId(selectedRequest.id, otId, time),
    ...
    setRequests((current) =>
      current.map((request) =>
        request.id === selectedRequest.id
          ? { ...request, status: "Scheduled", surgeryDate: date, surgeryTime: time }
          : request,
      ),
    );
    setRequestSearch("");
  ...
  function syncContextDraftForRequest(request: SurgeryRequest) {
    const requestBooking = bookings.find(
      (booking) => booking.requestId === request.id && booking.date === date,
    );
  ...
  function saveSelectedRequestSchedule() {
    ...
    const occupiedBooking = dateBookings.find(
      (booking) =>
        booking.otId === contextOtId &&
        booking.time === contextTime &&
        booking.requestId !== selectedRequest.id,
    );
    if (occupiedBooking) {
      const occupiedRequest = requestForBooking(occupiedBooking);
      setContextScheduleError(
        `This OT/time is already booked${occupiedRequest ? ` for ${occupiedRequest.patientName}` : ""}.`,
      );
      return;
    }
    setContextScheduleError("");
      ...
      if (existingBooking) {
        return current.map((booking) =>
          booking.id === existingBooking.id
            ? { ...booking, otId: contextOtId, time: contextTime, date }
            : booking,
        );
      }
      const next: OtBooking = {
        id: createBookingId(selectedRequest.id, contextOtId, contextTime),
        requestId: selectedRequest.id,
        otId: contextOtId,
        time: contextTime,
        date,
    ...
    setRequests((current) =>
      current.map((request) =>
        request.id === selectedRequest.id
          ? { ...request, status: "Scheduled", surgeryDate: date, surgeryTime: contextTime }
          : request,
      ),
    );
    setRequestSearch("");
  ...
  function updateBookingStatus(bookingId: string, status: SlotStatus) {
    ...
    if (booking) {
      const requestStatus: SurgeryRequestStatus =
        status === "Completed"
          ? "Completed"
          : status === "Stopped"
            ? "Stopped"
      ...
      setRequests((current) =>
        current.map((request) =>
          request.id === booking.requestId ? { ...request, status: requestStatus } : request,
        ),
      );
    }
  }
  ...
  function saveScheduleDraft() {
    ...
    setRequests((current) =>
      current.map((request) => (request.id === scheduleDraft.id ? scheduleDraft : request)),
    );
  ...
  function saveTimeEdit() {
    ...
    const duplicate = scheduleTimes.some(
      (time) =>
        time !== editingTime && normalizeFilterValue(time) === normalizeFilterValue(nextTime),
    );
    if (duplicate) {
    ...
    setBookings((current) =>
      current.map((booking) =>
        booking.time === editingTime ? { ...booking, time: nextTime } : booking,
      ),
    );
    setRequests((current) =>
      current.map((request) =>
        request.surgeryTime === editingTime ? { ...request, surgeryTime: nextTime } : request,
      ),
    );
    setEditingTime(null);
    setTimeDraft("");
  ...
  function addSlot() {
    ...
    if (
      scheduleTimes.some((time) => normalizeFilterValue(time) === normalizeFilterValue(nextTime))
    ) {
  ...
  function addOt() {
    ...
    if (
      scheduleRooms.some(
        (room) => normalizeFilterValue(room.name) === normalizeFilterValue(roomName),
      )
    ) {
    ...
    setScheduleRooms((current) => [
      ...current,
      { id: `ot-local-${otIdRef.current}`, name: roomName, specialty: roomSpecialty || "General" },
    ]);
    otIdRef.current += 1;
    setNewOtName("OT ");
    setNewOtSpecialty("");
  ...
  return (
    <SurgeryShell
      title="OT Schedule"
      description="Schedule all OTs in 15-minute slots and update patient movement status."
    >
      <Card>
        <CardHeader>
          <CardTitle>Patient details</CardTitle>
          <CardDescription>Shown when user schedules against a patient.</CardDescription>
        </CardHeader>
              ...
              {requestSearchOpen && requestSearch.trim() ? (
                <div className="absolute z-40 mt-1 max-h-72 w-full overflow-auto rounded-md border border-border bg-surface shadow-soft">
                  ...
                  {searchResults.length === 0 ? (
                    <div className="px-3 py-2 text-sm text-muted-foreground">No request found</div>
                  ) : null}
                </div>
              ) : null}
            </div>
              ...
              <div className="mb-3 flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {selectedRequest.mrn} • {selectedRequest.service}
                  </div>
                </div>
              ...
              <div className="mt-3 rounded-md border border-border bg-surface p-2 text-xs text-muted-foreground">
                ...
                <div className="mt-2 grid gap-2">
                  ...
                  <div className="flex justify-end">
                    <Button className="min-w-16" size="sm" onClick={saveSelectedRequestSchedule}>
                      OK
                    </Button>
                  </div>
                </div>
                {contextScheduleError ? (
      ...
      <Card className="-mx-4 rounded-none border-x-0 md:-mx-6">
        <CardHeader>
          ...
          <CardDescription>
            Click an empty slot to book. Click a booked slot to view full request details.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full min-w-[1280px] border-collapse text-sm">
              <thead className="bg-surface-muted text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  {scheduleRooms.map((room) => (
                    <th className="border-b border-border px-3 py-2 text-left" key={room.id}>
                      {room.name}
                    </th>
                  ))}
                  ...
                  <tr className="border-b border-border last:border-b-0" key={time}>
                    <td className="px-3 py-2 font-medium">
                      <DropdownMenu.Root>
                        <DropdownMenu.Trigger asChild>
                          <button
                            className="rounded px-2 py-1 text-left font-medium hover:bg-surface-muted focus:outline-none focus:ring-2 focus:ring-ring/20"
                            type="button"
                          >
                            {time}
                          </button>
                        ...
                        <DropdownMenu.Portal>
                          <DropdownMenu.Content
                            align="start"
                            sideOffset={6}
                            className="z-50 min-w-36 rounded-md border border-border bg-surface p-1 shadow-soft"
                          >
                            <DropdownMenu.Item
                              className="cursor-pointer rounded px-2 py-2 text-sm outline-none hover:bg-surface-muted focus:bg-surface-muted"
                    ...
                    {scheduleRooms.map((room) => {
                      ...
                      return (
                        <td className="min-w-[150px] px-2 py-2 align-top" key={room.id}>
                          {booking && request ? (
                            <div
                              className={`space-y-2 rounded-md border p-2 ${bookingColor(booking.status)}`}
                            >
                              <button
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  updateBookingStatus(booking.id, event.target.value as SlotStatus)
                                }
                              >
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {slotStatuses.map((status) => (
            <SurgeryStatus key={status} status={status} />
          ))}
        </CardContent>
      </Card>
      <Drawer
        open={Boolean(slotDetail)}
        onOpenChange={(open) => !open && setSlotDetail(null)}
        title="Booked OT slot"
        description={slotDetail?.time}
      >
        {slotDetail ? (
          <div className="space-y-4">
            <SurgeryStatus status={slotDetail.status} />
            {requestForBooking(slotDetail) ? (
              <RequestSummary request={requestForBooking(slotDetail)!} />
            ) : null}
          </div>
        ) : null}
      </Drawer>
      <SurgeryRequestModal
        ...
        onOpenChange={(open) => !open && setScheduleDraft(null)}
        onChange={setScheduleDraft}
        onSave={saveScheduleDraft}
        onDelete={setDeleteRequestDraft}
      />
      <CenterModal
        open={Boolean(deleteRequestDraft)}
        onOpenChange={(open) => !open && setDeleteRequestDraft(null)}
        title="Delete request"
        description={deleteRequestDraft?.mrn}
      >
        <div className="space-y-4">
          <AlertBanner icon={ClipboardList} title="Warning">
            Deleting this request will remove the patient request and any OT schedule booking linked
            to it.
          </AlertBanner>
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setDeleteRequestDraft(null)}>
              Cancel
            </Button>
      ...
      <CenterModal
        open={Boolean(editingTime)}
        onOpenChange={(open) => !open && setEditingTime(null)}
        title="Edit time row"
        description={editingTime ?? undefined}
      >
        <div className="space-y-3">
          <Input
            value={timeDraft}
            onChange={(event) => setTimeDraft(event.target.value)}
            placeholder="Example: 09:15"
          />
          {timeError ? (
      ...
      <CenterModal
        open={Boolean(deleteTime)}
        onOpenChange={(open) => !open && setDeleteTime(null)}
        title="Delete time row"
        description={deleteTime ?? undefined}
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setDeleteTime(null)}>
              Cancel
            </Button>
            <Button variant="danger" onClick={confirmDeleteTime}>
              Delete time
            </Button>
          </div>
        </div>
      ...
      <CenterModal open={deleteSlotOpen} onOpenChange={setDeleteSlotOpen} title="Delete time slot">
        <div className="space-y-4">
          <select
            className="h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
            value={slotToDelete}
            onChange={(event) => setSlotToDelete(event.target.value)}
          >
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button variant="danger" onClick={deleteSelectedSlot}>
              Delete slot
            </Button>
          </div>
        </div>
      ...
      <CenterModal open={addOtOpen} onOpenChange={setAddOtOpen} title="Add OT">
        <div className="space-y-3">
          <Input
            value={newOtName}
            onChange={(event) => setNewOtName(event.target.value)}
            placeholder="Example: OT 7"
          />
          <Input
            value={newOtSpecialty}
            onChange={(event) => setNewOtSpecialty(event.target.value)}
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setAddOtOpen(false)}>
              Cancel
            </Button>
      ...
      <CenterModal open={deleteOtOpen} onOpenChange={setDeleteOtOpen} title="Delete OT">
        <div className="space-y-4">
          <select
            className="h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
            value={otToDelete}
            onChange={(event) => setOtToDelete(event.target.value)}
          >
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setDeleteOtOpen(false)}>
              Cancel
            </Button>
            <Button variant="danger" onClick={deleteSelectedOt}>
              Delete OT
            </Button>
          </div>
        </div>
