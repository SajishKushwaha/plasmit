...
import { PatientAlertChips, PatientStatusBadge } from "@/features/clinical/patients/patient-shared";
...
export const diagnosticsReadOnlyRoles: Role[] = [
  "Hospital Admin",
  "Doctor",
  "Doctor OPD",
  "Doctor IPD",
    ...
    <div className="space-y-4">
      {access.readOnly ? (
        <AlertBanner icon={LockKeyhole} tone="warning" title="Read-only diagnostics access">
          {access.role} can review diagnostic status, approved reports, and safety states in this
          static preview, but collection, approval, correction, and integration actions are
          disabled.
        </AlertBanner>
      ) : null}
...
export function DiagnosticSafetyBanner({
  type = "diagnostics",
}: {
  type?: "diagnostics" | "lab" | "radiology" | "viewer";
}) {
  const text =
    type === "viewer"
  ...
  return (
    <AlertBanner icon={ShieldAlert} tone="warning" title="Diagnostic safety">
      {text}
    </AlertBanner>
  );
}
...
export function CriticalBanner({ children }: { children: React.ReactNode }) {
  return (
    <AlertBanner icon={AlertTriangle} tone="critical" title="Critical result / finding">
      {children}
    </AlertBanner>
  );
}
...
export function DiagnosticOrderHeader({
  ...
  const sample = labOrder
    ? mockSampleCollections.find((item) => item.orderId === labOrder.id)
    : undefined;
  ...
  if (!order || !patient) {
    return (
      <EmptyState
        icon={UserRound}
        title="Order context not found"
        description="The selected static diagnostic order is not available."
      />
    );
  }
    ...
    <Card className="sticky top-[132px] z-20">
      <CardContent className="flex flex-col gap-3 p-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-2 xl:min-w-[520px]">
          ...
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Source: {order.source} • {order.priority}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="outline" asChild>
            <Link href={`/emr/patients/${patient.id}/timeline`}>EMR timeline</Link>
          </Button>
          {sample ? (
...
export function BarcodePreview({
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      ...
      <div
        className="flex h-16 items-end justify-center gap-1 rounded-md bg-surface-muted p-3"
      ...
      >
        {Array.from({ length: 26 }).map((_, index) => (
          <span
            ...
            style={{ height: `${18 + (index % 5) * 7}px`, width: index % 4 === 0 ? "3px" : "2px" }}
          />
        ))}
      </div>
