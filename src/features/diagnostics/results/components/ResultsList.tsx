  ...
  return mockLabResults.map((result) => {
    ...
    return {
      ...
      summary: result.parameters
        .map((item) => `${item.parameter}: ${item.value} ${item.unit}`)
        .join(" | "),
      owner: result.approvedBy === "Pending" ? result.enteredBy : result.approvedBy,
      updatedAt:
        result.approvedAt === "Pending" ? (order?.orderedAt ?? "Pending") : result.approvedAt,
...
function getRows() {
  return [...buildLabRows(), ...buildRadiologyRows()].sort(
...
export function ResultsList() {
  ...
  const filteredRows = rows.filter((row) => {
    ...
    return (
      ...
      (category === "All result" || row.category === category) &&
      (status === "All status" || row.status === status)
    );
  });
  const labPending = mockLabOrders.filter((item) =>
    ["Not entered", "Draft", "Approval pending"].includes(item.resultStatus),
  ).length;
  const radiologyPending = mockRadiologyOrders.filter((item) =>
    ["Not entered", "Report draft", "Approval pending"].includes(item.reportStatus),
  ).length;
  const columns = React.useMemo<ColumnDef<DoctorResultRow>[]>(
    () => [
      ...
      { header: "IPD / bed", accessorKey: "bedWard" },
      {
        ...
        cell: ({ row }) => (
          <div>
            <div className="font-semibold">{row.original.testName}</div>
            <div className="text-xs text-muted-foreground">
              {row.original.category} | {row.original.orderNo}
            </div>
          </div>
        ),
      },
      ...
      {
        header: "Summary",
        cell: ({ row }) => <span className="line-clamp-2 text-xs">{row.original.summary}</span>,
      },
        ...
        <>
          <PageHeader
            ...
            actions={
              <>
                ...
                <Button variant="outline" onClick={() => toast.info("Result list refreshed")}>
          ...
          <SummaryGrid>
            <StatCard
              label="Total results"
              value={rows.length}
              change="Lab + Radiology"
              context="Current mock worklist"
              tone="info"
              icon={FileText}
            />
            <StatCard
              label="Critical"
              value={rows.filter((row) => row.critical).length}
              change="Review now"
              context="Visible flag"
              tone="critical"
              icon={AlertTriangle}
            />
            <StatCard
          ...
          <Tabs defaultValue="list" className="space-y-4">
            ...
            <TabsContent value="list" className="space-y-4">
              <FilterBar
                search={search}
                onSearch={setSearch}
                placeholder="Search patient, UHID, bed, order, test, result..."
              >
                <NativeSelect
                  label="Type"
                  value={category}
            ...
            <TabsContent value="critical" className="space-y-4">
              {rows.some((row) => row.critical) ? (
                        ...
                        <CardContent className="space-y-3 p-4">
                          <div className="flex flex-wrap items-start justify-between gap-2">
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                {row.uhid} | {row.bedWard}
                              </div>
                            </div>
                            <Badge tone="critical">{row.category}</Badge>
                          </div>
                          <CriticalBanner>
                            {row.testName}: {row.summary}
                          </CriticalBanner>
                ...
                <EmptyState
                  ...
                  description="No current result row is marked critical."
                />
              )}
            </TabsContent>
          </Tabs>
...
function ResultDetailDrawer({
  ...
  result: DoctorResultRow | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}) {
  const labResult =
    result?.category === "Lab"
      ? mockLabResults.find((item) => item.id === result.rawId)
      : undefined;
  const radiologyReport =
    result?.category === "Radiology"
      ? mockRadiologyReports.find((item) => item.id === result.rawId)
      : undefined;
    ...
    >
      {result ? (
        <div className="space-y-4">
          {result.critical ? (
            <CriticalBanner>
              Critical result requires doctor review and acknowledgement before clinical closure.
            </CriticalBanner>
          ) : null}
          <Card>
