...
type ReportModalState =
  ...
  | {
      ...
      records: ResultRecord[];
      comparisonRecords?: ResultRecord[];
    }
  | null;
...
function latestDateRecords(records: ResultRecord[]) {
  const latestDateKey = [...records]
    .map(getResultDateKey)
    .sort((first, second) => second.localeCompare(first))[0];
  return latestDateKey
    ? records.filter((result) => getResultDateKey(result) === latestDateKey)
    : records;
}
...
function buildGroups(records: ResultRecord[]) {
  ...
  return Object.entries(grouped)
    ...
    .map(([dateKey, items]) => ({
      ...
      records: [...items].sort(
        (first, second) =>
          new Date(second.orderedAt).getTime() - new Date(first.orderedAt).getTime(),
      ),
    }));
}

function _groupTotal(groups: ReportGroup[], department?: ResultDepartment) {
  return groups.reduce(
    (total, group) =>
      total +
      (department
        ? group.records.filter((result) => result.department === department).length
        : group.records.length),
    0,
  );
}

function _getReportName(result: ResultRecord) {
...
function _getSampleId(result: ResultRecord) {
...
function buildAdditionalPatientReports(records: ResultRecord[], seed = 0) {
  ...
  const reportSuffix =
    (anchor.mrn || anchor.patientName || `patient-${seed}`)
      .replace(/[^a-zA-Z0-9]/g, "")
      .slice(-8) || String(seed);
  ...
  const radiologyPair = [
    [
      "CT Brain Plain",
      "No acute intracranial hemorrhage. Mild age-related cortical atrophy.",
      "ACC-CT",
    ],
  ...
  return [
      ...
      values: [
        {
          ...
          unit: "%",
          range: "< 5.7",
          flag: Number(hba1c) > 6.4 ? ("Critical" as const) : ("High" as const),
        },
        {
          name: "Estimated Average Glucose",
          value: estimatedAverageGlucose,
          unit: "mg/dL",
          range: "70 - 140",
          flag: Number(estimatedAverageGlucose) > 140 ? ("High" as const) : ("Normal" as const),
        },
      ],
      timeline: [
    ...
    },
    {
      ...
      values: [
        {
          name: "Hemoglobin",
          value: hemoglobinBaseline,
          unit: "g/dL",
          range: "13.0 - 17.0",
          flag: Number(hemoglobinBaseline) < 13 ? ("Low" as const) : ("Normal" as const),
        },
        {
          name: "WBC Count",
          value: wbcBaseline,
          unit: "10^3/uL",
          range: "4.0 - 11.0",
          flag: Number(wbcBaseline) > 11 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "Platelets",
          value: plateletBaseline,
          unit: "10^3/uL",
          range: "150 - 450",
    ...
    },
    {
      ...
      values: [
        {
          name: "Hemoglobin",
          value: hemoglobinPrevious,
          unit: "g/dL",
          range: "13.0 - 17.0",
          flag: Number(hemoglobinPrevious) < 13 ? ("Low" as const) : ("Normal" as const),
        },
        {
          name: "WBC Count",
          value: wbcPrevious,
          unit: "10^3/uL",
          range: "4.0 - 11.0",
          flag: Number(wbcPrevious) > 11 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "Platelets",
          value: plateletPrevious,
          unit: "10^3/uL",
          range: "150 - 450",
          flag: "Normal" as const,
        },
    ...
    },
    {
      ...
      values: [
        {
          name: "Creatinine",
          value: creatinineBaseline,
          unit: "mg/dL",
          range: "0.7 - 1.3",
          flag: Number(creatinineBaseline) > 1.3 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "BUN",
          value: bunBaseline,
          unit: "mg/dL",
          range: "7 - 20",
          flag: Number(bunBaseline) > 20 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "Sodium",
          value: "136",
    ...
    },
    {
      ...
      values: [
        {
          name: "Creatinine",
          value: creatinineCurrent,
          unit: "mg/dL",
          range: "0.7 - 1.3",
          flag: Number(creatinineCurrent) > 1.3 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "BUN",
          value: bunCurrent,
          unit: "mg/dL",
          range: "7 - 20",
          flag: Number(bunCurrent) > 20 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "Sodium",
          value: "138",
      ...
      values: [
        {
          ...
          unit: "U/L",
          range: "7 - 56",
          flag: Number(sgptBaseline) > 56 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "SGOT",
          value: sgotBaseline,
          unit: "U/L",
          range: "8 - 40",
          flag: Number(sgotBaseline) > 40 ? ("High" as const) : ("Normal" as const),
        },
        {
      ...
      values: [
        {
          ...
          unit: "U/L",
          range: "7 - 56",
          flag: Number(sgptCurrent) > 56 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "SGOT",
          value: sgotCurrent,
          unit: "U/L",
          range: "8 - 40",
          flag: Number(sgotCurrent) > 40 ? ("High" as const) : ("Normal" as const),
        },
        {
          name: "Bilirubin Total",
          value: "0.8",
    ...
    },
    {
      ...
      resultSummary:
        seed % 2 === 0
          ? "Mild fatty liver changes. No free fluid detected."
          : "Follow-up portable chest film reviewed. Lines and tubes position satisfactory.",
      values: [
        { name: "Image status", value: "Available", flag: "Normal" as const },
    ...
    },
    {
      ...
      values: [
        ...
        {
          name: "Lactate",
          value: (1.2 + (seed % 12) / 10).toFixed(1),
          unit: "mmol/L",
          range: "0.5 - 2.2",
          flag: seed % 12 > 9 ? ("High" as const) : ("Normal" as const),
        },
...
function scopedPatientRecords(patientContext?: ResultsPatientContext) {
  ...
  const fallbackRecords = Array.from(
    { length: Math.min(6, resultRecords.length) },
    (_, index) => resultRecords[(fallbackStart + index) % resultRecords.length],
  );
...
function DateRangeCalendar({
  ...
  return (
    <div
      className="absolute right-0 top-full z-50 mt-2 w-[min(92vw,360px)] rounded-lg border border-border bg-white p-3 shadow-soft"
      ref={wrapperRef}
    >
      ...
      <div className="mt-3 flex items-center justify-between gap-3 border-t border-border pt-3">
        <span className="text-xs text-muted-foreground">
          {startDate && !endDate ? "Select end date" : "Select start date, then end date"}
        </span>
        <Button
          onClick={() => {
            onChange("", "");
...
export function ResultsWorkflowView({
  ...
  patientContext,
  showPoctTab = true,
  viewTitle: _viewTitle = "Results Center",
  viewDescription:
    _viewDescription = "Laboratory, radiology, and POCT reports organized for IPD review.",
  ...
  onAddLaboratoryOrder?: (_patientContext?: DoctorOrdersPatientContext) => void;
}) {
  const [activeDepartment, setActiveDepartment] = useState<DepartmentFilter>(
    criticalOnly ? "all" : defaultDepartment,
  );
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((result) => {
      const matchesDepartment =
        activeDepartment === "all" || result.department === activeDepartment;
      const matchesCritical = !criticalOnly || result.status === "Critical";
  ...
  const counts = useMemo(
    () => ({
      all: records.filter(
        (result) =>
          (!criticalOnly || result.status === "Critical") &&
          matchesDateFilter(result, selectedDate, rangeStart, rangeEnd),
      ).length,
      laboratory: records.filter(
        (result) =>
          result.department === "laboratory" &&
          (!criticalOnly || result.status === "Critical") &&
          matchesDateFilter(result, selectedDate, rangeStart, rangeEnd),
      ).length,
      radiology: records.filter(
        (result) =>
          result.department === "radiology" &&
          (!criticalOnly || result.status === "Critical") &&
          matchesDateFilter(result, selectedDate, rangeStart, rangeEnd),
      ).length,
      poct: records.filter(
        (result) =>
          result.department === "poct" &&
          (!criticalOnly || result.status === "Critical") &&
          matchesDateFilter(result, selectedDate, rangeStart, rangeEnd),
      ).length,
    }),
  ...
  useEffect(() => {
    ...
    const departmentRecords = records.filter(
      (result) =>
        result.department === autoOpenAllDepartment &&
        (!criticalOnly || result.status === "Critical"),
    );
    const latestDateKey = departmentRecords
      ...
      .sort((first, second) => second.localeCompare(first))[0];
    const recordsToView =
      autoOpenLatestDateOnly && latestDateKey
    ...
    setReportModal({
      type: "all",
      department: autoOpenAllDepartment,
  ...
  function printReports(recordsToPrint: ResultRecord[], title: string) {
    ...
    const printRecords =
      recordsToPrint[0]?.department === "laboratory" && recordsToPrint.length > 1
              ...
              <div className="relative w-full min-w-0" ref={dateFilterRef}>
                ...
                {dateMenuOpen ? (
                  <div className="absolute right-0 top-full z-40 mt-2 w-full min-w-[230px] overflow-hidden rounded-lg border border-border bg-white p-1 shadow-soft">
                    <button
                      className={cn(
                        "flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm font-semibold transition hover:bg-surface-muted",
                        selectedDate === "all" && "bg-primary-soft text-primary",
                      )}
                      onClick={() => {
                        setSelectedDate("all");
                    ...
                    <button
                      className={cn(
                        "flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm font-semibold transition hover:bg-surface-muted",
                        selectedDate === "custom" && "bg-primary-soft text-primary",
                      )}
                      onClick={() => {
                    ...
                    {availableDates.map((date) => (
                      <button
                        className={cn(
                          "flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm font-semibold transition hover:bg-surface-muted",
                          selectedDate === date.dateKey && "bg-primary-soft text-primary",
                        )}
                        key={date.dateKey}
                        onClick={() => {
      ...
      {groups.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Try a different report name, report ID, date, or tab.
            </p>
            {query ? (
        ...
        <div className="space-y-4">
          {groups.map((group, index) => {
            ...
            return (
              <section
                className="overflow-hidden rounded-xl border border-border bg-white shadow-sm transition-all duration-200"
                key={group.dateKey}
              >
                <div className="flex flex-col gap-3 border-b border-border bg-surface-muted/50 px-4 py-3 transition hover:bg-surface-muted sm:flex-row sm:items-center">
                  <button
                    ...
                    onClick={() =>
                      setOpenGroups((current) => ({ ...current, [group.dateKey]: !expanded }))
                    }
                    type="button"
                  >
                    <div className="text-base font-bold text-foreground">{group.label}</div>
                    <div className="text-xs text-muted-foreground">
                      {group.records.length} report(s) in this date group
                    </div>
                  </button>
                  <label className="relative w-full min-w-0 sm:w-[min(440px,46%)]">
                    ...
                    <Input
                      ...
                      placeholder="Search report name, report ID..."
                    />
                  </label>
                  <button
                ...
                {expanded ? (
                  <div className="space-y-4 p-4 transition-all duration-200">
                    {(activeDepartment === "all"
                      ? departmentCards
                      : departmentCards.filter((card) => card.id === activeDepartment)
                    ).map((card) => {
                      const reports = group.records.filter(
                        (result) => result.department === card.id,
                      );
                      ...
                      return (
                        <ResultCategorySection
                          ...
                          onAllView={(department, recordsToView) => {
                            const selectedReportNames = new Set(
                              recordsToView.map((result) => result.testName),
                            );
                            const comparisonRecords = records.filter((result) => {
                              const matchesDepartment = result.department === department;
                              const matchesSelectedReport = selectedReportNames.has(
                                result.testName,
                              );
                              const matchesCritical = !criticalOnly || result.status === "Critical";
                              return (
                          ...
                          onPrint={(result) =>
                            printReports(
                              [result],
                              `${getDepartmentLabel(result.department)} Report`,
                            )
                          }
...
function ResultCategorySection({
  ...
  onAddLaboratoryOrder?: (_patientContext?: DoctorOrdersPatientContext) => void;
  onAllView: (_department: ResultDepartment, _records: ResultRecord[]) => void;
  onPrint: (_result: ResultRecord) => void;
  onView: (_result: ResultRecord) => void;
  ...
  const headerActions = (
    <>
      ...
      <Button
        disabled={!department || allReports.length === 0}
        size="sm"
        variant="outline"
        onClick={() => department && onAllView(department, allReports)}
      >
        <Icon className="h-4 w-4" />
        All View
      </Button>
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody>
          {reports.map((result) => (
            <tr
              className="border-t border-border transition-colors hover:bg-primary-soft/40"
              key={result.id}
            >
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-2">
                  ...
                  <Button
                    ...
                    aria-label={`Print ${result.testName}`}
                  >
...
function ReportViewModal({
  ...
  onAddLaboratoryOrder?: (_patientContext?: DoctorOrdersPatientContext) => void;
  onClose: () => void;
  onPrint: (_records: ResultRecord[], _title: string) => void;
  patientContext?: ResultsPatientContext;
  payload: ReportModalState;
}) {
  const title =
    payload?.type === "single"
      ? payload.result.testName
      : payload
        ? `All ${getDepartmentLabel(payload.department)} Reports`
        : "";
  const patientName =
    patientContext?.name ??
    (payload?.type === "single" ? payload.result.patientName : payload?.records[0]?.patientName);
  const reportScope =
    payload?.type === "single"
  ...
  const orderPatientContext = showLaboratoryOrderAction
    ? toOrderPatientContext(payload.records[0])
    : undefined;
            ...
            <>
              <div className="flex items-start justify-between gap-3 border-b border-border p-4">
                <div>
                  ...
                  <Dialog.Description className="text-sm font-semibold text-foreground">
                    {description}
                  </Dialog.Description>
                </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const recordsToPrint =
                        payload.type === "single" ? [payload.result] : payload.records;
                      onPrint(
                        recordsToPrint,
                        payload.type === "single"
                          ? `${getDepartmentLabel(payload.result.department)} Report`
                          : title,
                      );
                    }}
                  >
              ...
              <div className="max-h-[calc(88dvh-82px)] overflow-y-auto p-4">
                {payload.type === "single" ? (
                  <SingleReportView result={payload.result} />
                ) : (
                  <AllCategoryView
                    comparisonRecords={payload.comparisonRecords}
              ...
              {showLaboratoryOrderAction ? (
                <CenterModal
                  ...
                  description={
                    payload.records[0]
                      ? `${payload.records[0].patientName} | ${payload.records[0].location}`
                      : undefined
                  }
...
function AllCategoryView({
  ...
  if (records.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
        No {getDepartmentLabel(department).toLowerCase()} reports available.
      </div>
    );
  }
  ...
  if (department === "laboratory") {
    return (
      <LaboratoryAllViewTabs comparisonRecords={comparisonRecords ?? records} records={records} />
    );
  }
...
function LaboratoryAllViewTabs({
  comparisonRecords,
  records,
}: {
  comparisonRecords: ResultRecord[];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 border-b border-border pb-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="text-xs font-semibold text-muted-foreground">
          {activeTab === "details"
            ? `${latestRecords.length} latest report(s)`
            : `${Array.from(new Set(comparisonRecords.map((result) => getDateKey(result.completedAt ?? result.orderedAt)))).slice(-3).length} date comparison`}
        </div>
      </div>

      {activeTab === "details" ? <LaboratoryAllDetailsView records={latestRecords} /> : null}
      {activeTab === "comparison" ? (
        <LaboratoryDateWiseComparison records={comparisonRecords} />
      ) : null}
    </div>
  );
}
...
function LaboratoryAllDetailsView({ records }: { records: ResultRecord[] }) {
  const sortedRecords = [...records].sort(
    (first, second) =>
      new Date(second.completedAt ?? second.orderedAt).getTime() -
      new Date(first.completedAt ?? first.orderedAt).getTime(),
  );
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-white">
      <div className="flex flex-col gap-1 border-b border-border bg-surface-muted px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="text-xs font-semibold text-muted-foreground">
          {sortedRecords[0]
            ? `${formatDate(sortedRecords[0].completedAt ?? sortedRecords[0].orderedAt)} | ${records.length} report(s)`
            : "No reports"}
        </div>
      </div>
      <div className="overflow-x-auto">
              ...
              result.values.map((value, index) => (
                ...
                >
                  {index === 0 ? (
                    <>
                      <td
                        className="border-r border-border px-3 py-2 align-top"
                        rowSpan={result.values.length}
                      >
                  ...
                  <td
                    className={cn(
                      ...
                      resultValueClass(value),
                    )}
                  >
                    {value.value}
                    {value.unit ? (
                      <span className="ml-1 font-medium text-muted-foreground">{value.unit}</span>
                    ) : null}
                  </td>
...
function LaboratoryDateWiseComparison({ records }: { records: ResultRecord[] }) {
  const chronologicalDateKeys = Array.from(
    new Set(records.map((result) => getDateKey(result.completedAt ?? result.orderedAt))),
  )
    .sort()
    .slice(-3);
  ...
  const criticalCount = sections.reduce(
    (total, section) =>
      total +
      section.rows.filter(
        (row) => row.trend === "Elevated" || row.trend === "Critical" || row.trend === "Watch",
      ).length,
    0,
  );
  const improvingCount = sections.reduce(
    (total, section) =>
      total +
      section.rows.filter((row) => row.trend === "Recovering" || row.trend === "Stabilizing")
        .length,
    0,
  );
    ...
    <div className="space-y-3">
          ...
          <table className="w-full min-w-[960px] border-collapse text-left text-xs">
            <thead>
              <tr className="bg-surface-muted/80 uppercase tracking-wide text-muted-foreground">
                <th className="w-[280px] border-b border-r border-border px-3 py-2 font-bold">
                  Laboratory Parameter
                </th>
                <th className="w-[190px] border-b border-r border-border px-3 py-2 font-bold">
                  Normal Range
                </th>
                {dateKeys.map((dateKey) => (
                  <th
                    className="min-w-[150px] border-b border-r border-border px-3 py-2 text-center"
                    key={dateKey}
                  >
                    <span className="block text-xs font-extrabold text-foreground">
                      {formatComparisonDate(dateKey)}
                    </span>
                    <span className="mt-0.5 block text-[11px] font-semibold italic normal-case text-muted-foreground">
                      {comparisonDateSubtitle(dateKey, dateKeys)}
                    </span>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {sections.map((section) => (
                <RowsForLaboratoryComparisonSection
                  dateKeys={dateKeys}
                  key={section.name}
                  section={section}
                />
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <ComparisonSummaryCard
          ...
          value={`+${Math.max(0, improvingCount * 6)}% Overall`}
          tone="info"
        />
        <ComparisonSummaryCard
          label="Last Update"
    ...
    <>
      ...
      {section.rows.map((row) => (
        <tr
          ...
          key={row.key}
        >
          <td className="whitespace-nowrap border-r border-border px-3 py-2 font-extrabold text-foreground">
            {row.parameter}
          </td>
          <td className="border-r border-border px-3 py-2 font-mono font-bold text-muted-foreground">
            {row.range}
          </td>
          {dateKeys.map((dateKey) => {
            ...
            return (
              ...
              >
                {value ? (
                  <span
                    className={cn(
                      "inline-flex items-center justify-center gap-1 text-xs font-extrabold",
                      comparisonValueClass(value),
                    )}
                  >
                    {value.value}
...
function buildLaboratoryComparisonSections(
  records: ResultRecord[],
  displayDateKeys: string[],
  chronologicalDateKeys: string[],
) {
  return Object.entries(groupRecordsByTestName(records))
    .sort(([first], [second]) => first.localeCompare(second))
    .map(([testName, items]) => {
      const valueNames = Array.from(
        new Set(items.flatMap((result) => result.values.map((value) => value.name))),
      );
      const rows = valueNames.map((valueName) => {
        const valuesByDate = displayDateKeys.reduce<Record<string, ResultValue | undefined>>(
          (values, dateKey) => {
        ...
        const firstValue = items
          ...
          .find((value) => value.name === valueName);
        const arrowByDate = displayDateKeys.reduce<
          Record<string, "up" | "down" | "same" | undefined>
        >((arrows, dateKey, index) => {
          const current = numericResultValue(valuesByDate[dateKey]);
          const previous = numericResultValue(
            index < displayDateKeys.length - 1
              ? valuesByDate[displayDateKeys[index + 1]]
              : undefined,
          );
          arrows[dateKey] =
...
function comparisonTrendLabel(values: Array<ResultValue | undefined>) {
  ...
  if (latest?.flag === "High") {
    return firstNumeric !== null && latestNumeric !== null && latestNumeric < firstNumeric
      ? "Recovering"
      : "Elevated";
  }
  ...
  if (
    firstNumeric !== null &&
    latestNumeric !== null &&
    Math.abs(latestNumeric - firstNumeric) > 0.1
  )
    return "Stabilizing";
  return "Normal";
}
  ...
  return (
    <InfoGrid
      rows={[
        ...
        [
          "Device Used",
          result.values.find((value) => value.name.toLowerCase().includes("device"))?.value ??
            "POCT bedside device",
        ],
        [
          "Performed By",
          result.timeline.find((item) => item.label.toLowerCase().includes("entered"))?.by ??
            result.timeline[0]?.by ??
            "Clinical staff",
        ],
...
function ResultValueTable({ result, title }: { result: ResultRecord; title: string }) {
  return (
    <div className="overflow-hidden rounded-lg border border-border">
      <div className="border-b border-border bg-surface-muted px-3 py-2 text-sm font-semibold text-foreground">
        {title}
      </div>
...
function ReportPrintView({
  ...
  const ageGender = firstResult
    ? getPatientAgeGender(firstResult, patientContext)
    : { age: "-", gender: "-" };
  const _printedAt = new Date().toLocaleString("en-IN");
  const allLaboratoryPrint =
    firstResult?.department === "laboratory" && (payload?.records.length ?? 0) > 1;
  const _signature =
            ...
            <div className="result-print-root">
              ...
              <section className="result-print-patient-box">
                ...
                <div className="result-print-patient-grid">
                  <div>
                    <PrintLineField
                      label="Patient Name:"
                      value={patientContext?.name ?? firstResult.patientName}
                    />
                    <PrintLineField label="MRN:" value={patientContext?.mrn ?? firstResult.mrn} />
                    <PrintLineField
                      label="Age / Gender:"
                      value={`${ageGender.age} / ${ageGender.gender}`}
                    />
                    <PrintLineField
                      label="Doctor Name:"
                      value={patientContext?.consultantDoctor ?? firstResult.orderingDoctor}
                    />
                  </div>
                  <div>
                    <PrintLineField
                      label="Sample Collection:"
                      value={formatDateTime(firstResult.collectedAt)}
                    />
                    <PrintLineField
                      label="Report Date:"
                      value={formatDate(firstResult.completedAt ?? firstResult.orderedAt)}
                    />
                    <PrintLineField label="Report Status:" value="Final" />
                    <PrintLineField
                      label="Report ID:"
                      value={payload.records.map((result) => result.id).join(", ")}
                    />
                  </div>
                </div>
              </section>
              ...
              <section className="mt-5">
                <div className="mb-2 text-sm font-bold uppercase tracking-wide text-primary">
                  {allLaboratoryPrint
                    ? "Latest Date Results"
                    : `${getDepartmentLabel(firstResult.department)} Details`}
                </div>
                {allLaboratoryPrint ? (
                  ...
                  payload.records.map((result) => (
                    <div className="mb-4 break-inside-avoid" key={`print-${result.id}`}>
                      <div className="mb-2 grid grid-cols-4 gap-2 text-xs">
                        ...
                        <PrintField
                          label="Report Date"
                          value={formatDateTime(result.completedAt ?? result.orderedAt)}
                        />
                      </div>
                      <table>
                        <thead>
              ...
              <div className="result-print-footer">
                ...
                <div className="result-print-footer-main">
                  ...
                  <div className="result-print-footer-row">
                    ...
                    <span>
                      PlasmIT Pty Ltd, Level 17, Tower 4, 727 Collins Street, Docklands, Victoria -
                      3008 Australia
                    </span>
                  </div>
                  ...
                  <div className="result-print-disclaimer">
                    All Lab results are subject to clinical interpretation by qualified medical
                    professional and this report is not subject to use for any medico-legal purpose.
                  </div>
                </div>
...
function PrintQrCode() {
  return (
    <img alt="QR verification code" className="result-print-qr" src="/laboratory-report-qr.png" />
  );
}
