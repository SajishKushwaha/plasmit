...
import { icuPatients, icuTasks } from "@/features/nursing-icu/nursing-icu-data";

function SummaryCard({ label, value, tone }: { label: string; value: React.ReactNode; tone: "info" | "warning" | "danger" | "success" | "critical" }) {
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_14px_32px_rgba(15,23,42,0.05)]">
      ...
      <label className="flex w-full cursor-pointer items-center justify-between gap-3 px-4 py-3 text-left transition duration-150 hover:bg-sky-50 peer-checked:[&_svg]:rotate-180" htmlFor={panelId}>
        ...
        <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-sky-600 shadow-sm">
          <svg className="h-4 w-4 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <path d="m6 9 6 6 6-6" />
          </svg>
        </span>
      </label>
...
function toneForStatus(status: string) {
  ...
  if (lower.includes("available") || lower.includes("healthy") || lower.includes("ready") || lower.includes("balanced")) return "success";
  if (lower.includes("busy") || lower.includes("limited") || lower.includes("low")) return "warning";
...
function statusPillTone(status: string) {
  ...
  return tone === "success" ? "success" : tone === "warning" ? "warning" : tone === "danger" ? "critical" : "info";
}
...
function computeUnitRows() {
  ...
  return units.map((unit) => {
    ...
    const totalBeds = Math.max(occupiedBeds + (unit.includes("General") ? 4 : unit.includes("Medical") ? 3 : 2), occupiedBeds + 1);
    ...
    const status = occupancy >= 90 ? "Over Capacity" : occupancy >= 75 ? "Critical" : occupancy >= 60 ? "Busy" : "Healthy";
    return { unit, totalBeds, occupiedBeds, availableBeds, isolationBeds, ventilatorBeds, occupancy, status };
  });
}
...
function computeNurseRows() {
  ...
  icuPatients.forEach((patient) => {
    nurseCounts.set(patient.assignedWardNurse, (nurseCounts.get(patient.assignedWardNurse) ?? 0) + 1);
  });
  ...
  return nurseNames.map((name, index) => {
    ...
    const icuExperience = index % 4 === 0 ? "5+ years" : index % 3 === 0 ? "3-5 years" : "1-3 years";
    const availability = assignedPatients === 0 ? "Available" : assignedPatients < maxCapacity ? "Balanced" : assignedPatients === maxCapacity ? "Busy" : "Overloaded";
    const workload = `${assignedPatients}/${maxCapacity}`;
    return { name, employeeId: `EMP-${2400 + index}`, shift, assignedPatients, maxCapacity, icuExperience, availability, workload };
  });
}

function computeEquipmentRows() {
  const runningTasks = icuTasks.filter((task) => task.status === "In progress" || task.status === "Assigned").length;
  return [
    ...
    { equipment: "Oxygen Cylinder", totalUnits: 20, inUse: 13 + Math.min(2, runningTasks), available: 20 - (13 + Math.min(2, runningTasks)) },
  ].map((row) => {
...
function FilterRow({ search, setSearch, unit, setUnit, shift, setShift, status, setStatus, onReset, unitOptions }: { search: string; setSearch: (value: string) => void; unit: string; setUnit: (value: string) => void; shift: string; setShift: (value: string) => void; status: string; setStatus: (value: string) => void; onReset: () => void; unitOptions: string[] }) {
  return (
    <div className="grid gap-3 bg-slate-50 px-3 py-3 lg:grid-cols-[minmax(260px,1fr)_180px_180px_180px_110px] lg:items-end">
      <label className="space-y-1 text-sm">
        <span className="block text-xs font-semibold text-slate-700">Search ICU unit / nurse / equipment</span>
        <div className="relative">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <Input className="h-10 border-slate-300 bg-white pl-9 text-sm" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search ICU unit / nurse / equipment" />
        </div>
      </label>
      <NativeSelect label="ICU Unit" value={unit} onChange={setUnit} options={unitOptions} />
      <NativeSelect label="Shift" value={shift} onChange={setShift} options={["All shifts", "Day", "Night"]} />
      <NativeSelect label="Status" value={status} onChange={setStatus} options={["All status", "Available", "Healthy", "Busy", "Critical", "Limited", "Low Stock", "Over Capacity"]} />
      <Button className="h-10" variant="outline" onClick={onReset}>Reset</Button>
    </div>
  );
}
...
export function UnitStaffAvailabilityPage() {
  ...
  const unitOptions = React.useMemo(() => ["All units", ...unitRows.map((row) => row.unit)], [unitRows]);
  ...
  const summary = React.useMemo(() => ({
    ...
    unitNursesAvailable: nurseRows.filter((row) => row.availability === "Available" || row.availability === "Balanced").length,
    wardNursesAvailable: Math.max(icuPatients.length - nurseRows.reduce((sum, row) => sum + row.assignedPatients, 0), 0) + 12,
    isolationBeds: unitRows.reduce((sum, row) => sum + row.isolationBeds, 0),
    nurseRatio: unitRows.length ? `1:${Math.max(2, Math.round(unitRows.reduce((sum, row) => sum + row.occupiedBeds, 0) / unitRows.length))}` : "1:2",
    workload: nurseRows.some((row) => row.availability === "Overloaded") ? "Heavy" : nurseRows.some((row) => row.availability === "Busy") ? "Moderate" : "Balanced",
  }), [nurseRows, unitRows]);

  const query = search.trim().toLowerCase();
  const filteredUnitRows = unitRows.filter((row) => (!query || row.unit.toLowerCase().includes(query)) && (unit === "All units" || row.unit === unit) && (status === "All status" || row.status === status || (status === "Available" && row.status === "Healthy")));
  const filteredNurseRows = nurseRows.filter((row) => (!query || `${row.name} ${row.employeeId} ${row.shift} ${row.availability} ${row.workload}`.toLowerCase().includes(query)) && (shift === "All shifts" || row.shift === shift) && (status === "All status" || row.availability === status || (status === "Available" && row.availability === "Available")));
  const filteredEquipmentRows = equipmentRows.filter((row) => (!query || `${row.equipment} ${row.status}`.toLowerCase().includes(query)) && (status === "All status" || row.status === status || (status === "Available" && row.status === "Ready")));
        ...
        <CardContent className="p-0">
          <Tabs className="w-full" value={tab} onValueChange={setTab}>
            <TabsList className="flex h-auto w-full min-w-max gap-2 overflow-x-auto rounded-none border-b border-slate-200 bg-slate-50 p-2">
              <TabsTrigger className="rounded-lg px-4 py-2 text-sm font-semibold data-[state=active]:bg-white data-[state=active]:text-sky-700" value="units">ICU Unit Availability</TabsTrigger>
              <TabsTrigger className="rounded-lg px-4 py-2 text-sm font-semibold data-[state=active]:bg-white data-[state=active]:text-sky-700" value="nurses">Unit Nurse Availability</TabsTrigger>
              <TabsTrigger className="rounded-lg px-4 py-2 text-sm font-semibold data-[state=active]:bg-white data-[state=active]:text-sky-700" value="equipment">Equipment Availability</TabsTrigger>
            </TabsList>
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {filteredUnitRows.map((row) => (
                      <tr className="align-middle hover:bg-sky-50/40" key={row.unit}>
                        ...
                        <td className="px-4 py-4 text-center"><StatusPill tone={statusPillTone(row.status)}>{row.status}</StatusPill></td>
                      </tr>
                    ))}
                    {!filteredUnitRows.length ? <tr><td className="px-4 py-12 text-center text-sm text-slate-500" colSpan={8}>No ICU unit matched the selected filters.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </TabsContent>
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {filteredNurseRows.map((row) => (
                      <tr className="align-middle hover:bg-sky-50/40" key={row.employeeId}>
                        ...
                        <td className="px-4 py-4 text-center"><StatusPill tone={statusPillTone(row.availability)}>{row.availability}</StatusPill></td>
                        <td className="px-4 py-4 text-slate-700">{row.workload}</td>
                      </tr>
                    ))}
                    {!filteredNurseRows.length ? <tr><td className="px-4 py-12 text-center text-sm text-slate-500" colSpan={8}>No nurse matched the selected filters.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </TabsContent>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[800px] border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    ...
                    {!filteredEquipmentRows.length ? <tr><td className="px-4 py-12 text-center text-sm text-slate-500" colSpan={5}>No equipment matched the selected filters.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </TabsContent>
