...
export function RolesPage() {
  ...
  const filtered = mockRoles.filter(
    (role) =>
      textMatch(`${role.name} ${role.description} ${role.departmentScope}`, search) &&
      (type === "All types" || role.type === type),
  );
      ...
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-1">
            ...
            <DisabledReason disabled={row.original.protected}>
              <Button
                size="sm"
                variant="ghost"
      ...
      {({ readOnly }) => (
        <>
          <PageHeader
            ...
            actions={
              <>
                <Button variant="outline" onClick={() => pageToast("Role export")}>
                  <Download className="h-4 w-4" />
                  Export
                </Button>
                <Button variant="outline" onClick={() => pageToast("Permission matrix")}>
          ...
          <SummaryGrid>
            ...
            <StatCard
              label="System roles"
              value={mockRoles.filter((role) => role.type === "System").length}
              icon={Lock}
              change="Protected"
              context="Destructive edits locked"
              tone="warning"
            />
            <StatCard
              label="Custom roles"
              value={mockRoles.filter((role) => role.type === "Custom").length}
              icon={Copy}
              change="Editable"
              context="Can duplicate/edit"
              tone="info"
            />
          </SummaryGrid>
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search role, scope, description..."
          >
            <NativeSelect
              label="Role type"
              value={type}
              onChange={setType}
              options={["All types", "System", "Custom"]}
            />
            <Button variant="outline" onClick={() => toast.success("Static role data refreshed")}>
...
function RoleDrawer({
  ...
  return (
    <Drawer
      ...
      description="Role details, permissions, assigned users, and audit summary."
    >
      {role ? (
        <DrawerTabs>
          <TabsList>
          ...
          <TabsContent value="overview">
            ...
            <DetailRow
              label="Protected"
              value={role.protected ? "System protected role" : "Editable custom role"}
            />
            <DetailRow label="Risk" value={<RiskBadge risk={role.risk} />} />
          </TabsContent>
          <TabsContent value="permissions" className="space-y-2">
            {mockPermissions.slice(0, 6).map((permission) => (
              <PermissionLine key={permission.id} permission={permission} />
            ))}
          </TabsContent>
            ...
            {mockUsers
              ...
              .map((user) => (
                <MiniRecord
                  key={user.id}
                  title={user.name}
                  meta={`${user.designation} • ${user.status}`}
                />
              ))}
          </TabsContent>
    ...
    <div className="rounded-md border border-border p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm font-medium">
          {permission.page} • {permission.action}
        </div>
        {permission.sensitive ? (
          <Badge tone="critical">Sensitive</Badge>
        ) : (
...
export function PermissionsPage() {
  ...
  const filtered = mockPermissions.filter(
    (permission) =>
      textMatch(`${permission.module} ${permission.page} ${permission.action}`, search) &&
      (module === "All modules" || permission.module === module),
  );
  const modules = [
    "All modules",
    ...Array.from(new Set(mockPermissions.map((permission) => permission.module))),
  ];
      ...
      {({ readOnly }) => (
        <>
            ...
            actions={
              <>
                <NativeSelect
                  ...
                  options={mockRoles.map((item) => item.name)}
                />
                <Button variant="outline" onClick={() => pageToast("Expand all")}>
                  <SlidersHorizontal className="h-4 w-4" />
                  Expand
                </Button>
              </>
          ...
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search permission, page, module..."
          >
            <NativeSelect
              label="Module group"
              value={module}
          ...
          <Tabs defaultValue="module" className="space-y-4">
            <TabsList>
              <TabsTrigger value="module">Module access</TabsTrigger>
              <TabsTrigger value="page">Page access</TabsTrigger>
              <TabsTrigger value="tab">Tab access</TabsTrigger>
              <TabsTrigger value="action">Action access</TabsTrigger>
              <TabsTrigger value="sensitive">Sensitive access</TabsTrigger>
            </TabsList>
          ...
          <thead className="sticky top-0 bg-surface-muted text-xs uppercase text-muted-foreground">
            <tr>
              {[
                ...
                <th key={heading} className="border-b border-border px-3 py-2">
                  {heading}
                </th>
              ))}
            </tr>
                ...
                {["View", "Create", "Edit", "Delete", "Approve", "Export"].map((action) => (
                  <td key={action} className="px-3 py-2">
                    <input
                      ...
                      type="checkbox"
                      defaultChecked={
                        permission.enabled && (action === "View" || permission.action === action)
                      }
                      disabled={readOnly || (!permission.enabled && action !== "View")}
                      aria-label={`${permission.page} ${action}`}
                    />
                  </td>
                ))}
...
export function UsersPage() {
  ...
  const filtered = mockUsers.filter(
    (user) =>
      textMatch(`${user.name} ${user.email} ${user.employeeCode} ${user.designation}`, search) &&
  ...
  const columns = React.useMemo<ColumnDef<UserRecord>[]>(
    () => [
      ...
      {
        header: "Role",
        cell: ({ row }) => row.original.roleIds.map((id) => roleById[id]).join(", "),
      },
      {
        header: "Department",
        cell: ({ row }) => row.original.departmentIds.map((id) => deptById[id]).join(", "),
      },
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      { header: "Last login", accessorKey: "lastLoginAt" },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-1">
            ...
            <Button size="sm" variant="ghost" onClick={() => setConfirm(row.original)}>
      ...
      {({ readOnly }) => (
        <>
          <PageHeader
            ...
            description="Manage staff accounts, access mapping, security status, and activity preview."
            actions={
              <>
                <Button variant="outline" onClick={() => pageToast("User import")}>
                  <Download className="h-4 w-4" />
                  Import
                </Button>
                <Button variant="outline" onClick={() => pageToast("Invite user")}>
                  Invite
                </Button>
                <Button disabled={readOnly}>
                  <Plus className="h-4 w-4" />
          ...
          <SummaryGrid>
            ...
            <StatCard
              label="Locked users"
              value={mockUsers.filter((user) => user.locked).length}
              icon={Lock}
              change="Review"
              context="Security action needed"
              tone="danger"
            />
            <StatCard
              label="Online sessions"
              value={mockSecuritySessions.filter((session) => session.status === "Active").length}
              icon={ShieldCheck}
              change="Live"
          ...
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search user, code, email, designation..."
          >
            <NativeSelect
              label="Status"
              value={status}
              onChange={setStatus}
              options={["All status", "Active", "Locked", "Invited", "Inactive"]}
            />
          </FilterBar>
          <DataTable data={filtered} columns={columns} />
          <UserDrawer
          ...
          <ConfirmDrawer
            ...
            target={confirm?.name ?? ""}
            action={confirm?.locked ? "Unlock" : "Lock"}
          />
        </>
      )}
    </ProtectedAdmin>
...
function UserDrawer({
  ...
  return (
    <Drawer
      ...
      description="Profile, access, security, and recent activity."
    >
      {user ? (
        <DrawerTabs>
          <TabsList>
          ...
          <TabsContent value="overview">
            <DetailRow label="Code" value={user.employeeCode} />
          ...
          <TabsContent value="access">
            <DetailRow label="Roles" value={user.roleIds.map((id) => roleById[id]).join(", ")} />
            <DetailRow
              label="Departments"
              value={user.departmentIds.map((id) => deptById[id]).join(", ")}
            />
            <DetailRow label="Landing" value="/dashboard" />
          </TabsContent>
          <TabsContent value="security" className="space-y-3">
            <SecurityNote />
            <DetailRow label="Failed logins" value={user.failedLogins} />
            <DetailRow label="Password" value="Never displayed" masked />
          ...
          <TabsContent value="activity">
            <DetailRow label="Last login" value={user.lastLoginAt} />
            <DetailRow label="Recent action" value="Viewed dashboard summary" />
            <DetailRow label="Failed attempt" value={`${user.failedLogins} recent attempts`} />
          </TabsContent>
        </DrawerTabs>
      ) : null}
    </Drawer>
  ...
  const filtered = departments.filter(
    (dept) =>
      ...
      (emergency === "All emergency" ||
        (emergency === "Emergency enabled" ? dept.emergencyAvailable : !dept.emergencyAvailable)),
  );
  const columns = React.useMemo<ColumnDef<DepartmentRecord>[]>(
    () => [
      ...
      { header: "OPD timing", accessorKey: "opdTiming" },
      {
        ...
        cell: ({ row }) => (
          <div>
            <div>{row.original.floor}</div>
            <div className="text-xs text-muted-foreground">{row.original.roomWing}</div>
          </div>
        ),
      },
      {
        ...
        cell: ({ row }) =>
          row.original.emergencyAvailable ? (
            <Badge tone="critical">Enabled</Badge>
          ) : (
      ...
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            ...
            <Button size="sm" variant="ghost" onClick={() => pageToast("Assign doctors")}>
              Assign
            </Button>
            <Button size="sm" variant="ghost" onClick={() => pageToast("Manage OPD timing")}>
              Timing
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setConfirm(row.original)}>
      ...
      {({ readOnly }) => (
        <>
          <div className="flex justify-end">
            <Button disabled={readOnly} onClick={() => setFormOpen(true)}>
              <Plus className="h-4 w-4" />
              Add department
            </Button>
          </div>
          <SummaryGrid>
            ...
            <StatCard
              ...
              value={departments.reduce((sum, dept) => sum + dept.totalDoctors, 0)}
              icon={Users}
              change="Mapped"
            ...
            <StatCard
              ...
              value={
                departments.filter(
                  (d) => d.enabledWorkflows.includes("OPD") || d.opdTiming !== "Not applicable",
                ).length
              }
              icon={CalendarDays}
              change="Scheduled"
              context="Timing configured"
              tone="warning"
            />
            <StatCard
              label="Emergency-enabled"
              value={departments.filter((d) => d.emergencyAvailable).length}
              icon={ShieldAlert}
              change="Priority"
              context="Emergency coverage"
              tone="danger"
            />
          </SummaryGrid>
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search department by name, code, or head doctor..."
          >
            <NativeSelect
              label="Status"
              value={status}
            ...
            />
            <NativeSelect label="Branch" value={branch} onChange={setBranch} options={branches} />
            <NativeSelect
              label="Emergency"
              value={emergency}
              onChange={setEmergency}
              options={["All emergency", "Emergency enabled", "No emergency"]}
            />
          </FilterBar>
          <DataTable data={filtered} columns={columns} />
          <DepartmentDrawer
          ...
          <DepartmentFormDrawer
            ...
            open={formOpen}
            onCreate={(department) => setDepartments((current) => [department, ...current])}
          ...
          <ConfirmDrawer
            ...
            target={confirm?.name ?? ""}
            action={confirm?.status === "Active" ? "Deactivate" : "Activate"}
          />
        </>
      )}
    </ProtectedAdmin>
...
export function DoctorsPage() {
  ...
  const filtered = doctors.filter(
    (doctor) =>
      textMatch(
        `${doctor.name} ${doctor.mobile} ${doctor.email} ${doctor.department} ${doctor.specialization}`,
        search,
      ) &&
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div>
            <div className="font-medium">{row.original.name}</div>
            <div className="text-xs text-muted-foreground">{row.original.doctorId}</div>
          </div>
        ),
      },
      ...
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        ...
        cell: ({ row }) => (
          <div className="flex flex-wrap gap-1">
            ...
            <Button size="sm" variant="ghost" onClick={() => pageToast("Manage availability")}>
              Availability
            </Button>
            <Button size="sm" variant="ghost" onClick={() => pageToast("Assign department")}>
              Assign
            </Button>
            <Button size="sm" variant="ghost" onClick={() => pageToast("Password reset")}>
      ...
      {({ readOnly }) => (
        <>
          <div className="flex flex-wrap justify-end gap-2">
            <Button variant="outline" onClick={() => pageToast("Doctor CSV export")}>
              <Download className="h-4 w-4" />
              Export CSV
            </Button>
            <Button variant="outline" onClick={() => pageToast("Bulk upload doctors")}>
          ...
          <SummaryGrid>
            <StatCard
              ...
              icon={Users}
              change="Staff master"
              context="Credentialed doctors"
              tone="info"
            />
            <StatCard
              label="Available doctors"
              value={doctors.filter((doctor) => doctor.availabilityStatus === "Available").length}
              icon={UserRoundCheck}
              change="Now"
              context="Can accept appointments"
              tone="success"
            />
            <StatCard
              label="On leave"
              value={doctors.filter((doctor) => doctor.availabilityStatus === "On leave").length}
              icon={CalendarDays}
              change="Planned"
            ...
            />
            <StatCard
              label="Active OPD doctors"
              value={doctors.filter((doctor) => doctor.activeOpd).length}
              icon={ShieldCheck}
              change="OPD"
            ...
            <StatCard
              ...
              value={doctors.filter((doctor) => doctor.telemedicine).length}
              icon={UserCog}
              change="Virtual"
              context="Online consult ready"
              tone="info"
            />
          </SummaryGrid>
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search doctor by name, phone, email, department, specialization..."
          >
            <NativeSelect
              label="Department"
              value={department}
            ...
            <NativeSelect
              ...
              options={["All availability", "Available", "On leave", "Unavailable"]}
            />
          </FilterBar>
          <DataTable data={filtered} columns={columns} />
          ...
          <DoctorFormDrawer
            ...
            open={formOpen}
            onCreate={(doctor) => setDoctors((current) => [doctor, ...current])}
            onOpenChange={setFormOpen}
            readOnly={readOnly}
          />
          <ConfirmDrawer
...
function DoctorDrawer({
  ...
  return (
    <Drawer
      ...
      description="Profile, department, schedule, fees, permissions, and validation status."
    >
      {doctor ? (
        <DrawerTabs>
          <TabsList>
            <TabsTrigger value="profile">Profile</TabsTrigger>
            <TabsTrigger value="work">Work setup</TabsTrigger>
          ...
          <TabsContent value="profile">
            ...
            <DetailRow label="Registration" value={doctor.registrationNumber} />
          </TabsContent>
          <TabsContent value="work">
            <DetailRow label="Branch" value={doctor.branch} />
            <DetailRow label="Room" value={doctor.room} />
            <DetailRow label="Consultation" value={doctor.consultationTypes.join(", ")} />
            <DetailRow label="Fee" value={doctor.consultationFee} />
            <DetailRow label="Emergency" value={doctor.emergencyFee} />
            <DetailRow
              label="Slot"
              value={`${doctor.slotDuration} • ${doctor.maxPatientsPerSlot} patient/slot`}
            />
          </TabsContent>
          <TabsContent value="availability">
            <DetailRow label="Status" value={<StatusBadge status={doctor.availabilityStatus} />} />
            <DetailRow label="OPD timing" value={doctor.opdTiming} />
            <DetailRow label="Break" value={doctor.breakTiming} />
            <DetailRow label="Telemedicine" value={doctor.telemedicineTiming} />
            <DetailRow label="Leave" value={doctor.leaveSchedule} />
          </TabsContent>
          <TabsContent value="login">
            <DetailRow label="Username" value={doctor.username} />
            <DetailRow label="Password" value="Temporary password masked" masked />
            <DetailRow label="Role" value={doctor.role} />
            <DetailRow label="Modules" value={doctor.allowedModules.join(", ")} />
            <DetailRow label="Status" value={<StatusBadge status={doctor.status} />} />
          </TabsContent>
        </DrawerTabs>
      ) : null}
    </Drawer>
...
function DepartmentDrawer({
  ...
  const assignedDoctors = mockDoctors.filter((doctor) =>
    department?.assignedDoctorIds.includes(doctor.id),
  );
  return (
    <Drawer
      ...
      description="Department profile, location, doctor assignment, OPD, emergency, and services."
    >
      {department ? (
        <DrawerTabs>
          <TabsList>
          ...
          <TabsContent value="overview">
            ...
            <DetailRow label="Type" value={department.type} />
            <DetailRow label="Description" value={`${department.name} operational master setup`} />
            <DetailRow label="Status" value={<StatusBadge status={department.status} />} />
          </TabsContent>
          <TabsContent value="location">
            <DetailRow label="Branch" value={department.branch} />
            <DetailRow label="Floor" value={department.floor} />
            <DetailRow label="Room/wing" value={department.roomWing} />
            <DetailRow label="Contact" value={department.contactNumber} />
            <DetailRow label="Email" value={department.email} />
          </TabsContent>
          <TabsContent value="doctors" className="space-y-2">
            <DetailRow label="Head" value={department.head} />
            <DetailRow label="Total doctors" value={department.totalDoctors} />
            {assignedDoctors.map((doctor) => (
              <MiniRecord
                key={doctor.id}
                title={doctor.name}
                meta={`${doctor.specialization} • ${doctor.availabilityStatus}`}
              />
            ))}
          </TabsContent>
          <TabsContent value="opd">
            <DetailRow label="OPD timing" value={department.opdTiming} />
            <DetailRow label="Working days" value={department.workingDays} />
            <DetailRow label="Emergency" value={department.emergencyAvailable ? "Enabled" : "No"} />
            <DetailRow label="Emergency doctor" value={department.emergencyContactDoctor} />
            <DetailRow
              label="Capacity"
              value={`${department.patientCapacityPerDay} patients/day`}
            />
          </TabsContent>
          <TabsContent value="services" className="space-y-2">
            <DetailRow label="Fee range" value={department.feeRange} />
            <div className="flex flex-wrap gap-2">
              {department.servicesOffered.map((item) => (
            ...
            <div className="flex flex-wrap gap-2">
              {department.linkedServices.map((item) => (
...
function DoctorFormDrawer({
  ...
  open: boolean;
  onCreate: (_doctor: DoctorRecord) => void;
  onOpenChange: (_open: boolean) => void;
  readOnly?: boolean;
}) {
  ...
  function handleSave() {
    ...
    if (
      existingDoctors.some((doctor) => doctor.doctorId.toLowerCase() === doctorId.toLowerCase())
    ) {
    ...
    if (
      existingDoctors.some(
        (doctor) => doctor.registrationNumber.toLowerCase() === registrationNumber.toLowerCase(),
      )
    ) {
    ...
    const availabilityStatus =
      form.availabilityStatus === "On leave" || form.availabilityStatus === "Unavailable"
    ...
    const newDoctor: DoctorRecord = {
      ...
      status,
      telemedicine:
        consultationTypes.includes("Telemedicine") || Boolean(form.telemedicineTiming.trim()),
      activeOpd:
        status === "Active" && Boolean(form.opdTiming.trim()) && availabilityStatus === "Available",
  ...
  return (
    <Drawer
      open={open}
      onOpenChange={(nextOpen) => (nextOpen ? onOpenChange(true) : resetAndClose())}
      title="Add doctor"
      description="Five-step doctor setup with department, schedule, fee, availability, and login mapping."
    ...
    >
      <Tabs defaultValue="basic" className="space-y-4">
        ...
        {error ? (
          <div className="rounded-lg border border-danger/30 bg-danger/10 px-3 py-2 text-xs font-medium text-danger">
            {error}
          </div>
        ) : null}
        ...
        <TabsContent value="professional" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            <DepartmentInput
              label="Doctor ID / Employee ID"
              value={form.doctorId}
              onChange={(value) => updateField("doctorId", value.toUpperCase())}
            />
            <DepartmentInput
              label="Department"
            ...
            <DepartmentInput
              ...
              onChange={(value) => updateField("licenseDocument", value)}
            />
          </div>
          <StepNote
            items={[
              "Duplicate doctor ID check",
              "Medical registration validation",
              "Department required before save",
            ]}
          />
        </TabsContent>
        <TabsContent value="work" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <DepartmentInput
              ...
              onChange={(value) => updateField("consultationTypes", value)}
            />
            <DepartmentInput
            ...
            <DepartmentInput
              ...
              onChange={(value) => updateField("emergencyFee", value)}
            />
            <DepartmentInput
              label="Appointment slot duration"
              value={form.slotDuration}
              onChange={(value) => updateField("slotDuration", value)}
            />
            <DepartmentInput
              label="Max patients per slot"
              type="number"
              value={form.maxPatientsPerSlot}
              onChange={(value) => updateField("maxPatientsPerSlot", value)}
            />
          </div>
        </TabsContent>
        <TabsContent value="availability" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            <DepartmentInput
              ...
              onChange={(value) => updateField("weeklySchedule", value)}
            />
            <DepartmentInput
              label="OPD timing"
              value={form.opdTiming}
              onChange={(value) => updateField("opdTiming", value)}
            />
            <DepartmentInput
              label="Break timing"
              value={form.breakTiming}
              onChange={(value) => updateField("breakTiming", value)}
            />
            <DepartmentInput
              label="Telemedicine timing"
              value={form.telemedicineTiming}
              onChange={(value) => updateField("telemedicineTiming", value)}
            />
            <DepartmentInput
              label="Leave schedule"
              value={form.leaveSchedule}
              onChange={(value) => updateField("leaveSchedule", value)}
            />
            <DepartmentInput
          ...
          <StepNote
            items={["Slot overlap validation", "Leave schedule blocks appointment slots"]}
          />
        </TabsContent>
        <TabsContent value="login" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <DepartmentInput
              ...
              onChange={(value) => updateField("allowedModules", value)}
            />
            <DepartmentInput
...
function DepartmentFormDrawer({
  ...
  open: boolean;
  onCreate: (_department: DepartmentRecord) => void;
  onOpenChange: (_open: boolean) => void;
  readOnly?: boolean;
}) {
  ...
  function handleSave() {
    ...
    if (
      existingDepartments.some((department) => department.code.toLowerCase() === code.toLowerCase())
    ) {
    ...
    const assignedDoctorIds = mockDoctors
      .filter((doctor) => assignedDoctorNames.includes(doctor.name))
    ...
    const newDepartment: DepartmentRecord = {
      ...
      emergencyContactDoctor:
        form.emergencyContactDoctor.trim() || form.head.trim() || "Not assigned",
  ...
  return (
    <Drawer
      ...
      description="Five-step department setup with location, doctors, OPD, emergency, and linked services."
      className="md:w-[720px]"
      footer={
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={resetAndClose}>
      ...
      <Tabs defaultValue="details" className="space-y-4">
        ...
        <TabsContent value="location" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <DepartmentInput
              label="Room/wing"
              value={form.roomWing}
              onChange={(value) => updateField("roomWing", value)}
            />
            <DepartmentInput
        ...
        <TabsContent value="doctors" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <DepartmentInput
              ...
              value={form.assignedDoctors}
              onChange={(value) => updateField("assignedDoctors", value)}
            />
            <DepartmentInput
              label="Primary consultant"
              value={form.primaryConsultant}
              onChange={(value) => updateField("primaryConsultant", value)}
            />
            <DepartmentInput
              label="On-call doctors"
              value={form.onCallDoctors}
              onChange={(value) => updateField("onCallDoctors", value)}
            />
          </div>
          <StepNote
            items={[
              "Head doctor required for active department",
              "Assigned doctors shown dynamically on department page",
            ]}
          />
        </TabsContent>
        <TabsContent value="opd" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            />
            <DepartmentInput
              label="Emergency availability yes/no"
              value={form.emergencyAvailable}
              onChange={(value) => updateField("emergencyAvailable", value)}
            />
            <DepartmentInput
              label="Emergency contact doctor"
              value={form.emergencyContactDoctor}
              onChange={(value) => updateField("emergencyContactDoctor", value)}
            />
            <DepartmentInput
              label="Patient capacity per day"
              type="number"
              value={form.patientCapacityPerDay}
              onChange={(value) => updateField("patientCapacityPerDay", value)}
            />
          </div>
          <StepNote
            items={[
              "At least one doctor required before enabling OPD",
              "OPD timing overlap validation",
            ]}
          />
        </TabsContent>
        <TabsContent value="services" className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            />
            <DepartmentInput
              label="Consultation fee range"
              value={form.feeRange}
              onChange={(value) => updateField("feeRange", value)}
            />
            <DepartmentInput
              label="Lab/radiology linked services"
              value={form.linkedServices}
              onChange={(value) => updateField("linkedServices", value)}
            />
            <DepartmentInput
...
function GenderSelect({ onChange, value }: { onChange: (_value: string) => void; value: string }) {
  return (
    <label className="space-y-1 text-sm">
    ...
    <label className="space-y-1 text-sm">
      ...
      <div className="flex min-h-9 items-center gap-3 rounded-md border border-input bg-background px-3 py-2">
        ...
        <div className="min-w-0 flex-1">
          <input
            className="block w-full text-xs text-muted-foreground file:mr-3 file:rounded-md file:border-0 file:bg-surface-muted file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-foreground"
            type="file"
            accept="image/*"
            onChange={onChange}
          />
  ...
  return (
    <ProtectedAdmin readOnly={adminReadOnlyRoles}>
      {({ readOnly }) => (
        <>
          <PageHeader
            eyebrow="Phase 2 • Hospital Master"
            title="Hospital Master Setup"
            description="Hospital profile, contact, legal, operations, branding, print settings, and audit summary."
            actions={
              <>
                <Button variant="outline" onClick={() => pageToast("Print preview")}>
          ...
          <Tabs defaultValue="profile" className="space-y-4">
            ...
            <TabsContent value="profile">
              <SettingsForm
                fields={[
                  ["Hospital name", mockHospitalProfile.name],
                  ["Short name", mockHospitalProfile.shortName],
                  ["Hospital type", mockHospitalProfile.type],
                  ["Established year", mockHospitalProfile.establishedYear],
                ]}
              />
            </TabsContent>
            <TabsContent value="contact">
              <SettingsForm
                fields={[
                  ["Address", mockHospitalProfile.address],
            ...
            <TabsContent value="legal">
              <SettingsForm
                fields={[
                  ["Registration number", mockHospitalProfile.registrationNumber],
                  ["GST number", mockHospitalProfile.gst],
                  ["NABH/NABL", "Prepared placeholder"],
                  ["PAN", "•••••• masked"],
                ]}
              />
            </TabsContent>
            <TabsContent value="operations">
              <SettingsForm
                fields={[
                  ["Working hours", mockHospitalProfile.workingHours],
                  ["Timezone", mockHospitalProfile.timezone],
                  ["Currency", mockHospitalProfile.currency],
            ...
            <TabsContent value="branding">
              <AdminSection
                ...
                description="Branding maps into Phase 1 theme tokens without bypassing semantic colors."
              >
                <SettingsForm
                  fields={[
                    ["Logo", "Upload placeholder"],
                    ["Primary preset", "Uses global dynamic theme"],
                    ["Hospital seal", "Future upload placeholder"],
                  ]}
                />
              </AdminSection>
            </TabsContent>
            <TabsContent value="print">
              <AdminSection
                ...
                description="Navigation and actions hide during print. Sensitive fields remain masked."
              >
                <div className="rounded-lg border border-border bg-white p-4 text-slate-900">
                  <div className="font-semibold">{mockHospitalProfile.name}</div>
                  <div className="text-xs">{mockHospitalProfile.address}</div>
                  <div className="mt-4 border-t pt-3 text-sm">
                    Prescription / Invoice / Lab report header placeholder
                  </div>
                </div>
              </AdminSection>
            </TabsContent>
            <TabsContent value="audit">
              <SettingsForm
                fields={[
                  ["Last updated by", "Hospital Admin"],
                  ["Last updated", "Today 09:20"],
                  ["Sensitive changes", "Legal fields masked in audit log"],
                ]}
              />
            </TabsContent>
          </Tabs>
  ...
  return (
    <ProtectedAdmin>
      {() => (
        <>
          <PageHeader
            eyebrow="Phase 2 • Future Ready"
            title="Branch Management"
            description="Single-hospital mode today, with branch-ready information architecture reserved for future expansion."
            actions={
              <>
                <DisabledFutureAction />
                <Button disabled>
          ...
          />
          <AdminSection
            title="Single hospital mode"
            description="Future branches are visible as disabled records so current users are not confused by unavailable operations."
          >
          ...
          <Drawer
            ...
            description="Overview, departments, contacts, and future settings."
          >
            {selected ? (
              <DrawerTabs>
                <TabsList>
                ...
                <TabsContent value="departments">
                  <DetailRow label="Departments" value={`${selected.departments} available`} />
                </TabsContent>
                <TabsContent value="contacts">
                  <DetailRow
                    label="Contact"
                    value={
                      selected.status === "Active" ? mockHospitalProfile.phone : "Future setup"
                    }
                  />
                </TabsContent>
  ...
  return (
    <ProtectedAdmin>
      {({ readOnly }) => (
        <>
          <PageHeader
            eyebrow="Phase 2 • Security"
            title="Security Management"
            description="Sessions, devices, IP restrictions, password policy, MFA policy, and login rules."
            actions={
              <Button
                disabled={readOnly}
                onClick={() => toast.success("Security policy saved in static preview")}
              >
                <Save className="h-4 w-4" />
                Save policy
              </Button>
          ...
          <SummaryGrid>
            <StatCard
              ...
              value={mockSecuritySessions.filter((s) => s.status === "Active").length}
              icon={ShieldCheck}
              change="Live"
            ...
            <StatCard
              ...
              value={mockDevices.filter((d) => d.trustStatus === "Trusted").length}
              icon={UserCog}
              change="Approved"
              context="Remembered devices"
              tone="info"
            />
            <StatCard
              label="Blocked IPs"
              value={mockIpRules.filter((r) => r.type === "Block").length}
              icon={Lock}
              change="Restricted"
              context="IP rules"
              tone="warning"
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="sessions">
              <SessionsTable
                data={mockSecuritySessions}
                onRisk={(row) => setConfirm({ target: row.user, action: "Force logout" })}
              />
            </TabsContent>
            <TabsContent value="devices">
              <DevicesTable
                data={mockDevices}
                onRisk={(row) => setConfirm({ target: row.name, action: "Block device" })}
              />
            </TabsContent>
            <TabsContent value="ip">
              <IpRulesTable
                onRisk={(target) => setConfirm({ target, action: "Disable IP rule" })}
              />
            </TabsContent>
            <TabsContent value="password">
              <PolicyGrid
                fields={[
                  ["Minimum length", `${mockPasswordPolicy.minLength}`],
                  ["Uppercase", "Required"],
                  ["Lowercase", "Required"],
                  ["Number", "Required"],
                  ["Special character", "Required"],
                  ["Expiry", `${mockPasswordPolicy.expiryDays} days`],
                  ["Failed lock count", `${mockPasswordPolicy.failedAttemptLockCount}`],
                  ["Reuse limit", `${mockPasswordPolicy.reuseLimit}`],
                ]}
              />
            </TabsContent>
            <TabsContent value="mfa">
              <PolicyGrid
                fields={[
                  ["Admin roles", mockMfaPolicy.requiredForAdmin ? "Required" : "Optional"],
                  ["Security settings", "Required"],
                  ["Financial approvals", "Required placeholder"],
                  ["Methods", mockMfaPolicy.methods.join(", ")],
                  ["Trusted device", `${mockMfaPolicy.trustedDeviceDurationDays} days`],
                  ["Failed OTP limit", `${mockMfaPolicy.failedAttemptLimit}`],
                ]}
              />
            </TabsContent>
            <TabsContent value="login">
              <PolicyGrid
                fields={[
                  ...
                  ["Remember device", "Allowed after MFA"],
                  ["Multiple sessions", "Warn and allow"],
                  ["First login", "Force password change"],
                  ["Location warning", "Enabled placeholder"],
                  ["New device alert", "Enabled placeholder"],
                ]}
              />
            </TabsContent>
          </Tabs>
...
function SessionsTable({
  ...
  onRisk: (_row: SecuritySession) => void;
}) {
  const columns = React.useMemo<ColumnDef<SecuritySession>[]>(
    () => [
      ...
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        header: "Actions",
        cell: ({ row }) => (
          <Button size="sm" variant="danger" onClick={() => onRisk(row.original)}>
            Force logout
          </Button>
        ),
      },
...
function DevicesTable({
  ...
  data: TrustedDevice[];
  onRisk: (_row: TrustedDevice) => void;
}) {
  const columns = React.useMemo<ColumnDef<TrustedDevice>[]>(
    () => [
      { header: "Device", accessorKey: "name" },
      { header: "User", accessorKey: "user" },
      { header: "Browser", accessorKey: "browser" },
      { header: "Last used", accessorKey: "lastUsed" },
      { header: "Trust", cell: ({ row }) => <StatusBadge status={row.original.trustStatus} /> },
      { header: "Risk", cell: ({ row }) => <RiskBadge risk={row.original.risk} /> },
      {
        header: "Actions",
        cell: ({ row }) => (
...
function IpRulesTable({ onRisk }: { onRisk: (_target: string) => void }) {
  const columns = React.useMemo<ColumnDef<(typeof mockIpRules)[number]>[]>(
    () => [
      { header: "IP/range", accessorKey: "range" },
      { header: "Type", accessorKey: "type" },
      ...
      { header: "Status", cell: ({ row }) => <StatusBadge status={row.original.status} /> },
      {
        header: "Actions",
        cell: ({ row }) => (
          <Button size="sm" variant="outline" onClick={() => onRisk(row.original.range)}>
            Disable
          </Button>
        ),
      },
...
export function AuditLogsPage() {
  ...
  const filtered = mockAuditLogs.filter(
    (log) =>
      textMatch(
        `${log.actorName} ${log.module} ${log.eventType} ${log.target} ${log.ipAddress}`,
        search,
      ) &&
  ...
  const columns = React.useMemo<ColumnDef<AuditLog>[]>(
    () => [
      ...
      { header: "Severity", cell: ({ row }) => <StatusBadge status={row.original.severity} /> },
      {
        header: "Actions",
        cell: ({ row }) => (
          <Button size="sm" variant="outline" onClick={() => setSelected(row.original)}>
  ...
  return (
    <ProtectedAdmin allowed={["Super Admin", "Hospital Admin"]} readOnly={["Management"]}>
      {() => (
        <>
          <PageHeader
            eyebrow="Phase 2 • Immutable Audit"
            title="Audit Logs"
            description="Searchable read-only activity records for user, role, setup, and security events."
            actions={
              <>
                <Button variant="outline" onClick={() => pageToast("Audit print")}>
                  <Printer className="h-4 w-4" />
                  Print
                </Button>
                <Button variant="outline" onClick={() => pageToast("Audit export")}>
                  <Download className="h-4 w-4" />
                  Export
                </Button>
              </>
          ...
          <FilterBar
            search={search}
            onSearch={setSearch}
            placeholder="Search audit event, user, module, target, IP..."
          >
            <NativeSelect
              label="Severity"
              value={severity}
              onChange={setSeverity}
              options={["All severity", "Info", "Warning", "Critical", "Security"]}
            />
          </FilterBar>
          <DataTable data={filtered} columns={columns} />
          <Drawer
            ...
            description="Read-only detail. Clinical and sensitive values are masked."
          >
            {selected ? (
              <div>
                <DetailRow label="Actor" value={`${selected.actorName} (${selected.actorRole})`} />
                <DetailRow label="Target" value={selected.target} />
                <DetailRow label="Module" value={selected.module} />
                <DetailRow label="Device/IP" value={`${selected.device} • ${selected.ipAddress}`} />
                <DetailRow
                  label="Before"
                  value={selected.before}
                  masked={selected.sensitiveFieldsMasked}
                />
                <DetailRow
                  label="After"
