...
import { DetailRow, SecurityNote } from "@/features/operations/admin/admin-shared";
import { PatientAlertChips } from "@/features/clinical/patients/patient-shared";
...
export const appointmentAccessRoles: Role[] = [
  "Super Admin",
  "Hospital Admin",
  "Receptionist",
  "Doctor",
...
export const appointmentReadOnlyRoles: Role[] = [
  "Doctor",
  "Doctor OPD",
  "Nurse",
  "Billing Executive",
...
export function ProtectedAppointment({
  children,
}: {
  ...
  if (!access.allowed) {
    return (
      <EmptyState
        icon={LockKeyhole}
        title="Appointment permission required"
        description="Your current static role cannot access Phase 4 scheduling and front-office workflows."
      />
    );
  }
  return (
...
export function operationalTone(status: string): StatusTone {
  if (
    ["Completed", "Confirmed", "Paid", "Available", "Active", "Ready to join", "Signed"].includes(
      status,
    )
  )
    return "success";
  if (
    ...
    return "warning";
  if (
    ["Cancelled", "No-show", "Skipped", "Expired", "Failed/no-show", "Missing", "Delayed"].includes(
      status,
    )
  )
    return "danger";
  if (
    ["Emergency", "Critical delay", "In consultation", "Serving", "In call placeholder"].includes(
      status,
    )
  )
    return "critical";
  if (
    ["Checked in", "Called", "Issued", "Scheduled", "Rescheduled", "Overbook allowed"].includes(
      status,
    )
  )
    return "info";
    ...
    <div className="min-w-0">
      ...
      {!compact ? (
        <div className="mt-1 text-xs text-muted-foreground">
          {patient.age}/{patient.gender} • {patient.maskedMobile}
        </div>
      ) : null}
    </div>
  );
...
export function AppointmentDetailDrawer({
  ...
  const token = appointment?.tokenId
    ? mockTokens.find((item) => item.id === appointment.tokenId)
    : undefined;
  const queue = appointment
    ? mockQueueEntries.find((item) => item.appointmentId === appointment.id)
    : undefined;
  const visits = appointment
    ? mockPatientVisits.filter((item) => item.patientId === appointment.patientId)
    : [];
  ...
  return (
    <Drawer
      ...
      description="Appointment, patient, queue, communication, and audit context."
    >
      {appointment && patient ? (
        <Tabs defaultValue="summary">
          ...
          <TabsContent value="patient">
            ...
            <Button className="mt-3" asChild variant="outline">
          ...
          <TabsContent value="queue">
            ...
            <DetailRow
              label="Queue"
              value={queue ? <OperationalStatus status={queue.status} /> : "Not in queue"}
            />
          ...
          <TabsContent value="audit">
            ...
            <DetailRow
              ...
              value="Check-in, token, reschedule, cancel events will be audited."
            />
            <SecurityNote />
          </TabsContent>
        </Tabs>
...
export function TokenBoardPreview() {
  const active = mockTokens
    .filter((token) => ["Called", "Serving", "Issued", "Held"].includes(token.status))
    .slice(0, 4);
  return (
    <Card className="bg-sidebar text-sidebar-foreground">
      <CardContent className="p-4">
        <div className="flex items-center justify-between gap-3 border-b border-sidebar-foreground/15 pb-3">
          <div>
            <div className="text-xs uppercase tracking-wide text-sidebar-foreground/65">
              Token board preview
            </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {active.map((token) => (
            <div
              key={token.id}
              className="rounded-lg border border-sidebar-foreground/15 bg-sidebar-foreground/5 p-3"
            >
              <div className="text-3xl font-semibold">{token.tokenNo}</div>
              <div className="mt-1 text-sm">{token.counter}</div>
              <div className="text-xs text-sidebar-foreground/70">
                {token.department} • {token.doctor}
              </div>
            </div>
          ))}
        </div>
