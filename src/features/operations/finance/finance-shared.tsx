  ...
  if (!access.allowed) {
    return (
      <EmptyState
        ...
        description="Your current static role cannot access billing, finance, insurance, or TPA workflows."
      />
    );
  }
  return (
    <div className="space-y-4">
      {access.readOnly ? (
        <AlertBanner icon={LockKeyhole} tone="warning" title="Read-only finance access">
          {access.role} can review financial status in this static preview, but billing, refund,
          discount, cash, claim, and reconciliation actions are disabled.
        </AlertBanner>
      ) : null}
...
export function FinanceSafetyBanner({ module }: { module: FinanceModule }) {
  ...
  return (
    <AlertBanner icon={ShieldAlert} tone="warning" title="Financial governance">
      {text}
    </AlertBanner>
  );
}
...
export function AmountWarning({ children }: { children: React.ReactNode }) {
  return (
    <AlertBanner icon={AlertTriangle} tone="critical" title="Amount / approval warning">
      {children}
    </AlertBanner>
  );
}

export function BillingPatientHeader({ bill, title }: { bill: BillingRecord; title: string }) {
  ...
  if (!patient)
    return (
      <EmptyState
        ...
        description="Static patient context is unavailable for this bill."
      />
    );
  return (
    <Card className="sticky top-[132px] z-20">
      <CardContent className="flex flex-col gap-3 p-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-2 xl:min-w-[560px]">
          ...
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Bill: {bill.billNo} • {bill.source}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Payer: {bill.payerType}
          </div>
          <div className="rounded-md border border-border bg-surface-muted p-2">
        ...
        <Button size="sm" variant="outline" asChild>
          <Link href={`/emr/patients/${bill.patientId}/timeline`}>Audit/EMR handoff</Link>
        </Button>
      </CardContent>
    </Card>
  );
      ...
      <CardContent className="space-y-3 p-4">
        ...
        <div className="grid gap-2 text-xs">
          <div className="rounded-md bg-surface-muted p-2">
            Payer: {payer?.name ?? "Self / package / TPA placeholder"}
          </div>
          <div className="rounded-md bg-surface-muted p-2">
            Tariff: {payer?.tariff ?? "Hospital default tariff"}
          </div>
          <div className="rounded-md bg-surface-muted p-2">
            Contract: {payer?.contractStatus ?? "Not applicable"}
          </div>
        </div>
      </CardContent>
    </Card>
