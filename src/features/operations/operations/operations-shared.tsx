...
const operationsAccessRoles: Role[] = [
  "Super Admin",
  "Hospital Admin",
  "Doctor",
  "Nurse",
...
export function ProtectedOperations({
  ...
  if (!access.allowed) {
    return (
      <EmptyState
        icon={LockKeyhole}
        title="Phase 9 permission required"
        description="Your current static role cannot access pharmacy, inventory, store, or OT workflows."
      />
    );
  }
  return (
    <div className="space-y-4">
      {access.readOnly ? (
        <AlertBanner icon={LockKeyhole} tone="warning" title="Read-only operational access">
          {access.role} can review Phase 9 operational states, but dispense, stock, purchase, OT,
          sterilization, and infection-control actions are disabled.
        </AlertBanner>
      ) : null}
...
export function operationTone(status: string): StatusTone {
  ...
  if (
    [
      "Critical stock",
      "Controlled medicine placeholder",
      "Emergency",
      "Quarantined",
      "Blocked",
      "Override required",
...
export function OperationSafetyBanner({ module }: { module: OperationsModule }) {
  ...
  return (
    <AlertBanner icon={ShieldAlert} tone="warning" title="Operational safety">
      {text}
    </AlertBanner>
  );
}
...
export function CriticalOperationBanner({ children }: { children: React.ReactNode }) {
  return (
    <AlertBanner icon={AlertTriangle} tone="critical" title="High-impact action">
      {children}
    </AlertBanner>
  );
}
  ...
  if (!patient)
    return (
      <EmptyState
        ...
        description="The patient context is not available in static Phase 9 data."
      />
    );
    ...
    <Card className="sticky top-[132px] z-20">
      <CardContent className="flex flex-col gap-3 p-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-2 xl:min-w-[520px]">
          ...
          <div className="rounded-md border border-border bg-surface-muted p-2">
            Privacy: audit handoff placeholder
          </div>
        </div>
        <Button size="sm" variant="outline" asChild>
          <Link href={`/emr/patients/${patientId}/timeline`}>EMR timeline</Link>
        </Button>
      </CardContent>
    </Card>
  );
...
export function StockWarningStrip({
  status,
  batch,
  expiry,
}: {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-surface-muted p-3 text-xs">
      ...
      {expiry ? (
        <Badge
          tone={
            expiry.includes("Expired") || expiry === "Expired"
              ? "danger"
              : expiry.includes("Near")
                ? "warning"
                : "muted"
          }
        >
      ...
      <span className="text-muted-foreground">
        Expired, quarantined, recalled, or damaged stock remains blocked visually.
      </span>
    </div>
  );
}
