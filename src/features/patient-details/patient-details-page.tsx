...
import {
  PatientHistoryPage,
  type HistoryTab,
} from "@/features/patient-history/patient-history-page";
import {
  calculatorDefinitions,
  initialCalculatorValues,
  validateCalculator,
  type CalculationResult,
} from "./medical-calculator-engine";
import {
  applyPatientSection,
...
const visiblePatientDetailTabs = patientDetailTabs.filter(
  (tab) => tab.id !== "calculator" || showMedicalCalculator,
);

const patientHistoryTabOrder: HistoryTab[] = [
  "medical",
  "surgical",
  "medication",
  "allergy",
...
const patientDocumentSchema = [
  ...
  {
    tabId: "clinical",
    tabLabel: "2. Physical & Clinical",
    fields: [
      "Blood Group (Reconfirm)",
      "Height",
      "Weight",
      "BMI (Auto)",
      "Allergies",
...
const medicalCalculators: Array<{
  ...
  },
  {
    ...
    category: "Nephrology",
    fhirTarget: "Observation + DiagnosticReport",
  },
  {
    id: "mdrd",
    name: "MDRD eGFR",
    subtitle: "Estimated GFR",
    category: "Nephrology",
    fhirTarget: "Observation + DiagnosticReport",
  },
  {
    ...
    category: "Neurology",
    fhirTarget: "Observation + RiskAssessment",
  },
  {
    id: "apache",
    name: "APACHE II Score",
    subtitle: "ICU mortality risk",
    category: "Critical Care",
    fhirTarget: "RiskAssessment",
  },
  {
    id: "sofa",
    name: "SOFA Score",
    subtitle: "Organ failure assessment",
    category: "Critical Care",
    fhirTarget: "RiskAssessment",
  },
  {
...
function MedicalCalculatorWorkspace({
  ...
  const [favorites, setFavorites] = React.useState<CalculatorId[]>([
    "creatinine",
    "bmi",
    "gcs",
    "cha2ds2",
  ...
  const [calculatorValues, setCalculatorValues] = React.useState<Record<string, string>>(() =>
    initialCalculatorValues("creatinine", patientContext),
  );
  ...
  const selected =
    medicalCalculators.find((calculator) => calculator.id === selectedCalculator) ??
    medicalCalculators[0];
  ...
  function runCalculation() {
    ...
    const input = calculatorDefinitions[selectedCalculator].fields
      .map((field) => {
            ...
            <div className="min-w-0">
              ...
              {screen === "input" ? (
                <CalculatorInput
                  ...
                  selected={selected}
                  onValueChange={updateCalculatorValue}
                  onReset={() =>
                    setCalculatorValues(initialCalculatorValues(selectedCalculator, patientContext))
                  }
...
function CalculatorHome({
  ...
  onChoose: (_id: CalculatorId) => void;
  onToggleFavorite: (_id: CalculatorId) => void;
  selectedCalculator: CalculatorId;
}) {
  const [query, setQuery] = React.useState("");
  const [category, setCategory] = React.useState<CalculatorCategory>("All");
  const favoriteCalculators = medicalCalculators.filter((calculator) =>
      ...
      <div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {favoriteCalculators.map((calculator) => (
            <CalculatorTile
              active={selectedCalculator === calculator.id}
          ...
          {!favoriteCalculators.length ? (
            <div className="w-full rounded-md border border-dashed border-border p-4 text-center text-sm text-muted-foreground">
              No frequently used calculators yet.
            </div>
          ) : null}
        </div>
      </div>
      <Card>
        ...
        <CardContent className="pt-3">
          <div className="mb-3 flex gap-1 overflow-x-auto border-b border-border">
            {categories.map((item) => (
              <button
                className={`shrink-0 border-b-2 px-3 py-2 text-xs font-medium ${category === item ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
                key={item}
...
function CalculatorInput({
  ...
  onCalculate: () => void;
  onReset: () => void;
  onValueChange: (_key: string, _value: string) => void;
            ...
            field.type === "checkbox" ? (
              ...
              >
                <input
                  ...
                  type="checkbox"
                  onChange={(event) => onValueChange(field.key, event.target.checked ? "1" : "0")}
                />
    ...
    return (
      <Card>
        <CardContent className="p-6 text-sm text-muted-foreground">
          No current result. Return to Input and calculate again.
        </CardContent>
      </Card>
    );
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Calculated result with interpretation and formula trace.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-md border border-success/25 bg-success/10 p-4">
            ...
            <div className="mt-1 text-4xl font-bold text-success">
              {calculation.value} <span className="text-sm font-semibold">{calculation.unit}</span>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Clinical safety</CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          Verify inputs, units, eligibility criteria, and local protocol. A calculated score
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <CardContent className="space-y-2">
          {medicalCalculators
            .filter((calculator) => favorites.includes(calculator.id))
            .map((calculator) => (
              <FavoriteRow
                calculator={calculator}
                key={calculator.id}
        ...
        <CardContent className="space-y-3">
          ...
          {medicalCalculators.map((calculator) => (
            <FavoriteRow
              ...
              onChoose={onChoose}
              onToggleFavorite={onToggleFavorite}
              starred={favorites.includes(calculator.id)}
            />
          ))}
        </CardContent>
      </Card>
...
function CalculatorSettings() {
  const [section, setSection] = React.useState<"General" | "Display" | "Safety" | "Audit">(
    "General",
  );
  ...
  React.useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      try {
        ...
        if (!stored) return;
        const preferences = {
          ...defaultCalculatorPreferences,
          ...JSON.parse(stored),
        } as CalculatorPreferences;
      ...
      <CardContent className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
        <div className="rounded-md border border-border bg-surface-muted p-2">
          {(["General", "Display", "Safety", "Audit"] as const).map((item) => (
            <button
              className={`block w-full rounded px-3 py-2 text-left text-sm ${section === item ? "bg-surface font-semibold text-primary shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
              key={item}
        ...
        <div className="space-y-4">
          {section === "General" ? (
            <div className="space-y-3">
              {toggle(
                ...
                "Prefill age, sex, height and weight from the current patient record.",
              )}
              {toggle(
                "confirmOverwrite",
                "Confirm manual-value overwrite",
                "Warn before replacing values entered manually.",
              )}
            </div>
          ) : null}
          {section === "Display" ? (
            <div className="space-y-3">
              {toggle(
                ...
                "Display the clinical category alongside the numeric result.",
              )}
              {toggle(
                "showFormulaTrace",
                "Show formula trace",
                "Display the formula or component-score breakdown.",
              )}
              <Field label="Decimal precision">
                <select
                  ...
                  onChange={(event) =>
                    update(
                      "decimalPlaces",
                      event.target.value as CalculatorPreferences["decimalPlaces"],
                    )
                  }
                >
          ...
          {section === "Safety" ? (
            <div className="space-y-3">
              {toggle(
                ...
                "Block calculations when values are missing or outside supported ranges.",
              )}
              <div className="rounded-md border border-warning/30 bg-warning/10 p-3 text-xs text-muted-foreground">
                Medical calculators support clinical decisions; they do not replace clinician review
                or local protocols.
              </div>
            </div>
          ) : null}
          {section === "Audit" ? (
            <div className="space-y-3">
              {toggle(
                "auditLog",
                "Enable calculation audit log",
                "Keep calculator, inputs, result and time in the patient-session history.",
              )}
            </div>
          ) : null}
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border pt-4">
            ...
            <div className="flex gap-2">
              ...
              <Button disabled={!dirty} type="button" onClick={save}>
                Save Changes
              </Button>
            </div>
          </div>
...
function CalculatorTile({
  ...
  onChoose: (_id: CalculatorId) => void;
  onRemove: (_id: CalculatorId) => void;
}) {
  return (
    <div
      className={`relative min-h-16 w-72 shrink-0 rounded-md border transition hover:border-primary/50 hover:bg-surface-muted ${active ? "border-primary bg-primary/5" : "border-border bg-surface"}`}
    >
      <button
        className="block w-full p-2.5 pr-9 text-left"
        onClick={() => onChoose(calculator.id)}
        type="button"
      >
        <div>
      ...
      <button
        aria-label={`Remove ${calculator.name} from Frequently Used`}
        className="absolute right-1.5 top-1.5 flex h-7 w-7 items-center justify-center rounded text-lg leading-none text-muted-foreground hover:bg-surface-muted hover:text-foreground"
        onClick={() => onRemove(calculator.id)}
        title="Remove from Frequently Used"
        type="button"
      >
        ×
      </button>
...
function fuzzyCalculatorMatch(calculator: (typeof medicalCalculators)[number], query: string) {
  ...
  const searchable =
    `${calculator.name} ${calculator.subtitle} ${calculator.category}`.toLowerCase();
...
function CalculatorList({
  ...
  const calculators = medicalCalculators.filter(
    (calculator) =>
  ...
  const list = (
    <div className="grid gap-2 md:grid-cols-2">
      {calculators.map((calculator) => (
        <div
          className={`flex items-center justify-between rounded-md border p-2 ${selectedCalculator === calculator.id ? "border-primary bg-primary/5" : "border-border"}`}
          key={calculator.id}
        >
          <button
          ...
          {favorites.includes(calculator.id) ? (
            ...
            <button
              aria-label={`Add ${calculator.name} to Frequently Used`}
              className="ml-2 shrink-0 rounded px-2 py-1 text-xs text-muted-foreground hover:bg-surface-muted hover:text-foreground"
  ...
  return (
    <div className="flex items-center justify-between rounded-md border border-border p-2">
      ...
      <button
        ...
        className="rounded px-2 py-1 text-xs text-muted-foreground hover:bg-surface-muted hover:text-foreground"
        onClick={() => onToggleFavorite(calculator.id)}
        type="button"
      >
        {starred ? "Remove" : "Add"}
...
export function PatientDetailsPage() {
  ...
  const calculatorHeight =
    clinicalHeight || (editingRecord ? getPatientRecordValue(editingRecord, "Height") : "");
  const calculatorWeight =
    clinicalWeight || (editingRecord ? getPatientRecordValue(editingRecord, "Weight") : "");
  ...
  async function handleDocumentFile(document: File | undefined) {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/patient-document/extract`, {
        method: "POST",
        body,
      });
      ...
      const sections = Array.isArray(result.sections)
        ? (result.sections as PatientRecordSection[])
        : [];
      if (!sections.length) {
      ...
      setFormKey((current) => current + 1);
      toast.success(
        `Document extracted. ${sections.reduce((count, section) => count + section.fields.length, 0)} fields are ready to review.`,
      );
    } catch (error) {
