...
import {
  ...
  bpValue,
  patientTone,
  patientToneCellClass,
  patientToneClass,
  patientToneRowClass,
  patientToneStripeClass,
...
export function PatientRow({
  ...
  return (
    <tr
      className={cn("h-[74px] border-b border-slate-100 font-semibold", patientToneRowClass(tone))}
    >
      <td
        className={cn(
      ...
      >
        ...
        <span
          ...
          className={cn(
            ...
            patientToneStripeClass(tone),
          )}
        />
        <Link
          className="relative z-10 flex min-h-[58px] flex-col justify-center rounded-md px-1 py-1 pl-2 transition hover:bg-slate-50"
          href={`/doctor-ipd/patients/${patient.id}`}
        >
          <div
            className={cn(
              "whitespace-nowrap text-sm font-extrabold leading-5",
              patientToneClass(tone),
            )}
          >
            {patient.name}
          </div>
          <div className="mt-0.5 break-words font-semibold leading-4 text-slate-700">
            {patient.bed}
          </div>
        </Link>
      </td>
      <td className="h-[74px] w-[230px] min-w-[230px] px-3 py-2 text-center font-medium text-slate-800">
        <Link
          className="flex min-h-12 items-center justify-center rounded-md px-2 py-1 leading-4 transition hover:bg-slate-100"
          href={`/doctor-ipd/patients/${patient.id}?tab=clinical-examination`}
        >
          {patient.diagnosis}
        </Link>
      </td>
      <td className="h-[74px] px-3 py-2 text-center">
        <VitalPill {...patient.hr} onClick={onOpenVitals} />
      </td>
      <td className="h-[74px] px-3 py-2 text-center">
        <VitalPill {...patient.spo2} onClick={onOpenVitals} />
      </td>
      <td className="h-[74px] px-3 py-2 text-center">
        <VitalPill value={bpValue(patient)} tone={bpTone(patient)} onClick={onOpenVitals} />
      </td>
      <td className="h-[74px] px-3 py-2 text-center">
        <VitalPill {...patient.temperature} onClick={onOpenVitals} />
      </td>
      <td className="h-[74px] px-3 py-2 text-center">
        <RoundActionButton
          icon={FlaskConical}
      ...
      <td className="h-[74px] px-3 py-2 text-center">
        <RoundActionButton
          dataTestId={`doctor-ipd-events-${patient.id}`}
          icon={Activity}
          tone="red"
