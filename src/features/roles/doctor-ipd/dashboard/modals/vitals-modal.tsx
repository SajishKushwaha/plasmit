...
import type {
  DoctorIpdPatient,
  VitalTone,
} from "@/features/roles/doctor-ipd/dashboard/dashboard.types";
...
export function DashboardVitalsPopup({ patient }: { patient: DoctorIpdPatient }) {
  ...
  const rapidReviewPatient = rapidReviewPatients.find(
    (item) => item.id === patient.rapidReviewPatientId,
  );
...
function DashboardSinglePatientMetric({
  label,
  value,
  tone,
  unit,
...
function dashboardVitalsForPatient(patient: DoctorIpdPatient) {
  return [
    ...
    {
      label: "Temperature",
      value: patient.temperature.value,
