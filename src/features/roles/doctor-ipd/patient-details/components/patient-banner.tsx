...
export function PatientBanner({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 px-3 transition-all duration-200 xl:px-4",
          isCompact ? "min-h-9 py-1" : "min-h-11 py-2",
        )}
      >
        <div className="horizontal-scrollbar min-w-0 overflow-x-auto overflow-y-hidden pb-1">
          <div
            className={cn(
              "flex min-w-full w-max flex-nowrap items-center justify-between transition-all duration-200",
              isCompact ? "gap-x-2 xl:gap-x-3" : "gap-x-3 xl:gap-x-4 2xl:gap-x-5",
            )}
          >
            <div className="flex shrink-0 items-center gap-2">
              <span className="max-w-[130px] truncate text-sm font-bold xl:max-w-[160px]">
                {patient.name}
              </span>
              <Badge
                className={cn(
            ...
            {details.map((item) => (
              <div
                className="shrink-0 whitespace-nowrap text-[13px] font-semibold leading-5 xl:text-sm"
                key={`${item.label}-${item.value}`}
              >
                {item.label ? (
                  <span
                    className={
                      item.label === "Isolation Type"
                ...
                <span
                  className={
                    item.label === "Isolation Type" ? "font-extrabold text-red-300" : undefined
                  }
                >
                  {item.value}
                </span>
            ...
            <div
              className={cn(
                "shrink-0 whitespace-nowrap text-[13px] font-bold leading-5 text-orange-300 xl:text-sm",
                !isCompact && "ml-0",
              )}
            >
              Allergies: Meropenem
